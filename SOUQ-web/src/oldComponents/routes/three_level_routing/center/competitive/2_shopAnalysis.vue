<!-- 店铺分析主页 -->
<template>
  <div class='keywordIndexContainer shopAnalysisContainer'>
    <!-- 添加产品弹窗 -->
    <el-dialog top="30vh" title="添加产品" :visible.sync="dialogShow.dialogFormVisible" width="25%">
      <div class='dialogContent' style="overflow:hidden;">
        <div class='dialogProAnsContainer'>
          <!-- asin -->
          <div class='dialogTabNAme'>店铺ID：</div>
          <div>
            <el-input type="textarea" :rows="4" placeholder="多个ID换行输入" v-model="dialogData.textarea" style="width:100%">
            </el-input>
          </div>
        </div>
        <!-- 是否马上添加到监控 -->
    ...
    <!-- 布局 -->
    <el-row :gutter="24" type='flex' justify="space-between" style="height:70px;">
      <!-- 输入框 -->
      <el-col :span="15">
        <el-input placeholder="店铺ID" v-model="inputText" clearable style='max-width:200px;'>
        </el-input>
    ...
    <!-- 表格数据 -->
    <el-table height="600px" v-loading="shopTableLoading" stripe ref="Table" :data="listData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
      <el-table-column type="selection">
      </el-table-column>
      <el-table-column label="店铺ID" align='center'>
        <template slot-scope="scope">
          <a class="aLink" target="_blank" :href="scope.row.sellerUrl" style="text-decoration:none;">{{ scope.row.sellerId!=""?scope.row.sellerId:"-"}}</a>
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      StatusValue: "",
      // 时间段
      tabPosition: this.$route.query.tabPosition || shopIndexDefaultDay,
      inputShop: "",
      inputText: "",
      tableHandle: {
        multipleSelection: [],
  ...
  methods: {
    ...
    addProduct() {
      ...
      params.sellerIds = removeArrEmpty(newskuArr); //functions
      if (params.sellerIds == "") {
        this.$alert("请至少输入一个店铺ID");
        return;
      }
      ...
      this.$axios
        ...
        .then(res => {
          ...
          if (res.data.code != 0) {
            ...
            this.$store.commit("competitive/shopTableLoading", false); // loading
            return;
          }
          this.$message({
            type: "success",
    ...
    handleClick(index, rowDetail) {
      ...
      this.$router.push(
        "/admin/competitive/checkShopAnalysis?taskId=" +
          rowDetail.taskId +
          "&sellerId=" +
          rowDetail.sellerId
      );
