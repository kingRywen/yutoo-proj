          ...
          <el-table-column align="center" v-if="column.operations" :prop="column.prop" :label="column.label" :width="column.width" :formatter="column.formatter">
            <template slot-scope="scope">
              ...
              <template v-for="operate in column.operations">
                <div v-if="operate.img" class="__img_wrapper">
                  <!-- <img v-for="operate in column.operations" :src="scope.row.imageUrl"> -->
                  <img class="loadingImg" :key="scope.row.imageUrl" v-lazy="scope.row.imageUrl"></div>
              </template>

              <el-button v-if="operate.label" v-for="operate in column.operations" :key="operate.label" size="small" type="primary" @click="operate.func(scope)">{{operate.label}}</el-button>
            </template>
          </el-table-column>
          <!-- 没有按钮的 -->
