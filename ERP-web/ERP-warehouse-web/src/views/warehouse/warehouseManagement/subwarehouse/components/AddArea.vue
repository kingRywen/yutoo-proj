<template>
  <div>
    <YtTableSelect
      ref="table"
      :queryParams="{'wmWarehouseId': id}"
      :options="options"
      :params="params"
      :editFn="edit"
      :editWidth="120"
      url="warehouse/wmWarehouseDivisionList"
    >
      <yt-dialog :options="addOpts" :events="addEvents" slot="add" name="添加">
        ...
        <div class="mt20 txc">
          ...
          <el-button @click="addOpts.visible = false">取消</el-button>
        </div>
      </yt-dialog>
      <div slot="del" uid="wmWarehouseDivisionId" uids="ids" url="warehouse/wmWarehouseDivisionDelete" name="删除"></div>
...
export default {
  ...
  methods: {
    edit(row) {
      ...
      this.addData = Array.isArray(row) ? row.map(el => ({...el, id: el.wmWarehouseDivisionId})) : [{...row, id: row.wmWarehouseDivisionId}]
