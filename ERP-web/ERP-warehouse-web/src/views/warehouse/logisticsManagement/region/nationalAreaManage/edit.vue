...
export default {
  ...
  watch: {
    params(val) {
      ...
      if (val.countryId) {
        ...
        vm.$api['warehouse/provinceList'](params).then(data => {
          let { rows } = data
  ...
  methods: {
    ...
    submitForm(cb) {
      let vm = this
      let provinceIdList = [...vm.selectData, ...Object.values(vm.obj)].map(
        item => item.provinceId
      )
      let logisticCountryId = vm.params.logisticCountryId
      let params = {
        provinceIdList,
        logisticCountryId
      }
    ...
    getList(type, ...obj) {
      ...
      let { countryId, logisticRegionId } = vm.params
      let params = {
        sortOrder: 'asc',
      ...
      vm.$api['warehouse/regionNationProvinceEditPage'](params)
        .then(data => {
