  ...
  <div class="print" v-loading="loading" :element-loading-text="loadingText">
    ...
    <template v-if="needCodePage.indexOf($route.query.title)>-1">
      ...
      <div id="print-div" :class="[type, prefixClass]">
        <div class="code" v-for="code in printCodes">
          <section v-if="type === 'bar'">
            <VueBarcode
              ref="bar"
              :value="$route.query.title==='打印采购清单产品条码' ? code.purchaseProductBarCode : code.productBarCode"
      ...
      <table :class="tableClass" v-for="(item, index) in data" :key="index">
        <tbody>
          ...
          <tr>
            ...
            <th style="word-break: break-all;">系统SKU
              <br>自定义SKU
            </th>
            <th style="word-break: break-all;">库位</th>
            <th style="word-break: break-all;" width="100">产品中文名
              <br>供应商SKU
            </th>
            <th style="word-break: break-all;">系统采购价</th>
            <th style="word-break: break-all;">需求数量
              <br>实际采购
            </th>
            <th style="word-break: break-all;">实际总金额</th>
            <th style="word-break: break-all;">备注</th>
          </tr>
          <tr v-for="(col, i) in item.products" :key="index">
            <td>{{col.ranking}}</td>
            <td>
          ...
          <tr>
            <td colspan="6">
              <el-row :gutter="12" class="purchaseList-wrapper">
                <el-col :span="12" class="box" v-for="i in item.products">
    ...
    <div v-if="$route.query.title==='打印成品加工单'">
      ...
      <table :class="tableClass" v-for="(item, index) in data" :key="index">
        <tbody>
          ...
          <tr v-for="(mate, i) in item.materials">
      ...
      <table :class="tableClass" class="invoice" v-for="(item, index) in data" :key="index">
        <tbody>
          ...
          <tr>
            ...
            <td colspan="2">USD {{getTotalShipping(item.products).toFixed(2)}}</td>
          </tr>
          <tr>
            <td colspan="2">
              <span class="title">Total amount</span>
            </td>
            <td colspan="2">USD {{item.totalAmount.toFixed(2)}}</td>
          </tr>
          <tr>
            <td colspan="4">
              <span class="title">The total declare value is {{getTotal(item).toFixed(2)}} USD ONLY</span>
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
    ...
    getData() {
      let api, params
      let { title, purchaseIds, purchaseOrderPinIds } = this.$route.query
      if (purchaseIds) {
