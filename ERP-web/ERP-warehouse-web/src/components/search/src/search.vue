                ...
                <el-form :inline="inline" :model="searchForm" class="searchForm" ref="form">
                    <template v-for="(item,index) in searchList">
                        <el-form-item :label="item.label || null" :key="index">
                            ...
                            <el-select
                                ...
                                :size="item.size || 'small'"
                            >
                                <el-option :disabled="j.disabled || false" :label="j.label ? j.label : j[item.props.label]" :value="j.value != null ? j.value : j[item.props.value]" :key="jindex" v-for="(j,jindex) in item.children"></el-option>
                            </el-select>
                            <!-- 时间 -->
                            <el-date-picker
                                v-if="item.type === 'time'"
                            ...
                            <el-select
                                ...
                                @change="handleChange(item,$event)"
                            >
                                <el-option :disabled="j.disabled || false" :label="j.label" :value="j.value" :key="jindex" v-for="(j,jindex) in item.children"></el-option>
                            </el-select>
                            <!-- 文本域 -->
                            <el-input v-if="item.type === 'textarea'" type="textarea" @clear="$emit('search', searchForm)" :autosize="item.autosize" :placeholder="item.placeholder || null" v-model="searchForm[item.value]"></el-input>
                            <!-- 级联选择 -->
                            <el-cascader
                            ...
                            <coutry
                                ...
                                @clear="() => {searchForm[item.value] = null;countryId = null;}"
                            ></coutry>
                        </el-form-item>
                    </template>
                    <!--  -->
                    <el-form-item>
                        <template v-if="searchBtnList.length">
                            <el-button v-for="(citem,cindex) in searchBtnList" :key="cindex" type="primary" @click="onSubmit(citem)" size="small">{{citem}}</el-button>
                        </template>
                        <template v-else>
