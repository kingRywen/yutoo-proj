<template>
  ...
  >
    ...
    <el-table-column
      ...
      :label="column.text"
      :width="column.width"
      :align="column.value=='nameCn'?'':'left'"
    >
      <template slot-scope="scope">
        ...
        <span v-if="column.add=='nameCn' && scope.$index !==0" class="add" @click="addCategoryNext(scope.row)">
          <i class="el-icon-circle-plus-outline"></i> 新增下級
        </span>
        <span v-if="column.flag">
          <!-- <span v-if="scope.row.activateFlag == true" class="el-icon-check"></span>
          <span v-else class="el-icon-close"></span>-->
          <span :class="{'el-icon-check':scope.row.activateFlag,'el-icon-close': !scope.row.activateFlag}"></span>
        </span>
      </template>
    </el-table-column>
...
export default {
  ...
  methods: {
    ...
    addCategoryNext(item) {
      // if (item.superId == 1) {
      //   this.$emit('addCategoryNext', [item.superId])
      // } else {
      //   let idlist = []
      //   //说明不是顶级目录
      //   /* eslint-disable */
      ...
      //       }
      //     }
      //     arr.unshift(item.emailClassificationId, item.superId)
      //     queryParentId(item.parent)
      //     idlist = arr
      //     //顶级目录
      //   } else {
      //     idlist = [item.emailClassificationId, item.superId]
      //   }
      //   this.$emit('addCategoryNext', idlist)
      // }
      this.$emit('addCategoryNext', item)
    },
