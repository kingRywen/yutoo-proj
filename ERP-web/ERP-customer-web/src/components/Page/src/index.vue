<template>
  <div class="yt-page">
    <twp-universal-layout
      ...
      @clickBtn="val => $emit('clickBtn', val)"
    >
      <!--  -->
      <slot>
        <template slot="search">
          <slot name="search">
            <twp-search :searchList="searchConfig" ref="search" @search="search"></twp-search>
          </slot>
        </template>
        <!--  -->
    ...
    <!--  -->
    <twp-dialog :options="dialogOpts" :events="events" v-if="!dialogEvents.use" @close="dialog.visible = false">
...
<script>
import twpUserDefined from './twpUserDefined'
import twpUniversalLayout from './twpUniversalLayout'
import twpDialog from './twpDialog'
import twpTable from './twpTable'
import twpSearch from './twpSearch'
import { filterObj } from './tools.js'
    ...
    getList(type) {
      ...
      this.$api[this.url](params).then(res => {
        if (this.fixedPage) {
          ...
          let { pageNo, pageSize, rows, total, buttonList = [] } = res
          pageNo && (this.pageNo = pageNo)
          pageSize && (this.pageSize = pageSize)
          total && (this.total = total)
          this.tableList = rows
          this.tableLoading = false
          this.$emit('btnChange', buttonList.map(item => item.buttonNum))
        }
      })
      // .catch(err => {
