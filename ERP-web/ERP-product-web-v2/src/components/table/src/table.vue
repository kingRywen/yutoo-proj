    ...
    >
      ...
      <template slot-scope="scope">
		...
		<template v-else-if="item['showTooltip']">
			<el-tooltip placement="top" effect="light" v-if="scope.row[item.value]">
				<span slot="content">{{scope.row[item.value]}}</span>
				<a v-if="item.urlItem" style="color: rgb(16, 142, 233);" :href="item.urlItem" target="_blank" class="fix__row_1">{{scope.row[item.value]}}</a>
				<span v-else class="fix__row_1">{{scope.row[item.value]}}</span>
			</el-tooltip>
			<span v-else>-</span>
		</template>
  ...
  render(h) {
    ...
    return (line && row[value] != null && row[value] !== '') ? (
      <el-tooltip effect="light" placement="top" open-delay={300}>
        ...
        <div class="wrap-overflow-2" style={{'-webkit-line-clamp': line}}>{content}</div>
      </el-tooltip>
    ) : (
      content
    )
...
export default {
  ...
  methods: {
	getPercent(item,scope){
		...
		if(scope.row[perField] || scope.row[perField] == 0){
			if(scope.row[perField]){
				let num = item.keepFew || 0
				var percent = Number(scope.row[perField]*100).toFixed(num);
				percent+="%";
				return percent;
			}else{
				return scope.row[perField] + '%'
			}			
		}else{
			return "-"
		}
