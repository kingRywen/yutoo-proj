...
//编辑判断提示消息  this.$isEdit(arr)
Vue.prototype.$isEdit = function (arr, list = true) {
...
// 下载模板
Vue.prototype.$download = function (path = '') {
  downloadFile(path)
}
...
Vue.prototype.$delete = function (url, arr, key) {
  ...
  if (
    !vm.utils.isString(url) ||
    (!vm.utils.isArray(arr) && !vm.utils.isObject(arr)) ||
    !vm.utils.isString(key)
  ) {
  ...
  if (vm.utils.isArray(arr)) {
    if (vm.$isEdit(arr, false)) {
      return Promise.reject()
    }
  ...
  return vm
    ...
    .then(() => {
      return vm.$api[url](params).catch(err => {
        console.log(err)
        vm.$message({
          type: 'info',
          message: err.msg
...
Vue.prototype.$jumpContractInfo = function (params) {
  window.open(
    `${loginRedirect}erp-finance/#/finance/associatedSettings/contractPrint?title=打印合同&contractModelId=${
    params.contractModelId
    }&purchaseIds=${params.purchaseOrderManageId}`
  )
  console.log(params)
  // window.open('')
}
...
Vue.prototype.$jumpOrderInfo = function (params) {
  GLOBAL.ajax[`order/orderGetOrderByNumber`]({orderNumber: params.orderNumPlatform})
    .then((data) => {
      console.log(data);
      window.open(
        `${loginRedirect}erp-order/#/salesManagement/associatedSettings/allOrderLists/lookOver?deliveryStatus=${data.deliveryStatusName}&orderServiceId=${data.orderId}&platformId=${data.platformId}&orderType=${data.orderType}`
      )
    })
}
 ...
 * options = [{
   ...
   type: 'number', //qq, phone, user, url, email,fax, number 或者['number',1] 0整数，1位小数，2,两位小数 默认为2位小数，discount折扣数（0.2,0.33）
   opt: [{
     condition: '!value',
     msg: '这个不能为空'
   }]
...
Vue.prototype.$resetForm = function (form, ...arg) {
  form = this.utils.clone(form)
