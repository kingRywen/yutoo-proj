      ...
      <div class="searchWarp">
        <el-form :inline="inline" :model="searchForm" class="searchForm" ref="form">
          <template v-for="(item,index) in searchLists">
            <el-form-item :label="item.label || null" :key="index" v-if="item.show ? (typeof item.show === 'function') ? item.show(searchForm) : item.show : true">
              <!-- 输入框 -->
              <el-input
              ...
              <el-select
                ...
                :loading="loading"
              >
                <el-option v-for="item in remoteData" :key="item.value" :label="item.label" :value="item.value"></el-option>
              </el-select>

              <!-- 区间搜索 extent-->
              <template v-if="item.type == 'extent' && (searchForm[item.value] ? true : searchForm[item.value] = [])">
                <el-input
                  :maxlength="item.max"
...
import costomTag from '../../costomTag/src/index'
