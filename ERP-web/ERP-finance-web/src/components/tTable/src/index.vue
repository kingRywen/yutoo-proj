            ...
            <div v-if="item.btns">
              <template v-for="(j,idx) in item.btns">
                <el-tooltip
                  :content="j.name"
                  placement="top"
                  v-if="j.show ? typeof j.show === 'function' ? j.show(scope) :true : true"
                  :open-delay="2"
                  :key="idx"
                >
                  <el-button v-if="j.type == 'text'" @click="handleClick(j, scope)" type="text">{{j.name}}</el-button>
                  <i
        ...
        >
          <template slot-scope="scope" slot="default">
            ...
            <div v-else style="display: inline-block">
              ...
              <div v-else style="display: inline-block">{{ defaultValue(scope.row[item.value])}}</div>
            </div>
          </template>
        </el-table-column>
      </template>
