<template>
    ...
    >
        <el-table-column fixed type="selection" width="55" align="center" v-if="selection" :selectable="selectableFunc" :reserve-selection="!!reserveSelection"></el-table-column>
        <slot name="left"></slot>
        <slot></slot>
        <el-table-column :show-overflow-tooltip="item.showOverflowTooltip " v-for="item in columns" :width="item.width" :min-width="item.minWidth" :key="item.label" :label="item.label" :align="item.align || 'center'">
            <template v-if="item.merge">
                <el-table-column v-for="k in item.merge" :width="k.width" :key="k.label" :prop="k.value" :label="k.label" :align="k.align || 'center'">
            ...
            <template slot-scope="scope">
                ...
                <div v-if="item['img']" class="toggle-wrapper">
                    <el-tooltip effect="light" placement="right">
                        <div slot="content" class="__img_wrapper mar-auto" :style="{width:(item.bulkyHeight || '300') + 'px', height: (item.bulkyHeight || '300') + 'px'}">
                        ...
                        <el-badge v-if="item.badge" :value="item.badge(scope)" style="z-index:999;margin:10px;">
                            <div>
                                <div class="__img_wrapper mar-auto" :style="{width:(item.style.width || '80') + 'px', height: (item.style.width || '80') + 'px'}">
                                    <img class="loadingImg" :key="scope.row[item.value]" v-lazy="scope.row[item.value]" />
                                </div>
                            </div>
                        </el-badge>
                    ...
                    <template v-if="item.toggleMethod && scope.row.products && scope.row.products.length> 1">
                        <el-button class="custom-btn" @click="handleToggleMethod(item, scope)" type="text" :icon="!scope.row._open ? 'el-icon-caret-bottom' : 'el-icon-caret-top'"></el-button>
                    </template>
                </div>
