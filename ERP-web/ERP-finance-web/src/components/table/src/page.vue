        ...
        <universal-layout :btnList="btnConfig" :pageNo="pageNo" :pageSize="pageSize" :total="total" @handleSizeChange="handleSizeChange" @handleCurrentChange="handleCurrentChange" @clickBtn="clickBtn">
            ...
            <slot>
                <template slot="search">
                    <slot name="search">
                        <search :searchList="searchConfig" ref="search" @search="search" :updatedModel="updatedSearch"></search>
                    </slot>
                </template>
                <!--  -->
...
export default {
    ...
    methods: {
        ...
        //设置自定义列表选中
        setDefaultData() {
            let key = this.customLocal
                ? this.customLocal
                : 'finance' + this.$route.name;
        ...
        getList(type) {
            ...
            if (this.staffType) {
                params.staffType = this.staffType;
            }
            if (type) {
                let search = {};
                if (this.searchData(this.searchInfo) === undefined) {
                    search = { ...this.searchInfo };
                } else {
                    search = this.searchData(this.searchInfo);
                }
            ...
            this.$api[this.url](params).then(res => {
                if (this.fixedPage) {
                    ...
                    this.$emit(
                        'btnChange',
                        buttonList.map(item => item.buttonNum)
                    );
                }
            });
            // .catch(err => {
