<template>
  <div class="t-page">
    <section class="search" v-if="searchFlag">
      <search :searchList="searchConfig" v-bind="$attrs" v-on="$listeners" @search="handleSearch"></search>
    </section>
    <section class="btns">
      <t-btns
        @clickBtn="clickBtn"
    ...
    <t-dialog ref="dialog" @btnClick="btnClick" v-bind="$attrs" v-on="$listeners" :listFlag="true"></t-dialog>
  </div>
</template>
      ...
      try {
        vm.$api[vm.url](params)
          .then(async data => {
            ...
            vm.total = total
            vm.tableData = await vm.handleResult(rows)
            // vm.tableData = rows
