      ...
      <ElRow style="width: 560px">
        <ElCol :span="5">
          <div class="wrap-left-min">
            <div class="btn-wrap">
              <ElButton type="primary" @click="handlePreview" size="mini">预览</ElButton>
            </div>
          </div>
        </ElCol>
        <ElCol :span="19">
...
export default {
  ...
  watch: {
    ruleList: {
      ...
      immediate: true,
      handler(val) {
        this.value = this.utils.clone(val)
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    handleAddRules(ruleType) {
      ...
      if (ruleType == '-') {
        ...
        if (ruleType == 'serialNum') {
          obj.value = []
          obj.name = '流水号'
        } else if (ruleType == 'randomLetter') {
          obj.name = '随机字母'
        } else if (ruleType == 'randomNum') {
          obj.name = '随机数字'
        } else if (ruleType == 'fixedNum') {
          obj.name = '固定数字'
        } else if (ruleType == 'fixedLetter') {
          obj.name = '固定字母'
        }
    ...
    handlePreview() {
      let vm = this
      this.$api[`finance/financeContractRulePreview`]({
        ruleInfoList: vm.ruleList
      }).then(({ number = '' }) => {
        this.preview = number
      })
    }
