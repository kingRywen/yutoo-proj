const state = {
  ...
  platformActiveList: [], // 应用平台列表,
  supplierListAll: [] // 供应商下拉列表
}
const getters = {
...
const actions = {
  ...
  getSupplierListAll({ commit }) {
    return GLOBAL.ajax['procurement/supplierListAll']()
      .then(res => {
        let data = res.rows
        commit('setSupplierListAll', data)
      })
      .catch(() => {
        return false
      })
  ...
  getPlatformActiveList({ commit }) {
    GLOBAL.ajax['product/platformActiveList']()
      .then(res => {
        commit('setPlatformActiveList', res.rows)
      })
      .catch(() => {
        return false
      })
...
const mutations = {
  ...
  setSupplierListAll(state, data) {
    state.supplierListAll = data.map(item => {
      return {
  ...
  setPlatformActiveList(state, list) {
    state.platformActiveList = list.map(item => {
      return {
        label: item.name,
