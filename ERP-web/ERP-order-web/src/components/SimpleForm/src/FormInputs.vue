<template>
    ...
    <!-- 选择 -->
    <el-select collapse-tags :multiple="item.multi" style="width:100%" v-else-if="item.type === 'select'" :value="currentVal" :placeholder="selectPlaceholder" @change="$emit('input', $event)" @clear="$emit('clear')" clearable>
        <el-option v-for="i in options" :key="i[labeVal]" :label="i[label]" :value="i[labeVal]"></el-option>
    </el-select>

    <!-- 树型展示 -->
    <el-cascader v-else-if="item.type === 'cascader'" :options="options" :props="item.bind" @clear="$emit('clear')" @change="$emit('input', $event)" :value="currentVal"></el-cascader>
    <!-- 日期范围 -->
    <el-date-picker
    ...
    <!-- 普通 -->
    <el-input :placeholder="item.label" v-else clearable :value="currentVal" @input="$emit('input', $event)" @clear="$emit('clear')"></el-input>
</template>
