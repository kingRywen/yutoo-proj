...
const state = {
  ...
  reasonsForNewListOrders: [], // 新增订单原因（海外仓）
  currencyType: [], // 货币类型
  paymentMethod: [], // 支付方式
  ...
  countryList: [], // 国家列表
  logisticCountrySearchList: [], // 所在国家列表

  allShopList: [], //所有平台的店铺

  platformList: [], //平台  //平台下有对应的店铺
  storeEbayList: [], //获取所有的ebay店铺
  ...
  bankList: [], //银行
  siteList: [], //站点列表
  ebayLabelGetCombobox: [], //标签列表

  riskTypeList: [], // 侵权风险
  logisticTypeList: [], // 物流属性
  productPlugSpecList: [], // 插头规格,
  emailAccountServiceList: [], // 订单来源店铺
  nationList: [], // 国家列表
  ebayPaypalAccountGetCombobox: [], // PayPal账户管理-获取下拉框
  platformShopList: [], // 所有平台店铺（树形结构）
  ebayStoreList: [] // ebay平台店铺（树形结构）
}
const getters = {
...
const actions = {
  initReasonList({ commit }, type) {
    let api = 'createReasonGetOrderTypeList'
    let params = {}
    if (type == 10) {
      // params.orderTypes = [3, 4, 5]
      // params.reserveFlag = 1
      api = 'createReasonGetOrderTypeListAll'
      params = {}
    } else {
      params.orderTypes = [+type]
    }
    return GLOBAL.ajax[`order/${api}`](params).then(data => {
  ...
  getEmailAccountServiceList({ commit }, type) {
    return GLOBAL.ajax[`email/emailAccountServiceList`]({}).then(data => {
      commit('setEmailAccountServiceList', data.rows)
    })
  },
  getSiteList({ commit }, type) {
    return GLOBAL.ajax[`product/siteList`]({
      platformId: type
    }).then(data => {
      commit('setSiteList', data.rows)
    })
  },
  getEbayLabelGetCombobox({ commit }, type) {
    return GLOBAL.ajax[`order/ebayLabelGetCombobox`]({}).then(data => {
      commit('setEbayLabelGetCombobox', data.rows)
    })
  },
  getStoreEbayList({ commit }, type) {
    return GLOBAL.ajax[`product/storeEbayList`]({}).then(data => {
      commit('setStoreEbayList', data.rows)
      return Promise.resolve(data.rows)
    })
  },
  getLogisticCountrySearchList({ commit }, type) {
    return GLOBAL.ajax[`warehouse/logisticCountrySearchList`]({}).then(data => {
      commit('setLogisticCountrySearchList', data.rows)
      return Promise.resolve(data.rows)
    })
  },
  getEbayPaypalAccountGetCombobox({ commit }, type) {
    return GLOBAL.ajax[`order/ebayPaypalAccountGetCombobox`]({}).then(data => {
      commit('setEbayPaypalAccountGetCombobox', data.rows)
    })
  },
...
const mutations = {
  setRiskTypeList(state, { data, type }) {
    /**
         ...
         */
    if (type === 2) {
      state.riskTypeList = data.map(el => ({
        label: el.label,
        value: el.label
      }))
    }
    ...
    if (type === 8) {
      state.logisticTypeList = data.map(el => ({
        label: el.label,
        value: el.label
      }))
    }
  },
  setReasonList(state, { payload, type }) {
    if (type == 10) {
      state.reasonsForNewListOrders = payload
    } else {
      state.reasonsForNewOrders = payload
    }
  },
  setEbayPaypalAccountGetCombobox(state, data) {
    state.ebayPaypalAccountGetCombobox = data.map(item => {
      return {
        label: item.name,
        value: item.id
      }
  ...
  setEmailAccountServiceList(state, rows) {
    state.emailAccountServiceList = rows.map(item => {
      return {
        label: item.nickName,
        value: item.userId
      }
  ...
  setSiteList(state, rows) {
    state.siteList = rows.map(item => {
      return {
  ...
  setEbayLabelGetCombobox(state, rows) {
    state.ebayLabelGetCombobox = rows.map(item => {
      return {
        label: item.name,
        value: item.id
      }
  ...
  setStoreEbayList(state, rows) {
    let storeEbayList = []
    rows.map((item, index) => {
      if (index == 0) {
        storeEbayList = item.stores.map(el => {
          return {
            label: el.storeName,
            value: el.storeId
          }
  ...
  setLogisticCountrySearchList(state, rows) {
    state.logisticCountrySearchList = rows.map(item => {
      return {
        label: item.countryName,
        value: item.countryId
      }
