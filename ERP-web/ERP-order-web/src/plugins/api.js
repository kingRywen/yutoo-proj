...
export async function getConfig() {
    let _CONST_PORT_CONFIG
    if (process.env.VUE_APP_MODE !== 'con') {
        _CONST_PORT_CONFIG = CONST_PORT_CONFIG
    } else {
        _CONST_PORT_CONFIG = await axios.get('/config.json')
        console.log(_CONST_PORT_CONFIG);
    }
    return _CONST_PORT_CONFIG
}
        ...
        config.forEach(api => {
            ...
            Object.defineProperty(this.api, `${namespace}${sep}${name}`, {
                value(outerParams, outerOptions) {
                    // 是否强制一一对应service api
                    const _data = _isEmpty(outerParams) ? params : strict ? _pick(_assign({}, params, outerParams), Object.keys(params)) : _assign({}, params, outerParams)
                    if (!isMock) {
                        //如果不是mock接口
                        baseURL = _CONST_PORT_CONFIG[namespace.toUpperCase() + '_URL']
                    }
