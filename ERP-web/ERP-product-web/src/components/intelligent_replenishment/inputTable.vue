<template>
  <section class="inputTable">
    ...
    >
      ...
      <el-table-column label="准备天数" align="center" min-width="120">
        <template slot-scope="scope">
          <p>运输天数：{{scope.row.transportDays | _formatData}}</p>
          <p>保底可售天数：{{scope.row.insuredDays | _formatData}}</p>
        </template>
      </el-table-column>
      <el-table-column prop="stockDays" label="备货天数" align="center" :formatter="_formatter"/>
      ...
      <el-table-column prop="produceLackCount" label="生产缺货数量" min-width="100" align="center" :formatter="_formatter"/>
      <el-table-column label="补货详情" align="center">
        <template slot-scope="scope">
          <!-- 在补 -->
          <div v-if="scope.row.remainingDays != null">
            <p>
              <span style="color:#00CCFF;">{{scope.row.remainingDays | _formatData}}</span>天
            </p>
