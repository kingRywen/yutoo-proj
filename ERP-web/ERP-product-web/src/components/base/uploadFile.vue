    ...
    <div class="upload">
      <div class="table-responsive">
        <div class="text-center p-5">
          <label :for="inputId" class="file-area" v-if="!files.length">
          ...
          <ul class="upload-allfiles">
            <li v-for="(file, index) in files" :key="file.id">
              ...
              <div class="bot">
                ...
                <div class="second" v-if="file.size">{{file.size | fixNumber}}KB</div>
                <v-progress-line
                  v-if="file.active || file.progress !== '0.00'"
                  :percent="+file.progress"
                  :status="file.active ? 'active': (file.error ? 'exception' : 'success')"
                  :stroke-width="5"
                ></v-progress-line>
                <div>
        ...
        <div class="btn-group" v-if="!$attrs.show">
          <v-row :gutter="10">
            ...
            <v-col span="4" style="text-align:right" v-if="!noEditPhotoBtn">
              <v-button type="primary" style="vertical-align: middle;" @click.prevent="linkToEditPhoto">平台图片编辑</v-button>
            </v-col>
          </v-row>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleDrop(e) {
      ...
      if (!/.*\.zip$/.test(e.dataTransfer.files[0].name)) {
        showToast('error', '只能上传zip格式的文件')
    ...
    linkToEditPhoto() {
      const params = top.location.href.split('.html?')[1]

      _storage.set('session', 'imageLibrary_param', params)
    ...
    inputFile1(newFile, oldFile) {
      ...
      if (newFile && !oldFile) {
        if (!/.*\.zip/.test(newFile.name)) {
          showToast('error', '只能上传zip格式的文件')
          return
        }

        this.$emit('setoldzipinfo')
        // 添加文件
        console.log(newFile)
