  ...
  <el-form :model="ruleForm" label-width="100px" :inline="true">
    ...
    <el-form-item label="同步人员：">
      <el-select v-model.number="ruleForm.userId" size="small" clearable>
        <el-option :label="item.nickName" :value="item.userId" :key v-for="item in userList"></el-option>
      </el-select>
    </el-form-item>
    <el-form-item>
  ...
  <el-table :data="recordList" style="width: 100%" @selection-change="handleSelectionChange">
    ...
    <el-table-column label="状态" align="center">
      <template slot-scope="scope">
        <p>
          <span :style="scope.row.synStatusName=='处理中'?'color:rgb(245, 108, 108);':''">{{ scope.row.synStatusName }}</span>
          <span v-if="scope.row.synStatusName!='处理中'">- 同步数量{{ scope.row.synCount }}</span>
        </p>
        <p
          v-if="scope.row.synStatusName!='处理中'"
        >成功{{(scope.row.synCount) - (scope.row.synFailureCount)}} - 失败{{scope.row.synFailureCount}}</p>
      </template>
    </el-table-column>
    <el-table-column prop="userName" label="同步人员" align="center"></el-table-column>
    <el-table-column prop="remarks" label="描述" align="center" show-overflow-tooltip></el-table-column>
    <el-table-column label="操作" align="center">
      <template slot-scope="scope">
        <el-button v-if="scope.row.reportPath!=null" type="text" @click="download(scope.row.reportPath)">下载报告</el-button>
        <el-button v-if="scope.row.synStatus == 3" type="text" @click="sync(scope.row)">重试</el-button>
      </template>
    </el-table-column>
  </el-table>
</template>
...
export default {
  ...
  methods: {
    ...
    sync(row) {
      fetchData({
        ...apis.REPLENISHSYN_RETRYSYN,
        data: { synProductId: row.synProductId }
      }).then(res => {
        if (res.data.code === 0) {
          this.$notify({
            title: '成功',
