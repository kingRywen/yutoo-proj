<template lang="html">
    ...
    <el-table-column label="周期监控参数" align="center">
      <template slot-scope="scope">
        <p v-if="scope.row.cycleMonitorParm">
          上架时间>={{scope.row.cycleMonitorParm[0].day || ' - '}}天: BSR排名
          <span>{{scope.row.cycleMonitorParm[0].bsrRanking.slice(0).pop().largeRankingEnd}}</span> ~
          <span>{{scope.row.cycleMonitorParm[0].bsrRanking.slice(0).pop().largeRankingStart}},</span>
          周期: {{scope.row.cycleMonitorParm[0].bsrRanking.slice(0).pop().cycle}}天, 幅度: {{scope.row.cycleMonitorParm[0].bsrRanking.slice(0).pop().rangeValue}}
          <span
            v-if="scope.row.cycleMonitorParm[0].bsrRanking[0].range === 1"
          >%</span>
          <el-tooltip content="Left center" placement="bottom" effect="light">
            <i class="el-icon-caret-bottom"></i>
            <div slot="content">
              <div v-for="(item, index) in getTrueData(scope.row.cycleMonitorParm)" style="margin-top:8px">
                上架时间{{index === 0 ? ' >=': ' <'}}{{item.day || ' - '}}天:
  ...
  <el-pagination
    ...
    :current-page="listsInfo.pageNo"
    :page-sizes="[10, 20, 30, 50, 100]"
    :page-size="listsInfo.pageSize"
    ...
    <el-form ref="ruleForm" label-width="120px" class="demo-ruleForm">
      ...
      <el-form-item label="缺货监控参数：">
        <div class="formItemAdd">
          ...
          <el-form-item label="保底可售天数" class="w80">
            <el-input size="small"/>
          </el-form-item>
          <el-form-item label="备货所需天数" class="w80">
            <el-input size="small"/>
          </el-form-item>
...
export default {
  ...
  methods: {
    getTrueData(cycleMonitorParm) {
      cycleMonitorParm = JSON.parse(JSON.stringify(cycleMonitorParm))
      let data = []
      cycleMonitorParm.forEach(el => {
        ...
        for (let i = 0; i < ranks.length; i++) {
          ...
          _ranks.push({
            ...
            largeRankingEnd: largeRankingEnd === 0 ? '∞' : largeRankingEnd
          })
        }
