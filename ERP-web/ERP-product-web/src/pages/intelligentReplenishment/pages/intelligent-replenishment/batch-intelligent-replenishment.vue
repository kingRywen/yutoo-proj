<template>
  <section class="intelligent-replenishment">
    ...
    <IRLayout>
      ...
      <template slot="header">
        <div class="title" style="display:flex">
          <div @click="back">
            ...
            {{$route.query.type === "FBA" ? '智能补货列表' : '生产列表'}}
          </div>
          <div>
            <img
              :src="lockStatus ? '/static/lock.png' : '/static/unlock.png'"
              width="30"
              @click.self="unlock"
        ...
        <ul class="title-btn">
          <li>
            <el-button type="primary" size="small" @click="exportData" :loading="exportBtnLoading">确认并导出数据</el-button>
          </li>
        </ul>
      </template>
      <!-- 搜索  -->
      ...
      <template slot="table">
        ...
        <el-table
          ...
          class="curtable"
        >
          <el-table-column type="selection" width="30" :disabled="true" :reserve-selection="true"/>
              ...
              <div class="imgbox">
                <el-tooltip placement="right" effect="light" :open-delay="300">
                  ...
                  <div class="divContent" style="position:relative">
                    <img :src="scope.row.productImage || '/static/no-image.jpg'">
                  </div>
                </el-tooltip>
                <el-button
                  v-if="scope.row.childs.length > 0 && scope.row._isOpen"
                  type="text"
          ...
          <el-table-column label="产品名称" width="130" align="left">
            <template slot-scope="scope">
          ...
          <el-table-column prop="localInventory" label="仓库库存" width="120" align="center" :formatter="_formatter"/>
          ...
          <el-table-column prop="produceingCount" label="采购中库存" width="80" align="center" :formatter="_formatter"/>
          <el-table-column label="当日销量/日均销量" width="140" align="center" :render-header="handlerRenderHeader2">
            <template slot-scope="scope">
              <el-tooltip
                placement="bottom"
                effect="light"
          ...
          <el-table-column label="预计日销量" width="120" align="center">
            <template slot-scope="scope">
              <div @click="tips">
                <el-input
                  size="small"
                  clearable
                  :disabled="scope.row.parentFlag && scope.row.childs.length > 0 && scope.row._isOpen || lockStatus"
                  v-model.number="scope.row.expectedDailySales"
                  @input="setVariantNum(scope.row, scope.$index, $event)"
                ></el-input>
              </div>
            </template>
          </el-table-column>
          <el-table-column label="运输方式" align="center" min-width="120px">
            <template slot-scope="scope">
              <div v-if="!scope.row._show">
                <p
                  v-for="(item, index) in scope.row.listSMP"
                  :key="index"
                  style="line-height:3em"
                >{{item.transportName | _formatData}}</p>
              </div>
              ...
              <el-tooltip placement="bottom" effect="light" content="Bottom Center">
                ...
                <img
                  src="/static/icons/intelligent_replenishment/more.png"
                  width="21"
                  height="21"
                  alt
                  class="more"
                  :class="scope.row._show ? 'show-reverse' : ''"
                  @click="toggleListSMPClick(scope.row)"
                >
              </el-tooltip>
            </template>
          </el-table-column>
          <el-table-column label="准备天数" align="center" min-width="130px">
            <template slot-scope="scope">
              <div v-if="!scope.row._show">
                <div v-for="(item, index) in scope.row.listSMP" :key="index" style="margin: 7px 0;">
                  <p>运输天数：{{item.transportDays | _formatData}}</p>
                  <p>保底可售天数：{{item.insuredDays | _formatData}}</p>
                </div>
              </div>

              <div v-else>
                <p>运输天数：{{scope.row.listSMP[0].transportDays | _formatData}}</p>
                <p>保底可售天数：{{scope.row.listSMP[0].insuredDays | _formatData}}</p>
              </div>
            </template>
          </el-table-column>
          <el-table-column prop="stockDays" label="备货所需天数" align="center" width="100px" :formatter="_formatter">
            <template slot-scope="scope">
              <div v-if="!scope.row._show">
                <p
                  v-for="(item, index) in scope.row.listSMP"
                  :key="index"
                  style="line-height:3em"
                >{{item.stockDays | _formatData}}</p>
              </div>

              <div v-else>
                <p style="line-height:3em">{{scope.row.listSMP[0].stockDays | _formatData}}</p>
              </div>
            </template>
          </el-table-column>
          <el-table-column prop="availableStock" label="可售库存" align="center" :formatter="_formatter"/>
          <el-table-column prop="availableDayCount" label="可售天数" align="center" :formatter="_formatter"/>
          <el-table-column
            prop="produceLackCount"
            label="欠缺仓库数量"
            align="center"
            min-width="100px"
            :formatter="_formatter"
          />
          <el-table-column prop="lackDayCount" label="缺货天数" align="center" :formatter="_formatter">
            <template slot-scope="scope">
              <div @click="tips">
                <div
                  v-if="scope.row._show"
                >{{scope.row.listSMP[0].lackDayCount != null && scope.row.listSMP[0].lackDayCount > 0 ? scope.row.listSMP[0].lackDayCount : '-'}}</div>
                <div v-else>
                  <div
                    style="margin-bottom:8px"
                    v-for="(item, index) in scope.row.listSMP"
                  >{{item.lackDayCount != null && item.lackDayCount > 0 ? item.lackDayCount : '-'}}</div>
                </div>
              </div>
            </template>
          </el-table-column>
          <el-table-column label="补货数量" width="200" fixed="right" align="center">
            <template slot-scope="scope">
              <div @click="tips">
                <el-input-number
                  size="small"
                  ...
                  :min="0"
                  v-if="scope.row._show"
                  :disabled="scope.row.childs.length > 0 && scope.row._isOpen || lockStatus"
                  :value="scope.row.listSMP[0].needReplenishNum || undefined"
                  @change="val => scope.row.listSMP[0].needReplenishNum = val"
                ></el-input-number>
                <div v-else>
                  <el-input-number
                    size="small"
                    ...
                    v-for="(item, index) in scope.row.listSMP"
                    :key="index"
                    :disabled="item.parentFlag || lockStatus"
                    :value="item.needReplenishNum || undefined"
                    @change="val => item.needReplenishNum = val"
                  ></el-input-number>
                </div>
              </div>
            </template>
    ...
    <!-- 提示解锁 -->
    <el-dialog title="提示" :visible.sync="tipVisible" width="50%">
...
export default {
  ...
  methods: {
    ...
    exportData() {
      let api =
        this.$route.query.type === 'FBA'
          ? apis.REPLENISHDETAILED_CONFIRMANDEXPORT
          : apis.REPLENISH_DETAILED_CONFIRM_AND_PRODUCE_EXPORT
      ...
      fetchData({
        ...
        if (res.data.code === 0) {
          ...
          this.$confirm('导出数据成功！', '提示', {
            ...
            type: 'success',
            callback: action => {
              if (this.$route.query.type === 'FBA') {
                this.$router.push('/IntelligentReplenishment/Replenishment')
              } else {
                this.$router.push('/IntelligentReplenishment/Produce')
              }
    ...
    getNewData(vm) {
      ...
      fetchData({
        ...
        data: {
          replenishType: this.$route.query.type,
        ...
        list.forEach((element, index) => {
          if (element.listSMP) {
            element.listSMP.forEach(el => {
              if (el.needReplenishNum < 0) {
                el.needReplenishNum = null
              }
            })
          }
          _list.push(element)
          if (element.childs && element.childs.length > 0) {
            element.childs.forEach(el => {
              if (el.listSMP) {
                el.listSMP.forEach(_el => {
                  if (_el.needReplenishNum < 0) {
                    _el.needReplenishNum = null
                  }
                })
              }
            })
            _list.push(...element.childs)
          }
        })

        console.log(list)
        vm.tableData = _list
        vm.hasSelect = _list.length
        vm.$nextTick(() => {
    ...
    setNum(row, index, val) {
      ...
      childs.forEach(child => {
        child.listSmp = JSON.parse(JSON.stringify(child.listSMP))
      })

      fetchData({
        ...apis.REPLENISH_FORMULA_CALCULATION,
        data: {
          replenishType: this.$route.query.type,
          listSmp: listSMP,
          parentFlag: row.childs && row.childs.length > 0,
          fnsku,
          avgSales,
          expectedAvgSales: val,
          childs
        }
      }).then(res => {
        ...
        if (code === 0) {
          let {
            ...
            fnsku: _fnsku,
            listSmp,
            expectedAvgSales: _expectedAvgSales
          } = data
          if (errorInfo && errorInfo.length > 0) {
            showToast('warning', errorInfo.join('<br>'))
          }

          if (_childs && _childs.length) {
            _childs.forEach(childEl => {
              const element = this.tableData.find(
                el => el.fnsku === childEl.fnsku
              )

              if (element) {
                const elementSmp = element.listSMP
                this.$set(
                  element,
                  'expectedDailySales',
                  childEl.expectedAvgSales
                )
                let smp = childEl.listSmp
                elementSmp.forEach(s => {
                  let _s = smp.find(el => el.sqId === s.sqId)
                  if (_s) {
                    s.lackDayCount = _s.lackDayCount
                    s.needReplenishNum = _s.needReplenishNum
                  }
                })
              }
            })
            ...
            let { listSMP: _listSMP } = element
            element.expectedAvgSales = _expectedAvgSales
            _listSMP.forEach(s => {
              let _s = listSmp.find(el => el.sqId === s.sqId)
              if (_s) {
                s.lackDayCount = _s.lackDayCount
                s.needReplenishNum = _s.needReplenishNum
              }
    ...
    // 列表-运输方式(提示框列表)
    getCurrentShippingList({ replenishProductId }, index) {
      fetchData({
    ...
    // 列表-在补(提示框列表)
    getCurrentReplenishmentNum({ replenishProductId, asin }, index) {
      fetchData({
    ...
    getAllSelected() {
      ...
      if (!Object.keys(selected).length) {
        selected = JSON.parse(
          this.$route.query.intelligentReplenishmentSelected
        )
      }
