    ...
    <section class="record">
        ...
        <div class="select">
            <el-form :inline="true" :model="selectForm" class="demo-form-inline selectform" size='medium'>
                ...
                <el-form-item label="导入人员：">
                      <el-input v-model="selectForm.merchantName" clearable></el-input>
                </el-form-item>
                <el-form-item>
        ...
        <div v-loading="loading"
            ...
            <el-table
                ...
                <el-table-column prop="nickName" label="导入人员" width="180" align='center'/>
    ...
    methods: {
        handleSizeChange(val) {
            ...
            if (this.selectForm.merchantName) {
                params = {
                    ...params,
                    merchantName: this.selectForm.merchantName
                };
            }
            if (this.selectForm.time != null && this.selectForm.time.length) {
                params = {
                    ...params,
                    startDate: this.selectForm.time[0],
                    endDate: this.selectForm.time[1]
                };
            }
            this.getImportRecord(params);
        },
        ...
        downloadAll() {
            if (this.downloadArr.length) {
                getData('GlobalUploadRecord/batchDownload', {
                    ids: this.downloadArr
                }).then(res => {
                    if (res.data.code == 0) {
        ...
        // 当个下载
        download(obj) {
            getData('GlobalUploadRecord/download', {
                globalUploadRecordId: obj.uploadRecordId
            }).then(res => {
                if (res.data.code == 0) {
                    window.location.href = res.data.path;
                } else {
