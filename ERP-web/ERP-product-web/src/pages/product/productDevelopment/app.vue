          ...
          <template slot="td" slot-scope="props">
            ...
            <div v-else-if="props.column.field === 'imgUrl'">
              <el-tooltip placement="right" effect="light" :open-delay="300">
                <div slot="content">
                  <img
                    :src="props.content && props.content.replace(/,h_\d+,w_\d+/ig, ',h_240,w_240') || '/static/no-image.jpg'"
                    :alt="props.column.productName"
                    width="240"
                  >
                </div>
...
export default {
  ...
  methods: {
    ...
    getUserList1() {
      fetchData({ ...apis.MERCHANT_USER_LIST }).then(res => {
        console.log(res)
        if (res.data.code === 0) {
          res.data.rows.map(el => {
            el.label = el.nickName
    ...
    handleCommand(command) {
      ...
      if (command === 'f') {
        showConfirm({
          ...
          onOk() {
            fetchData({
              ...apis.PRODUCTSRCDEVELOP_BATCHREMOVE,
              data: {
                id: data
              }
            }).then(res => {
  ...
  watch: {
    rangeDate2(val) {
      let _val = val || []
      this.params.startDate = _val[0]
      this.params.endDate = _val[1]
    },
