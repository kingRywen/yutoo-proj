<template lang="html">
    <div class="logistics_packaging">
        <el-breadcrumb separator-class="el-icon-arrow-right" class="nav">
            ...
            <el-breadcrumb-item>物流包材管理</el-breadcrumb-item>
        </el-breadcrumb>
        <!--  -->
        <div class="content">
            ...
            <!--  -->
            <el-row  class="btn">
                 <el-button type="primary" icon="el-icon-plus"  size="small" @click="openDialog('添加包材')">添 加</el-button>
                 <el-button type="danger"  icon="el-icon-delete"  :disabled="!selectPacking.length" size="small" @click="allDel">批量删除</el-button>
            </el-row>
            <!--  -->
            <el-table :data="packageList" border style="width: 100%" height="620px"  @selection-change="handleSelectionChange">
                ...
                <el-table-column  label="宽" align="center">
                    <template slot-scope="scope">{{scope.row.singleWide + scope.row.widthUnit}}</template>
                </el-table-column>
                <el-table-column  label="高" align="center">
                    <template slot-scope="scope">{{scope.row.singleHigh + scope.row.heightUnit}}</template>
                </el-table-column>
                ...
                <el-table-column  label="重量" align="center">
                    <template slot-scope="scope">{{scope.row.weight + scope.row.weightUnit}}</template>
                </el-table-column>
                <el-table-column   label="价格" align="center">
                    <template slot-scope="scope">{{scope.row.price + scope.row.priceUnit}}</template>
                </el-table-column>
                ...
                <el-table-column  label="操作" align="center" width="200">
                    <template slot-scope="scope">
    <el-button type="text" @click="openDialog('编辑包材',scope.row.logisticPackageId)">编辑</el-button>
    <el-button type="text" @click="openDialog('查看包材',scope.row.logisticPackageId)">查看</el-button>
    <el-button type="text" @click="del(scope.row.logisticPackageId)">删除</el-button>
        ...
        <!--  -->
        <el-dialog :title="dialogTitle"  width="600px"  :visible.sync="p_locationDialog">
            <el-form :model="form" :rules="rules" ref="form" label-width="100px">
                ...
                <div class="fxitem">
                    <el-form-item label="价格：" prop="price" class="first">
                        <el-input  v-model="form.price" size="medium" :disabled="islook"/>
                    </el-form-item>
...
export default {
    ...
    methods: {
        ...
        selectInfo(logisticPackageId) {
            getData('logistic/package/info', { logisticPackageId }).then(
                res => {
                    if (res.data.code == 0) {
        ...
        del(logisticPackageId) {
            this.$confirm('确定删除吗, 是否继续?', '提示', {
                ...
                this.remove({
                    logisticPackageIds: Array.of(logisticPackageId)
                });
            });
        },
        //获取单位
        getUnitList() {
            getData('global/unit/item/types/list', { types: [2, 3, 4] }).then(
                res => {
                    console.log(res);
                    if (res.data.code == 0) {
        ...
        //获取物流包材table
        getPackageList(
            params = { sortOrder: 'asc', pageSize: 10, pageNumber: 1 }
        ) {
        ...
        submitform(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    if (this.dialogTitle == '添加包材') {
                        this.send('logistic/package/save', this.form);
                    } else if (this.dialogTitle == '编辑包材') {
                        this.send('logistic/package/update', {
                            ...this.form,
                            logisticPackageId: this.logisticPackageId
                        });
                    }
                } else {
