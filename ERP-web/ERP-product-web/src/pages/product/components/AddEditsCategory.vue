<template lang="html">
    <div class="formContainer">
        <el-form :model="form" :rules="rules" ref="ruleForm" label-width="125px" size="small">
            ...
            <el-row>
                <!-- {{form.site}} -->
                <template v-for="(platform,index) in homologousSiteList">
  <el-col :span="12" :key="index">
    <el-form-item
      :label="platform[index].platformName + '类目映射'"
      v-if="platform.length"
      :prop="'site.' + index"
      :rules="{required: true, message: '请选择站点', trigger: 'change'}"
    >
      <el-checkbox-group v-model="form.site[index]">
        <el-checkbox-button :label="site" v-for="site in platform">{{site.siteNameZh}}</el-checkbox-button>
      </el-checkbox-group>
    </el-form-item>
  </el-col>
</template>
            ...
            <el-row>
                <!-- {{form.site}} -->
                <template v-for="(item,index) in form.site">
  <el-table
    :data="item"
    border
    style="width: 100%;margin-bottom:10px;"
    v-if="item.length && form.site.length"
    :key="index"
  >
    ...
    <el-table-column prop="type" :label="item[index]['platformName']+'类型'" align="center">
      <template slot-scope="scope">
        ...
        <p
          ...
          v-if="!scope.row.type && scope.row.flag"
        >请选择{{scope.row.platformName}}类型</p>
      </template>
    </el-table-column>
    ...
    watch: {
        'form.userPlatform'(platform) {
            this.homologousSiteList = platform.map(item => {
                ...
                for (let key in item) {
                    if (key == 'siteList') {
                        siteArr = item[key].map(citem => citem)
                    } else {
                        obj[key] = item[key]
                        obj['type'] = ''
                        obj['typeId'] = ''
                ...
                for (let i = 0; i < siteArr.length; i++) {
                    siteArr[i] = Object.assign({}, siteArr[i], obj)
                }
                return siteArr
            })
            ...
            console.log('homologousSiteList', this.homologousSiteList)
            let _this = this
            if (this.info.hasOwnProperty('siteId') && this.info['siteId']) {
                this.homologousSiteList.forEach((item, index) => {
                    item.forEach(citem => {
                        if (citem.siteId == _this.info.siteId) {
                            citem.type = _this.info.type
                            citem.typeId = _this.info.typeId
                            _this.form.site[index].push(citem)
                        }
                    })
                })
            }
        ...
        info: {
            handler(obj) {
                ...
                setTimeout(() => {
                    let platform = _this.platformLsit.find(
                        item => item.platformId == obj.platformId
                    )
                    this.$set(_this.form.userPlatform, 0, platform)
