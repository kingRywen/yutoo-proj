        ...
        <div style="width:100%;margin-top:20px">
          <el-form :inline="true" :model="ruleForm" :rules="rules" ref="ruleForm">
            <el-form-item prop="value">
              <el-input maxlength="20" clearable size="small" v-model="ruleForm.value" placeholder="请输入自定义值"></el-input>
            </el-form-item>
            <el-form-item prop="valueShort">
              <el-input maxlength="20" clearable size="small" v-model="ruleForm.valueShort" placeholder="请输入自定义缩写"></el-input>
            </el-form-item>
            <el-form-item>
...
export default {
  ...
  data() {
    let checkStr = (rule, value, cb) => {
      let arrs = _storage._get('local', 'ruleStr')
      if (!arrs.length) {
        showToast('error', '载入验证规则失败， 请刷新重试')
      }
      if (arrs.find(el => value.toUpperCase().indexOf(el.toUpperCase()) > -1)) {
        cb(new Error('不能包含平台名称'))
      }
      cb()
    }
    return {
      rules: {
        value: [
          { required: true, message: '请输入自定义值' },
          {
            validator: checkStr
          }
        ],
        valueShort: [
          { required: true, message: '请输入自定义缩写' },
          {
            validator: checkStr
          }
        ]
  ...
  methods: {
    ...
    add() {
      ...
      if (!vm.allItems.find(el => el.nameCn === value)) {
        vm.$refs.ruleForm.validate(valid => {
          if (valid) {
            vm.addItems.push({
              ...
              nameCn: value,
              nameEn: valueShort,
              code: vm.allItems[0] && vm.allItems[0].code ? '#fff' : null,
              categoryId,
              categoryPropertyId,
              propertyName,
              valueShort
            ...
            // vm.ruleForm.value = null
            // vm.ruleForm.valueShort = null
