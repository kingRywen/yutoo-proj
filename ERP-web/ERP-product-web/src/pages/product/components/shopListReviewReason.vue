<template>
  <div v-if="!$attrs.type">
    ...
    <div v-if="info && info[0]">
      <a v-if="info[0].downUrl" style="color:blue" target="_blank" :href="info[0].downUrl">下载</a>
...
import { fetchData, downloadFile } from "common/common";
...
export default {
  ...
  methods: {
    // download() {
    //   fetchData({
    //     url: this.info[0].downUrl,
    //     method: "get"
    //   }).then(res => {
    //     console.log(res);
    //     downloadFile(res.data, true);
    //   });
    //   //
    // },
    cancel() {
