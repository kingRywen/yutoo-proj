<template>
  <div class="box __batchMove">
    ...
    <div class="item-1">
      ...
      <el-table :row-key="pageType ==='shopProducts' ? 'storeProductId':'productId'" :data="variantList" ref="variantList" :key='variantTableKey' v-loading="variantListLoading" style="width: 100%" height="250" tooltip-effect="dark" :header-cell-style="{background:'#f5f7fa'}" @selection-change="handleVariantSelectionChange">
        ...
        <el-table-column label="属性" min-width="200">
          <template slot-scope="scope">
            ...
            <div>
              <div v-for="item in (scope.row.variantProperties || scope.row.properties)" :key="item.productStorePropertyId">{{item.propertyName}}: {{item.propertyValue}}</div>
            </div>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-table row-key="productId" :data="productMovePageShowList" :key='showTableKey' v-loading="showListLoading" style="width: 100%;margin-top: 5px;" height="250" tooltip-effect="dark" :header-cell-style="{background:'#f5f7fa'}" @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55" :reserve-selection="true">
        </el-table-column>
        <el-table-column label="图片" min-width="80">
          <template slot-scope="scope">
        ...
        <el-table-column :label="'属性'" min-width="200" show-overflow-tooltip="true">
          <template slot-scope="scope">
            <!-- <div v-if="pageType==='productList'">
              {{scope.row.categoryDetail}}
            </div> -->
      ...
      <div class="pagination-container">
        ...
        <el-pagination background @size-change="val => ShowListQuery.pageSize = val" @current-change="val => ShowListQuery.pageNumber = val" :current-page="ShowListQuery.pageNumber" :page-sizes="[10,20,30, 50]" :page-size="ShowListQuery.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="productMoveShowList.length">
        </el-pagination>
      </div>
      ...
      <el-form class="search-form search-form-end">
        <el-form-item>
          ...
          <el-button type="warning" size="mini" @click="handleParentFilter(null, parentListQuery.sku)">搜索</el-button>
...
export default {
  ...
  computed: {
    productMovePageShowList() {
      const { pageSize, pageNumber } = this.ShowListQuery
      console.log((pageNumber - 1) * pageSize, pageSize)
      return (
        this.productMoveShowList &&
        this.productMoveShowList.slice(
    ...
    selectedVar() {
      ...
      // console.log(arr);
      // return [...new Set(arr)];
      return this.$refs.variantList && this.$refs.variantList.selection
    }
  },
  methods: {
    init() {
            ...
            .then(res => {
              ...
              if (code === 0) {
                if (rows instanceof Object) {
                  ...
                  vm.formCategoryName = category.categoryName.replace(/;/g, '>')
                }
              } else {
                Message.error(msg || '服务器请求异常，请联系管理员')
              }
    ...
    getProductVariantMoveList(init) {
      ...
      if (this.pageType === 'shopProducts') {
        ...
        if (this.variantListQuery.parentSku && this.variantListQuery.parentSku.trim() !== '') {
          params.parentSku = this.variantListQuery.parentSku
          params.skuList = null
        } else if (this.textareaSku && this.textareaSku.trim() !== '') {
          params.skuList = this.textareaSku.split('\n')
          params.parentSku = null
        }
      }
      ...
      this.$nextTick(() => {
        setTimeout(() => {
          list.forEach((element, index) => {
            if (this.selectedVar.find(el => _isEqual(el, element))) {
              this.$refs.variantList.toggleRowSelection(element, true)
            }
          })
        })
      })
    },
    getProductMoveList(type, skus) {
      let vm = this
      vm.parentListLoading = true
      let url = type ? PRODUCT_LIST_SKU : PRODUCT_MOVE_PAGE_LIST
      let params = JSON.parse(JSON.stringify(vm.parentListQuery))
      // if (skus) {
      //     params.skuList = skus
      //   } else {
      ...
      if (this.pageType === 'productList') {
        if (vm.textareaSku !== null) {
          ...
          params = JSON.parse(JSON.stringify(vm.parentListQuery))
        }
        ...
        params = JSON.parse(JSON.stringify(vm.storeParentListQuery))
        params.selectedIds = this.productMoveShowListSelect && this.productMoveShowListSelect.map(el => el.storeProductId)
      }
      if (type) {
        delete params.categoryId
      }

      if (this.parentListQuery.sku && this.parentListQuery.sku.trim() !== '') {
          params.parentSku = this.parentListQuery.sku
          params.skuList = null
        } else if (this.textareaSku && this.textareaSku.trim() !== '') {
          params.parentSku = null
          params.skuList = this.textareaSku.split('\n')
    ...
    getCategoryOk() {
      ...
      this.$refs.variantList.clearSelection()
    ...
    handleVariantSelectionChange(val) {
      ...
      this.$set(this.selectedVars, this.variantListQuery.pageNumber - 1, val)

      let _val = this.selectedVar
      if (_val.length > 0) {
    ...
    customSkuList(type) {
      if (type !== 'parent') {
        this.variantListQuery.parentSku = ''
      } else {
        this.parentListQuery.sku = ''
      }
        ...
        if (this.pageType === 'productList') {
          fetchData({
            ...
            .then(res => {
              ...
              if (code === 0) {
                if (type === 'variant') {
                  let { failNum, successNum, errMsg, productVariantList } = rows
    ...
    check() {
      ...
      if (this.pageType === 'productList') {
        ...
        vm.$root.$children[0].setLoading('挪动中...')
        fetchData({
          ...PRODUCT_VARIANT_MOVE_CHECK,
          data: params
        })
          .then(res => {
            vm.$root.$children[0].stopLoading()
