        ...
        <div v-else>
          <v-form direction="horizontal" :model="ruleForm" :rules="rules" ref="ruleForm">
            <v-row>
              ...
              <v-col span="24">
                <v-form-item :key="ruleForm.auditStatus" label="原因" :label-col="labelCol" :wrapper-col="wrapperCol" :required="ruleForm.auditStatus === 3" :rules="[{required:ruleForm.auditStatus === 3 ? true : false, message: '请填写不通过的原因'}]" prop="auditReason">
  ...
  computed: {
    ...
    srcPurchase() {
      let hash =
        ...
        this.$attrs.item.productId +
        "&isshow=true&type=2&review=true";
      return hash;
    }
  }
};
