    ...
    <keep-alive>
      <v-modal title="添加产品" :width="1000" :visible="visible" @ok="handleOk" @cancel="handleCancel">
        <v-data-table
          ...
          :columns="columns1"
          check-type="checkbox"
          :current-data.sync="tableData1"
        >
          <template slot="td" slot-scope="props">
            <div v-if="props.column.field=='imgPath'">
              <img :src="props.content || '/static/no-image.jpg'" style="max-width: 49px; max-height:49px">
            </div>
            <div v-else>{{props.content}}</div>
          </template>
        </v-data-table>
      ...
      loadData1(pramas) {
        return fetchData({
          ...
          data: {
            ...
            sortOrder: 'asc',
            pageSize: 10,
            pageNumber: pramas.pageNo,
            ...pramas
          }
        }).then(res => {
          let {
            rows: { pageNo, pageSize, total, rows }
          } = res.data
          return {
            result: rows,
            totalCount: total,
