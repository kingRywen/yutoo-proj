<template lang="html">
    <div class="correlation">
        ...
        <div class="correlation_select">
            <el-form label-position="right" label-width="125px" :model="form" :rules="rules" ref="form" class="form">
                ...
                <div class="rows">
                     ...
                     <el-form-item label="选择类目来源：" v-show="isSource" prop="categoryId">
                        <!-- <el-cascader
                            ...
                            size="small"/> -->
                        <SyncCascader ref="platformTree" v-model="form.categoryId" :key="form.siteId"  :props="platformProps" :load="form.siteId?loadTree:loadTree1" size="medium"/>
...
export default {
    ...
    methods: {
        ...
        getTemplate(siteCategoryId) {
            getData('product/plt/detail/module', { siteCategoryId }).then(
                res => {
                    if (res.data.code == 0) {
                ...
                if (valid) {
                    if (vm.form.type == 4) {
                        ...
                        vm.$set(
                            vm.uploadData,
                            'categoryId',
                            vm.form.categoryId[vm.form.categoryId.length - 1]
                        );
                    }
    ...
    watch: {
        'form.type'(val) {
            if (val == 1)
                this.uploadUrl =
                    BASE_URL + '/template/store/product/rel/upload';
            if (val == 2)
                this.uploadUrl = BASE_URL + '/product/batch/update/upload';
            if (val == 3)
                this.uploadUrl =
                    BASE_URL + '/product/plt/detail/batch/update/upload';
            if (val == 4)
                this.uploadUrl = BASE_URL + '/template/store/product/upload';
        },
        'form.categoryId'(val) {
