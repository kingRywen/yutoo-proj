<template lang="html">
    <div class="title-management">
        <el-breadcrumb separator-class="el-icon-arrow-right" class="nav">
            ...
            <el-breadcrumb-item :to="{path:'/skuTitle'}">SKU平台标题管理</el-breadcrumb-item>
            <el-breadcrumb-item>标题库管理</el-breadcrumb-item>
        </el-breadcrumb>
        <el-button size="small" type="text" icon="el-icon-back" style="float: right;position: relative;top: -33px;right:10px;" @click="$router.back(-1)">返 回</el-button>
        <div>
            <div class="left">
                <div class="input_select">
                    <el-input size="medium" v-model.trim="filterText" placeholder="关键搜索" clearable suffix-icon="el-icon-search">
                ...
                <el-tabs type="border-card" class="tabs" @tab-click="changeLanguage" v-model="activeLanguage">
                    <template v-for="item in languageList">
                        <el-tab-pane :label="item.languageName" :name="item.languageId+''" :key="item.languageId">
                            ...
                            <content-table    
                                v-if="activeLanguage==item.languageId"                            
...
export default {
    ...
    methods: {
        ...
        del(titleGroupContentId) {
            getData('title/group/content/remove', { titleGroupContentId }).then(
                res => {
                    if (res.data.code == 0) {
                        this.$message({
                            message: `成功${res.data.rows.success}，失败 ${
                                res.data.rows.fail
                            }`,
                            type: 'success'
                        });
        ...
        open(title, titleGroupContentId) {
            ...
            if (titleGroupContentId) {
                getData('title/group/content/info', {
                    titleGroupContentId
                }).then(res => {
                    if (res.data.code == 0) {
                        this.name = res.data.rows.name;

                        this.redactParams.titleGroupContentId =
                            res.data.rows.titleGroupContentId;
