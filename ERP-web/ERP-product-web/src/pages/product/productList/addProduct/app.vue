        ...
        >
          ...
          >
            <keep-alive>
              <component
                ref="com"
                v-if="refresh && (name === item.title)"
                :is="item.component"
                v-bind="{edit, catelogy, allData, formatField, parentSKU, baseTab, productId, tabInfo, remarkTab, show}"
                @set="setData"
                @openload="openSpin"
                @close="closeSpin"
                @setSkuCustom="setSkuCustom"
...
const Portfolios = () =>
  import('./components/portfoliosNew.vue').then(m => m.default)
...
export default {
  ...
  created() {
    ...
    let groupFlag = (this.groupFlag = getQueryString('groupFlag'))
      ...
      fetchData({
        ...
        if (res.data.code === 0) {
          ...
          this.$nextTick(() => {
            ...
            // };
            this.allData.history = res.data.auditDataList
          })
        }
      })
    }
  ...
  methods: {
    ...
    // 切换标签之前的钩子
    handleBeforeLeave(activeName, oldName) {
      let vm = this

      if (top.location.href.indexOf('product/productList/addProduct.html') === -1 && top.location.href.indexOf('product/productList/editAll.html') === -1) {
        return Promise.resolve()
      }
      // 新增页
      // 如果变体页没有保存,提示一下是否保存
      if (!(this.review || this.isReview)) {
        if (oldName === '产品变体') {
          let _com = this.$refs.com.find(
            el => el.$options._componentTag === 'productVariant'
          )
          // 如果数据为空不跳转
          let _resolve = true
          let { tableData, isCreate } = _com
          let warn = '请先填写完整的变体属性'
          if (!tableData.length) {
            _resolve = false
          } else if (!isCreate) {
            _resolve = false
            ...
            tableData.forEach(el => {
              for (const key in el) {
                if (el.hasOwnProperty(key) && !isNaN(key)) {
          ...
          return new Promise((resolve, reject) => {
            ...
            if (
              ...
              showToast('warning', '请先保存变体数据')
              reject()
            }
            return _resolve ? resolve() : reject()
          })
        }
        if (oldName === '基本信息' || oldName === '产品备注') {
          let _com = this.$refs.com.find(el =>
            oldName === '基本信息'
              ? el.$options._componentTag === 'BasicInformation'
              : el.$options._componentTag === 'Remark'
          )
          if (!_isEqual(_com.originData, _com.getSavedData())) {
            return Promise.resolve().then(() => {
              return new Promise((resolve, reject) => {
                showConfirm({
                  title: '提示',
                  content: `是否保存 <span style="color:red;font-weight:bolder">${oldName}</span> 的数据?`,
                  onOk() {
                    _com.save()
                    resolve()
                    vm.$nextTick(() => {
                      _com.originData = _com.getSavedData()
                    })
                  },
                  onCancel() {
                    resolve()
    ...
    getVariantData(id) {
      ...
      fetchData({
        ...
        if (res.data && res.data.remarkTabFied) {
          this.formatField.remarkTabFied = this.setField(res.data.remarkTabFied)
        }
