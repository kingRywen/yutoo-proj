      ...
      <v-row>
          ...
          <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="供应商" required prop="supplierId">
            <div class="flex">
              <v-select
                ...
                size="lg"
              ></v-select>
              <v-button style="margin-left:10px" type="primary" @click="addLink">添加供应商</v-button>
            </div>
          </v-form-item>
        </v-col>
        <v-col span="24">
        ...
        <v-col span="24">
          <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="供应商发货地" required prop="supplierAddressId">
            ...
            <SyncCascader
              ...
              :load="lists"
              :props="prop"
              v-model="ruleForm.supplierAddressId"
              @change="$refs.ruleForm.validate()"
            ></SyncCascader>
          </v-form-item>
        </v-col>
                ...
                <v-row class="__row">
                      ...
                      <v-row>
                        <v-col span="11">
                          <v-form-item
                            style="width:100%"
                            required
                            :prop="'priceArr.' + index + '.numStart'"
                            :rules="numRuleStart(priceModel.priceArr, index)"
                          >
                            <v-input-number
                              size="large"
                              style="width:100%"
                              :disabled="index === 0"
                              :value.number="item.numStart || ''"
                              @input="val => (index === 0 ? '': priceModel.priceArr[index-1].numEnd = parseInt(val) -1, item.numStart = val)"
                            ></v-input-number>
                          </v-form-item>
                        </v-col>
                        ...
                        <v-col span="11">
                          <v-form-item
                            style="width:100%"
                            required
                            :prop="'priceArr.' + index + '.numEnd'"
                            :rules="numRuleEnd(priceModel.priceArr, index)"
                          >
                            <v-input-number
                              size="large"
                              style="width:100%"
                              :value.number="item.numEnd || ''"
                              @input="val => (!priceModel.priceArr[index+1] ? '': priceModel.priceArr[index+1].numStart = parseInt(val) +1, item.numEnd = val)"
                              :min="1"
                            ></v-input-number>
                          </v-form-item>
                        </v-col>
                  ...
                  <v-col span="8">
                    <v-form-item
                      ...
                      label="单价"
                      required
                      :prop="'priceArr.' + index + '.supplierPrice'"
                      :rules="numFloat"
                    >
                      <price-toggle
                        @reset="reset"
                        ...
                        :data="priceUnits"
                        v-model.number="item.supplierPrice"
                        :unit.sync="item.supplierPriceUnit"
                        :min="1"
                      ></price-toggle>
                    </v-form-item>
                  </v-col>
                  <v-col span="8">
                    <v-form-item
                      ...
                      label="采购运费"
                      required
                      :prop="'priceArr.' + index + '.purchaseFreight'"
                      :rules="numFloat"
                    >
                      <price-toggle
                        @reset="reset"
                        v-if="priceUnits.length"
                        :data="priceUnits"
                        v-model.number="item.purchaseFreight"
                        :unit.sync="item.purchaseFreightUnit"
                      ></price-toggle>
                    </v-form-item>
                  </v-col>
                </v-row>
                <v-row class="__row">
                  <v-col span="8">
                    <v-form-item
                      ...
                      label="加工费用"
                      required
                      :prop="'priceArr.' + index + '.processPrice'"
                      :rules="numFloat"
                    >
                      <price-toggle
                        @reset="reset"
                        v-if="priceUnits.length"
                        :data="priceUnits"
                        v-model.number="item.processPrice"
                        :unit.sync="item.processPriceUnit"
                      ></price-toggle>
                    </v-form-item>
                  </v-col>
                  <v-col span="8">
                    <v-form-item
                      ...
                      label="其它费用"
                      required
                      :prop="'priceArr.' + index + '.otherPrice'"
                      :rules="numFloat"
                    >
                      <price-toggle
                        @reset="reset"
                        v-if="priceUnits.length"
                        :data="priceUnits"
                        v-model.number="item.otherPrice"
                        :unit.sync="item.otherPriceUnit"
                      ></price-toggle>
                    </v-form-item>
                  </v-col>
                </v-row>
        ...
        <v-col span="8">
          <v-form-item :label-col="{span:12}" :wrapper-col="{span:12}" label="生产天数" required prop="produceTime">
            <v-input-number
              @change="val => ruleForm.produceTime  = val || ''"
              :min="1"
              :value.number="ruleForm.produceTime || ''"
              size="large"
              style="width:100%"
            ></v-input-number>
          </v-form-item>
        ...
        <v-col span="8">
          <v-form-item :label-col="{span:12}" :wrapper-col="{span:12}" label="运输天数" required prop="freightTime">
            <v-input-number
              @change="val => ruleForm.freightTime  = val || ''"
              :min="1"
              :value.number="ruleForm.freightTime || ''"
              size="large"
              style="width:100%"
            ></v-input-number>
          </v-form-item>
        ...
        <v-col span="6">
          <v-form-item :label-col="{span:12}" :wrapper-col="{span:12}" label="入库天数" required prop="storageTime">
            <v-input-number
              @change="val => ruleForm.storageTime  = val || ''"
              :min="1"
              :value.number="ruleForm.storageTime || ''"
              size="large"
              style="width:100%"
            ></v-input-number>
          </v-form-item>
...
export default {
  ...
  methods: {
    ...
    addLink() {
      window.open('/product.html#/product/SupplierManagement.html')
    },
    reset() {
      let vm = this
      setTimeout(() => {
