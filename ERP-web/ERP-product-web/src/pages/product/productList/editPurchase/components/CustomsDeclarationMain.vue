...
export default {
  custom: 'CustomsDeclarationMain',
  inheritAttrs: false,
  name: 'CustomsDeclarationMain',
  components: {
    CustomsDeclaration
  },
  data() {
  ...
  created() {
    ...
    fetchData({
      ...
      if (res.data && res.data.code === 0) {
        ...
        vm.getPricePercent().then(data => {
          if (data) {
            const {
              supplierPrice,
              entryPricePercentEntityList,
              supplierPriceUnit
            } = data
            if (supplierPrice) {
              ...
              let amPercentEl =
                entryPricePercentEntityList &&
                entryPricePercentEntityList.find(el => el.nationName === '美国')
              if (amPercentEl) {
                amPercent = amPercentEl.entryPricePercent
              }

              let priceOrigin = supplierPrice
              ...
              entryInfoPrice =
                priceOrigin && priceOrigin !== 0
                  ? (priceOrigin * amPercent) / 100
                  : ''
            } else {
            }
          }
          if (isNaN(entryInfoPrice)) {
            entryInfoPrice = undefined
          }
          // debugger;
          // 审核提示
          vm.auditDataList = res.data.auditDataList || []
          ...
          res.data.variantEntryInfoEntityList.forEach(element => {
            ...
            vm.$set(vm.ruleForm, sku, {
              ...element,
              skuCustom,
              reviewData: variantChangeMap ? variantChangeMap[sku] : {},
              edit: false,
          ...
          vm.originData = JSON.stringify(this.getOriginData())
        })
      }
    })
  },
  methods: {
    ...
    getEntryData() {
      ...
      // debugger;
      let productEntryPriceEntityList = ret.parentEntryInfoEntity
        ? _cloneDeep(ret.parentEntryInfoEntity.productEntryPriceEntityList)
        : []

      ret.productEntryPriceEntityList = productEntryPriceEntityList
      if (ret.parentEntryInfoEntity) {
  ...
  watch: {
    ...
    parentData(val) {
      try {
        let res = Object.assign({}, this.priceTabOld.parentEntryInfoEntity, val)
