            ...
            <el-tabs style="height: 100%;" tabPosition="left" type="border-card" v-model="reviewName">
              ...
              >
                <div v-if="item.productPltDetailId != null" span="24" style="margin-bottom: 70px;">
                  <v-form direction="horizontal" :model="item" :rules="rules" :ref="'ruleForm' + item.name">
                    <v-row>
                      <v-col span="24">
...
export default {
  ...
  methods: {
    ...
    save() {
      let forms = this.$refs['ruleForm' + this.reviewName]
      Promise.all(
        forms.map(
          (form, index) =>
        ...
        .then(res => {
          ...
          this.reviewTabs.forEach(el => {
            if (el.productPltDetailId && el.name === this.reviewName) {
              // console.log(el.productPltDetailId, forms[0].model.name)
              let data = {
                // FIXME:
                ...
                productPltDetailId: el.name !== '图片信息审核' ? el.productPltDetailId : this.ruleForm.productPltDetailId,
                productPltAuditId: this.ruleForm.productPltAuditId,
                auditType: 2,
              }
              if (el.name !== '图片信息审核') {
