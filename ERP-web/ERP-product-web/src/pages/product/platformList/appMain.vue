  ...
  <CustomTransition>
    <div>
      <v-form :model="ruleForm">
        ...
        <v-form-item label="编辑状态">
          <v-select
            style="width: 120px;"
            size="lg"
            v-model="ruleForm.productPlatformStatus"
      ...
      <div style="display:inline">
        ...
        <v-button type="primary" @click="bacthAssignPeople">批量分配人员</v-button>
        <v-button type="primary" @click="bacthAction('invalid')">批量作废</v-button>
        <v-button type="primary" @click="bacthAction('remove')">批量删除</v-button>
            ...
            <template slot="td" slot-scope="props">
              <div @click="linkToView(props)" style="color:#333;height:100%;width:100%;cursor: pointer;">
                ...
                <div v-else-if="props.column.field=='statusType'">
                  ...
                  <v-popover
                    trigger="hover"
                    v-if="getStatus(props) == 2 || getStatus(props) == 3 || getStatus(props) == 4 || getStatus(props) == 7"
                  ...
                  >
                    ...
                    <div
                      v-if="props.item.storeList && props.item.storeList.length > 0"
                    >待上架({{props.item.storeList.length}})</div>
                    <div slot="content" class="platform_status">
                      ...
                      <div class="platform_status_col">
                        ...
                        <div class="platform_status_col_item">
                          <div v-for="(val, key) in props.item.auditMap">
                            ...
                            <span v-else-if="val == 2" style="color: #f00">待</span>
                      ...
                      <div class="platform_status_col">
                        ...
                        <div class="platform_status_col_item">
                          <div v-for="item in props.item.storeList">{{item}}</div>
                        </div>
                      </div>
                    </div>
                  </v-popover>
                ...
                <div v-else-if="props.column.field=='operatorName'">
                  <div v-if="hasEditor(props.item.languageOperator)">
                    <template v-if="props.item.languageOperator.lenght <= 2">
                      <div v-for="item in props.item.languageOperator" :key="item">{{ item }}</div>
                    </template>
                    <template v-else>
                      <el-tooltip placement="left">
                        <div>
                          <div v-for="(item, index) in props.item.languageOperator.slice(0,2)" :key="item">
                            <template v-if="index === props.item.languageOperator.length-1">{{ item }} ...</template>
      ...
      <el-dialog width="40%" title="选择店铺" :visible.sync="selectShopDialog" append-to-body class="____platform__app">
        <v-row :gutter="16">
          <v-col span="8">
            <el-select v-model="searchStore.siteId" clearable placeholder="请选择">
              <el-option v-for="item in options" :key="item.siteId" :label="item.siteNameZh" :value="item.siteId"></el-option>
            </el-select>
          </v-col>
          <v-col span="16">
            <el-input
              ...
              v-model="searchStore.name"
              clearable
              @clear="selectShopName"
            >
        ...
        <div class="Lists_result">
          ...
          <el-table
            :data="shopList"
            :show-header="false"
            style="width: 100% !important;margin-top:15px;"
            @selection-change="checkedShops"
            ref="elTable"
        ...
        <div slot="footer" class="dialog-footer" style="text-align: center">
          <el-button type="primary" @click="selectShop" :loading="btnLoading">确 定</el-button>
        </div>
      </el-dialog>
    </div>
  </CustomTransition>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      lists: data => {
        ...
        if (reqData.skuType === 2) {
          delete reqData.sku
        } else {
          delete reqData.skuCustom
        }

        reqData.platformId = this.platform
        ...
        return fetchData(opts).then(res => {
          ...
          vm.allBtns[8].number = numMap.invalidNum
          vm.allBtns[9].number = numMap.putAwayNUm
          return {
            result: page.rows,
  ...
  methods: {
      ...
      showConfirm({
        ...
        onOk() {
          fetchData({
            ...
            const { fail, code, success } = res.data
            if (code === 0) {
              if (fail && !success) {
                showToast('error', `下架失败<br>${res.data.msg.join('.<br>')}`)
              } else if (fail && success) {
                showToast(
                  'error',
                  `<p>成功： ${res.data.success} 个, 失败： ${
                    res.data.fail
                  } 个。</p> <p>错误信息： ${res.data.msg.join('.<br>')}</p>`
                )
              } else if (!fail && success) {
    ...
    isSelect(type) {
      ...
      if (data.length <= 0) {
        showToast('warning', '请选择至少一项进行操作')
        return
      } else {
        let productPlatformIds = data.map(el => el.productPlatformId)
        let platformIds = data.map(el => el.platformId)
        return type ? { productPlatformIds, platformIds } : productPlatformIds
      }
    },
    ...
    bacthAssignPeople() {
      ...
      if (selected) {
        showDialog(
          ...
          selected.length === 1
            ? {
                productPlatformId: selected[0]
              }
            : {},
          {
            set(childVm) {
              let data = {
                productPlatformIds: vm.isSelect(),
                operatorList: []
              }
              childVm.options.forEach(el => {
                if (el.operatorId == null) {
                  return
                }
                data.operatorList.push({
                  operateType: el.operateType,
                  operatorId: el.operatorId
                })
              })
              fetchData({
                ...apis.PRODUCT_PLATFORM_OPERATION_BATCH_UPDATE,
                data
              }).then(res => {
                handlerCode(res, () => {
                  childVm.cancel()
      ...
      if (selected) {
        switch (name) {
          ...
          case 'invalid':
            content = '是否批量作废选中的产品？'
            api = {
              ...apis.PRODUCT_PLATFORM_BATCH_INVALID,
              data: {
                productPlatformIds: selected,
                platformId: vm.platform
              }
            }
            break
          ...
          case 'remove':
            content = '是否批量删除选中的产品？'
            api = {
              ...apis.PRODUCT_PLATFORM_BATCH_REMOVE,
              data: {
                productPlatformIds: selected,
                platformId: vm.platform
              }
    ...
    edit(props, type, params) {
      ...
      if (this.platform == 2 || this.platform == 5) {
        ...
        toParamUrl(
          this.platform == 2
            ? '/product/platformList/ebayEditLists.html'
            : '/product/platformList/wishEditLists.html',
          {
            productPltDetailId: props.item.languageDetailMap[1],
            productPlatformId: props.item.productPlatformId,
            productId: props.item.productId,
    ...
    lookAtReason(props, img) {
      showDialog(
        PRODUCT_PLT_LANGUAGE_AUDIT_INFO,
        { title: '审核不通过的原因', width: 1000, modalnoBtn: true },
        {
          props,
          img
        },
    ...
    invalid(props) {
      let vm = this
      let { platformId, productPlatformId } = props.item
      showConfirm({
        title: '是否作废此产品？',
        onOk() {
          fetchData({
            ...apis.PRODUCT_PLATFORM_INVALID,
            data: {
              platformId,
              productPlatformId
            }
    ...
    reload(props) {
      ...
      showConfirm({
        ...
        onOk() {
          fetchData({
            ...apis.PRODUCT_PLATFORM_INVALID_BACK,
            data: {
              platformId,
              productPlatformId
            }
    ...
    // 搜索相应产品列表
    searchColumns({ platformId = 1, categoryId }) {
      this.searchParams = {}

      this.searchParams.categoryId = categoryId
      this.searchParams.platformId = platformId
      this.params = {
        productPlatformStatus: this.params.productPlatformStatus,
        sortOrder: 'asc',
