<template>
  <div class="dialog-com">
    ...
    >
      ...
      >
        <div>
          <component :platformId="platformId" :selectedList="selectedList" :countries="countries" :type="type" :is="options.component" :reserveSelection="options.reserveSelection" :key="indexKey" ref="com"></component>
...
export default {
  ...
  props: ['platformId', 'selectedList', 'countries', 'type'],
  data() {
    return {
      options: {
        title: '',
            ...
            try {
              if (item.name == 'getList') {
                vm.$refs.com[item.name] &&
                  vm.$refs.com[item.name](null, item.params)
              } else {
                vm.$refs.com[item.name] && vm.$refs.com[item.name](item.params)
              }
            } catch (error) {
              console.log(error)
            }
