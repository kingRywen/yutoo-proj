<template lang="html">
    <section class="putaway">
            ...
            <hgroup class="putaway-select">
                ...
                <el-form :inline="true" :model="selectForm" :rules="selectFormRules" class="selectForm" label-position="right" ref="selectForm">
                    <el-form-item label="选择语种：" prop="languageId">
                        <el-select v-model.number="selectForm.languageId" size="medium" @change="changeLanguage" clearable>
                    ...
                    <el-form-item label="选择变体：" prop="variantIds" class="selectForm-item">
                        <el-select v-model="selectForm.variantIds"  size="medium" multiple collapse-tags @change="changeVariant" clearable>
                    ...
                    <el-form-item label="销售类型：" prop="type">
                        <el-select v-model.number="selectForm.type" size="medium" @change="changeSaleType" clearable>
                ...
                <!-- 跟卖 -->
                    <el-collapse-transition>
                        <el-card shadow="hover"  class="user-select-store" v-if="selectForm.type==1 && checkedStoreList.length">
                            <el-form label-position="right" ref="asinForm" :model="asinForm" >
                                <!-- {{asinForm}} -->
                                <div style="display: flex;">
                                    <el-form-item label="跟卖产品ASIN：" style="display: flex;"
                                        prop="asin"
                                        :rules="[{ required: true, message: '请输入跟卖的asin', trigger:['blur'] }]">
                                        <el-input size="medium" v-model="asinForm.asin" clearable>
                                            <el-button slot="append" size="medium" @click="setTailmsg(true,asinForm.asin)">设置单个变体值</el-button>
                                        </el-input>
                                    </el-form-item>
                                    <el-button type="primary" plain size="medium" style="height: 36px;margin: 3px 0 0 16px;" @click="appToSellStrategy">添加跟卖策略</el-button>
                                </div>
                            </el-form>

                            <template v-for="(item,index) in skuList">
                                <el-form label-position="right" ref="ruleForm" :model="skuForm[index]">
                                    <!-- {{skuForm[index]}} -->
                                    <div class="flex" style="margin-bottom:10px;">
                                        <p style="margin-right:12px;font-size: 14px;">{{item.storeName}}</p>
                                        <div class="border-pad" style="flex:1;">
                                            <el-form-item label="选择策略"
                                                :prop="skuForm[index]['sellRuleIdProp']"
                                                :rules="[{ required: true, message: '请选择策略', trigger:'blur'}]">
                                                <el-select size="medium" v-model="skuForm[index]['sellRuleId']" clearable>
                                                    <el-option
                                                        v-for="k in sellRulesList"
                                                        :key="k.sellRuleId"
                                                        :label="k.name"
                                                        :value="k.sellRuleId">
                                                    </el-option>
                                                </el-select>
                                                <el-button size="medium" @click="setTailmsg(false,skuForm[index]['sellRuleId'],index,item.storeId)">设置单个变体值</el-button>
                                            </el-form-item>
                                            <p style="border-bottom: 1px dashed #ccc;">店铺SKU</p>
                                            <div class="flex">
                                                <p style="padding: 12px 10px 0 0;">母体：</p>
                                                <el-form-item :label="item.parentSku" style="display:flex;"
                                                    :prop="item.parentSku"
                                                    :rules="[{ required: true, message: '请输入SKU', trigger:['blur'] }]">
                                                    <el-input size="medium" v-model="skuForm[index][item.parentSku]" clearable></el-input>
                                                </el-form-item>
                                            </div>
                                            <div class="flex">
                                                <el-form-item
                                                    ...
                                                    :label="j.skuCus" v-for="(j,jindex) in item.variantList" style="display:flex;margin-right:10px;">
                                                    <el-input clearable  size="medium"
                                                        :disabled="!j.uploadFlag"
                                                        v-model="skuForm[index].variantList[jindex][j.skuCus]" >
                                                        <i slot="suffix" v-if="!j.uploadFlag" style="margin-right:10px;">{{j.msg}}</i>
                                                    </el-input>
                                                </el-form-item>
                                            </div>
                                        </div>
                        ...
                        <el-form v-for="(item,index) in skuList" :model="skuForm[index]" label-position="right" class="store-sku-box flex mb15" >
                            ...
                            <div class="border-pad store-info">
                                <el-form-item :label="`母体：${item.parentSku}`" class="flex" ref="ruleForm"
                                    :prop="item.parentSku"
                                    :rules="rule">
                                    <el-input clearable size="medium" v-model="skuForm[index][item.parentSku]"></el-input>
                                </el-form-item>
                                ...
                                <div class="shopSku">
                                    <el-form-item :label="j.skuCus" class="flex" v-for="(j,jindex) in item.variantList"
                                        ...
                                        <el-input clearable size="medium" :disabled="!j.uploadFlag"
                                            v-model="skuForm[index].variantList[jindex][j.skuCus]">
                                            <i slot="suffix" v-if="!j.uploadFlag" style="margin-right:10px;">{{j.msg}}</i>
                                        </el-input>
                                    </el-form-item>
                            ...
                            <div class="store-asin" style="flex:1;flex-wrap: wrap;">
                                ...
                                <div class="store-asin-list border-pad" style="flex:1;">
                                    ...
                                    <el-form-item :label="j.skuCus" v-for="(j,jindex) in item.variantList" class="flex"
                                        :prop="'variantList.'+jindex+'.'+ j.skuCus"
                                        :rules="rule">
                                        <el-input clearable size="medium" v-model="skuForm[index].variantList[jindex][j.skuCus]"></el-input>
                                    </el-form-item>
                                </div>
                            </div>
                        </el-form>
                                ...
                                <el-form :inline="true" :model="item" ref="ruleForm" class="putawayform border-pad" label-position="right">
                                    <el-form-item
                                        ...
                                        <div class="flex" v-if="j.controlType==1">
                                            <el-input clearable v-model="j.propertyValue"></el-input>
                                            <el-button
                                        ...
                                        <div class="flex" v-if="j.controlType==2">
                                            <el-select clearable  v-model="j.propertyValue">
                                        ...
                                        <div class="flex" v-if="j.controlType==3">
                                            <el-select clearable multiple  collapse-tags v-model="j.propertyValue">
                                                <el-option v-for="k in j.itemProp"  :label="k.itemValue"  :value="k.salePfPropItemId"></el-option>
                                            </el-select>
                                            <el-button v-if="j.type==3 || j.type==5 || j.type==4"
                                                @click="setSingleMsg(j,j.propertyValue,index,jindex,item.siteId)">
                                        ...
                                        <div class="flex" v-if="j.controlType==4">
                                            <el-input clearable type="textarea" class="textarea" v-model="j.propertyValue"></el-input>
                                            <el-button v-if="j.type==3 || j.type==5 || j.type==4"
                                                @click="setSingleMsg(j,j.propertyValue,index,jindex,item.siteId)">
                                                    {{j.type | isShow}}
                                            </el-button>
                    ...
                    <section class="putaway-store-list layout-padding" v-if="isAllStoreDiffArr">
                        <el-card shadow="hover">
                            ...
                            <div class="store-item flex mb15" v-for="(item,index) in storeDifferenceList" :key="index" v-if="item.properties.length">
                                ...
                                <el-form :inline="true" :model="item" class="putawayform border-pad" label-position="right" label-width="200px" ref="ruleForm">
                                ...
                                <div class="">
                                    ...
                                    <el-form :inline="true" v-for="(item,index) in checkedStoreList" ref="ruleForm" :model="publishDetailForm[index]">
                                        ...
                                        <el-form-item
                                            ...
                                            <el-select clearable size="medium" v-model="publishDetailForm[index]['autiPublish']">
        ...
        <el-dialog :title="singleMsgTitle"  :visible.sync="singleMsgDialog" width="500px" center :modal-append-to-body="false">
            <el-form :model="singleMsgForm" ref="singleMsgForm" class="singform">
                ...
                <el-form-item  v-for="(item,index) in singleMsgForm.list"
                    ...
                    <!-- 输入款 -->
                    <el-input clearable v-model="item.value"  v-if="item.type == 1" size="medium"></el-input>
                    <!-- 单选款 -->
                    <el-select clearable v-model="item.value" v-if="item.type == 2" size="medium">
                        <el-option v-for="k in item.itemProp"  :label="k.itemValue"  :value="k.salePfPropItemId"></el-option>
                    </el-select>
                    <!-- 多选框 -->
                    <el-select clearable multiple  collapse-tags v-model="item.value" v-if="item.type == 3" size="medium">
                        <el-option v-for="k in item.itemProp"  :label="k.itemValue"  :value="k.salePfPropItemId"></el-option>
                    </el-select>
                    <!-- 文本域 -->
                    <el-input clearable type="textarea" class="textarea" v-model="item.value" v-if="item.type == 4" size="medium"/>
                </el-form-item>
            </el-form>
        ...
        <el-dialog
            title="设置单个变体值"
            :visible.sync="tailDialogVisible"
            width="500px"
            :modal-append-to-body="false">
            <el-form :model="asinForm" ref="asinForm" class="singform">
                <template v-if='asinForm.boole'>
    <el-form-item v-for="(item,index) in asinForm.list" :key="index" :label="item.sku" :prop="'list.' + index + '.skuAsinVal'" :rules="[{ required: true, message: '请输入跟卖的asin', trigger: 'blur' }]">
        <el-input clearable v-model="item.skuAsinVal" size="medium"></el-input>
    </el-form-item>
</template>
                <template v-else>
    <el-form-item v-for="(item,index) in asinForm.sellRuleList" :key="index" :label="item.sku" :prop="'sellRuleList.' + index + '.sellRuleId'" :rules="[{ required: true, message: '请输入跟卖的asin', trigger: 'blur' }]">
        <el-select clearable size="medium" v-model="item.sellRuleId">
            <el-option v-for="k in sellRulesList" :key="k.sellRuleId" :label="k.name" :value="k.sellRuleId"></el-option>
        </el-select>
    </el-form-item>
</template>
            ...
            <span slot="footer" class="dialog-footer">
                <el-button @click="tailDialogVisible = false" size="medium">取 消</el-button>
                <el-button type="primary" @click="saveSellRule" size="medium">确 定</el-button>
            </span>
        </el-dialog>
    </section>
</template>
...
export default {
    ...
    watch: {
        skuList: {
            handler(val) {
                this.skuForm = val.map((item, index) => {
                    //跟卖
                    if (this.selectForm.type == 1) {
                        return {
                            storeId: item.storeId,
                            sellRuleId: '',
                            sellRuleIdProp: 'sellRuleId',
    ...
    methods: {
        appToSellStrategy() {
            toUrl('/product/ToSellStrategy.html');
        },
        //跟卖弹框确定
        saveSellRule() {
            //asin
            if (this.asinForm.boole) {
                this.asinForm.list.forEach(item => {
                    if (item.skuAsinVal != this.asinForm.asin) {
                        item.flag = true;
                    } else {
                        item.flag = false;
                    }
                });
                this.tailDialogVisible = false;
                //策略
            } else {
                let _this = this;
                this.asinForm.sellRuleList.forEach((item, index) => {
                    if (
                        item.sellRuleId !=
                        _this.skuForm[item.storeIndex]['sellRuleId']
                    ) {
                        item.flag = true;
        ...
        setTailmsg(boole, value, storeIndex, storeId) {
            this.tailDialogVisible = true;

            this.asinForm.boole = boole;
            //点击的设置单个变体ansin
            if (boole) {
                this.asinForm.list = this.selectForm.variantIds.map(
                    (variantId, index) => {
                        //选择的变体Id
                        let sku = null;
                        this.variantList.forEach(item => {
                            if (variantId == item.platformVariantId)
                                sku = item.sku;
                        });
                        ...
                        let flag = false;
                        if (
                            this.asinForm.list.length &&
                            this.asinForm.list[index].flag
                        ) {
                            changeValue = this.asinForm.list[index].skuAsinVal;
                            flag = true;
                        }
                        return {
                            sku,
                            skuAsinVal: changeValue ? changeValue : value,
                            flag
                        };
                    }
                );
                ...
                this.asinForm.sellRuleList = this.selectForm.variantIds.map(
                    (variantId, index) => {
                        //选择的变体Id
                        let sku = null;
                        this.variantList.forEach(item => {
                        ...
                        let flag = false;
                        if (
                            this.asinForm.sellRuleList.length &&
                            this.asinForm.sellRuleList[index].flag
                        ) {
                            changeValue = this.asinForm.sellRuleList[index]
                                .sellRuleId;
                            flag = true;
                        }
                        return {
                            sku,
                            sellRuleId: changeValue ? changeValue : value,
                            flag,
                            storeId,
                            storeIndex
                        };
        ...
        submitForm(saveType, formName) {
            ...
            this.$refs.selectForm.validate(valid => {
                if (valid) {
                    if (
                        this.$refs.ruleForm != undefined &&
                        this.$refs.ruleForm.length
                    ) {
                        let isAllInput = [];
                        ...
                        if (this.selectForm.type == 1) {
                            this.$refs.asinForm.validate(valid => {
                                isAllInput.push(valid);
                            });
                        }
                                ...
                                this.skuList.forEach(j => {
                                    ...
                                    if (i.storeId == j.storeId) {
                                        ...
                                        if (this.selectForm.type == 2) {
                                            ...
                                            obj['parentSkuStore'] =
                                                j.parentSkuStore;
                                            obj['baseParentProductId'] =
                                                j.baseParentProductId;
                                            obj['detailParentProductId'] =
                                                j.detailParentProductId;
                                            i.variantList.map(k => {
                                        ...
                                        if (this.selectForm.type == 1) {
                                            ...
                                            obj[
                                                ...
                                                let asin;
                                                if (
                                                    _this.asinForm.list.length
                                                ) {
                                                    _this.asinForm.list.forEach(
                                                        aitem => {
                                                            if (
                                                                k.sole ==
                                                                aitem.sku
                                                    ...
                                                    asin = _this.asinForm.asin;
                                                }
                                                let strategyId;
                                                if (
                                                    _this.asinForm.sellRuleList
                                                        .length
                                                ) {
                                                    _this.asinForm.sellRuleList.forEach(
                                                        sitem => {
                                                            if (
                                                                sitem.sku ==
                                                                k.sole
                                                    ...
                                                    strategyId = i.sellRuleId;
                                                }
                                                variantObj['asin'] = asin;
                                                variantObj[
                                                    'strategyId'
                                                ] = strategyId;
                                                j.variantList.forEach(h => {
                            ...
                            this.storeDifferenceList.forEach(i => {
                                storeInfo = i.properties.map(j => {
                                    ...
                                    if (j.hasOwnProperty('list')) {
                                        ...
                                        !j.hasOwnProperty('list') &&
                                        isDifference
                                    ) {
                                        let singArr = this.selectForm.variantIds.map(
                                            variantId => {
                                                let variantInfo = {};
                                                this.variantList.forEach(
                                                    variantItem => {
                                                        if (
                                                            variantItem.platformVariantId ==
                                                            variantId
                                                        ) {
                                                            variantInfo['id'] =
                                                                variantItem.baseVariantId;
                                                            variantInfo[
                                                                'value'
                                                            ] = j.propertyValue;
                                                        }
                    ...
                    if (this.singleMsgTitle == '设置单个变体值') {
                        ...
                        let list = this.singleMsgForm.list.map(item => {
                            return {
                                ...
                                value: item.value,
                                active:
                                    this.startMsg == item.value ? false : true
                            };
                        });
                        console.log(list);
                        ...
                        this.$set(
                            this.storeDifferenceList[index].properties[jindex],
                            'list',
                            list
                        );
        ...
        setSingleMsg(inputItem, value, index, jindex, storeId) {
            ...
            //单个变体值 =>则弹框内显示选中的变体
            if (this.singleMsgTitle == '设置单个变体值') {
                this.singleMsgForm.list = this.selectForm.variantIds.map(
                    (variantId, kindex) => {
                        ...
                        if (
                            this.storeDifferenceList[index].properties[
                                jindex
                            ].hasOwnProperty('list')
                        ) {
                            newVal = this.storeDifferenceList[index].properties[
                                jindex
                            ].list[kindex].value;
                            active = this.storeDifferenceList[index].properties[
                                jindex
                            ].list[kindex].active;
                        }
                        return {
        ...
        onUserStore(arr) {
            ...
            if (arr.length) {
                ...
                this.getSuccessData(
                    ...
                    data => {
                        this.skuList = data.sku;
                        if (data.hasOwnProperty('sellRules')) {
                            this.sellRulesList = data.sellRules;
                        }
                    }
                );
                ...
                this.getSuccessData('putaway/diff', params, data => {
                    ...
                    //判断全都是否有差异控制页面显示
                    this.isAllSiteDiffArr = siteDiffList.every(
                        item => item.properties.length
                    );
                    this.isAllStoreDiffArr = storeDiffList.every(
                        item => item.properties.length
                    );
                    //
                ...
                if (res.data.code == 0) {
                    ...
                    this.variantList.forEach(item => {
                        if (item.uploadFlag == true) {
                            this.selectForm.variantIds.push(
        ...
        getSuccessData(url, params, callback) {
            getData(url, params).then(res => {
                this.loading = false;
                if (res.data.code == '0') {
                    let data = res.data.hasOwnProperty('rows')
                        ? res.data.rows
                        : res.data.msg;
        ...
        onSyncStore(val) {
            val.forEach(item => {
                if (
                    this.publishDetailForm.find(j => j.storeId == item.storeId)
                ) {
                    let index = this.publishDetailForm.findIndex(
                        k => k.storeId == item.storeId
                    );
                    this.publishDetailForm[
                        index
        ...
        syncStore(item, index, autiPublish, publishTime) {
            this.syncStoreList = this.storeList.filter(
