<template>
    <div class="yt-page">
        <universal-layout :btnList="btnConfig" :pageNo="pageNo" :pageSize="pageSize" :total="total" @handleSizeChange="handleSizeChange" @handleCurrentChange="handleCurrentChange" @clickBtn="clickBtn">
            <!--  -->
            <slot>
                <template slot="search">
                    <slot name="search">
                        <YtSearch :searchList="searchConfig" ref="search" @search="search"></YtSearch>
                    </slot>
                </template>
                <!--  -->
                <template slot="btnRight">
                    <user-defined v-if="leftCustom" :data="tableConfig" :defaultData="defaultData" :selectCheck="selectCheck">自定义列表展示</user-defined>
                </template>
                <!--  -->
                <template slot="table">
                    <yt-table :data="evalFunc ? tableTreeList : tableList" v-loading="tableLoading" ref="table" :columns="defaultData" :border="true" :reserveSelection="reserveSelection" @select-change="$emit('select-change',$event)">
                        <template name="left">
        ...
        <el-dialog v-else :title="dialog.title" :visible.sync="dialog.visible" :width="dialog.width" @open="open" @close="closeDialog" :append-to-body="true">
...
export default {
    ...
    computed: {
        dialogOpts() {
            ...
            let { confirmButtonText, cancelButtonText, dialogOpts, ...info } = this.dialog;
            return {
                ..._default,
                ...info,
                ...dialogOpts
    ...
    methods: {
                ...
                this.$nextTick(() => {
                    try {
                        ...
                        if (this.dialog.fnName) {
                            this.$refs.component[this.dialog.fnName](
                                this.dialog.params
                            );
                        }
                    } catch (err) {
        ...
        getList(type) {
            ...
            if (type) {
                let search =
                    this.searchData(
                        JSON.parse(JSON.stringify(this.searchInfo))
                    ) || {};
                this.$refs.table.clearSelection();
