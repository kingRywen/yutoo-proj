    ...
    <div class="box">
        ...
        <el-dialog
          ...
          <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="130px">
            ...
            <el-form-item label="套餐价格:" prop="charge">
                <el-input v-model.number="ruleForm.charge" autocomplete="off" :disabled="isDialogView" size="mini"></el-input>
            </el-form-item>
            <el-form-item label="套餐限制:" class="form-item-duration" prop="duration">
            ...
            <el-form-item label="功能列表:">
                <el-table
                  ...
                  <el-table-column
                    label="次数"
                    :render-header="handlerRenderHeaderFrequency">
                    <template slot-scope="scope">
                      <el-input autocomplete="off" size="mini" :ref="'input_functionId_'+scope.row.functionId" :disabled="isDialogView"></el-input>
                    </template>
                  </el-table-column>
                </el-table>
            </el-form-item>
...
export default {
  ...
  data() {
      ...
      if(t==='edit'||t==='view'){
        ...
        httpPost(reqData).then(res => {
          ...
          data.functions.forEach(element => {
           this.$refs['input_functionId_'+element.functionId].$data.currentValue=element.useNumber
          });
          if(t==='view'){
            let renderInputFrequency =document.getElementById('renderInputFrequency')
            renderInputFrequency.disabled =true
            renderInputFrequency.style.backgroundColor='#f5f7fa'
            renderInputFrequency.style.cursor='not-allowed'
    ...
    getAllFunctionList(){
        let reqData={
              url:'/function/allList',
              params:{}
            }
        
        httpPost(reqData).then(res => {
          let {data} =res
          this.functionAllList =data
          this.setFrequency(null) // 新增时清空 功能列表
        }).catch(err=>{
          console.log(err)
        })
    },
    submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
           ...
           let flag =false
            this.functionAllList.forEach(element => {
              let useNumber=this.$refs['input_functionId_'+element.functionId].$data.currentValue
              // parseInt('2ddd) 2
              // parseInt('ddd') NaN
              if(isNaN(parseInt(useNumber))){
                flag =true
              }else{
                params.functions.push({functionId:element.functionId,useNumber:parseInt(useNumber)})
              }
            });
            if(flag){
              this.$message.warning('功能列表的所有次数请输入数字！');
              return
            }
    ...
    handlerRenderHeaderFrequency(h, { column, $index }){
      const inputData ={
      ...
      return(
         <div class="span-input-frequency">
          <div class="input-label-frequency">{column.label}</div>
          <div class="el-input el-input--mini input-frequency">
            <input type="text" id="renderInputFrequency" {...inputData}/>
          </div>
         </div>
         )
    },
    setFrequency(e){
      if(e===null){
        // 用ref 设置没用 onBlur={this.setFrequency}
        document.getElementById('renderInputFrequency').value =null
        this.functionAllList.forEach(element => {
            let inputFunctionId = this.$refs['input_functionId_'+element.functionId]
            if(inputFunctionId===undefined){
              throw new Error('undefined is stop forEach')
            }else{
              inputFunctionId.$data.currentValue=null
            }
          });
      }else{
        if(e.srcElement.value.trim()!==''){
          this.functionAllList.forEach(element => {
            this.$refs['input_functionId_'+element.functionId].$data.currentValue=e.srcElement.value
          });
        }
      }
...
.box{
    ...
    /deep/.span-input-frequency{
      display: flex;
      padding: 0;
    }
    /deep/.input-label-frequency{
      padding: 0;
      width: 40px;
    }
    /deep/.input-frequency{
      padding: 0;
    }
    // .input-frequency{
