...
window.vm = new Vue({
	...
	created () {
		console.log(getQueryString('menuid'));
		post('/sys/menu/info', getQueryString('menuid'))
			.then(res => {
	...
	methods: {
		...
		postData() {
			post('/sys/menu/update?_' + Date.now(), { ...this.ruleForm })
				.then(res => {
					handleRes(res.data, () => {
						let vmMain = getVueInstance('main')
						vmMain.getMenuList(null, res => {
							vmMain.tableData = res.data.rows
						})
					})
				})
		}
	...
	computed: {
		treeSelect() {
			return ['一级菜单', '组织机构',this.ruleForm.parentName]
		}
	}
})