    ...
    <section class="section">
      ...
      <el-form :model="ruleForm" class="__register" :rules="rules" ref="ruleForm">
        ...
        <el-form-item prop="password">
          ...
          <el-input placeholder="请输入新密码" v-model.trim="ruleForm.password">
            <template slot="prepend">
              <span class="required">*</span>
              新密码
            </template>
        ...
        <el-form-item prop="code">
          <el-row>
            ...
            <el-col :span="5">
              <span v-show="toggleCodeStatus" class="commonPhoneCodeStyle phoneCode" @click="getPhoneCode(1)">获取验证码</span>
        ...
        <!-- <el-form-item> -->
        <el-button type="primary" :loading="loading" @click="submitForm('ruleForm','/sys/forgetPwd',1)" style="width:100%;margin-top:5px;">提交</el-button>
        <div class="line"></div>
        <!-- <div class="toLoginCon">
          <span>已有账号？</span>
          <span @click="toLogin" class="tologin">请登陆</span>
        </div> -->
        <!-- </el-form-item> -->
      </el-form>
      <div class="regAndFind">
        <span class="s" style="margin-left:55px;" @click="goLogin">登录</span>
        <span class="s" style="margin-left:20px;" @click="goReg">注册</span>
      </div>
    </section>
  </div>
</template>
...
export default {
  data() {
    ...
    //       callback(new Error("密码由6-21位字母和数字组成"));
    //     }
    //     callback();
    //   }
    // };
  ...
  methods: {
    ...
    //       if (res.data.code == 0) {
    //         if (messageType === 1) {
    //           this.$message({ type: "success", message: "找回成功" });
    //         } else {
