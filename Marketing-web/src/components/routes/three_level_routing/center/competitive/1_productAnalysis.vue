...
<template>
  <el-row :gutter="16" class="h100 h100_box">
    ...
    <el-col :sm="!show ? 24 : 16" :xs="!show ? 24 : 16" :xl="!show ? 24 : 20" :lg="!show ? 24 : 20" :md="!show ? 24 : 18" :class="['h100', 'right', {'no-border': !show}]">
      <div class='keywordIndexContainer productAnalysisContainer __wrapper_i'>
        <section class="parentPosition">
          <!-- 添加产品弹窗 -->
          <el-dialog :close-on-click-modal="false" title="添加产品" :visible.sync="dialogShow.dialogFormVisible" width="600px">
            <!-- <section style="position:relative"> -->
            <!-- <div class="timeCss">
            <span> {{this.point.charge}}点/每次</span>
          </div> -->
            <div slot="title">
              <span class="el-dialog__title">添加产品</span>
              <span class="timeCss"> {{point.charge}}点/每次</span>
            </div>
            ...
            <div class='box __box_wrapper'>
              ...
              <el-form :model="dialogData" :rules="rules" ref="ruleForm" style="width:100%;">
                ...
                <el-form-item label="产品ASIN:" prop="textarea" :label-width="formLabelWidth" style="width:100%;">
                ...
                <el-form-item :label-width="formLabelWidth" style="width:100%;">
                  <el-button type="primary" style="width:100%;" @click="showPoint('ruleForm')" :loading="buttonLoading.add">确 认</el-button>
                </el-form-item>
              </el-form>
            </div>
          </el-dialog>
          ...
          <el-dialog :close-on-click-modal="false" title="添加监控" :visible.sync="dialogShow.dialogMonitorVisible" width="600px">
            <div class='dialogContent' style="overflow:hidden;">
              ...
              <div class='saveContainer' style='text-align:center;margin-top:40px;'>
                <el-button type="primary" size="large" style='width:50%' @click="addChartMoni">确 认</el-button>
              </div>
            </div>
          </el-dialog>
          ...
          <el-row style="margin-top:8px;margin-bottom: 8px;height:30px" type="flex" justify="space-between">
            ...
            <el-col :lg="{span:10}" :md="{span:9}" :sm="{span:6}" class="__J_flex">
              <el-input placeholder="输入ASIN" v-model.trim="inputText" clearable style='max-width:150px;'>
              </el-input>
              <!-- 搜索按钮 -->
          ...
          <!-- 表格数据 -->
          <!-- {{innerHeight}} -->
          <el-table :max-height="12000" v-loading="tableLoading" ref="Table" :data="listData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="sku" label="ASIN" align='center' width="125px">
            </el-table-column>
              ...
              <template slot-scope="scope">
                ...
                <el-tooltip v-else placement="top" :content="scope.row.category">
                  <div class="fix__row_2">
                    <!-- <span>{{ scope.row.category!=""?scope.row.category:""}}
                </span> -->
                    <span>{{ scope.row.category| _formatData_cate}}
                    </span>
                  </div>
                </el-tooltip>
              </template>
            ...
            <el-table-column label="监控状态" align='center' width="80px">
              <template slot-scope="scope">
                <!-- <span v-if="scope.row.status ===0||scope.row.status===1||scope.row.status ===2"> -->
            ...
            <el-table-column prop="handle" label="操作" align='center' width="100px">
              <template slot-scope="scope">
                <el-button @click="handleClick(scope.$index,scope.row)" type="primary" v-if="scope.row.buttonText&&scope.row.parentSku">查看</el-button>
              </template>
            </el-table-column>
          </el-table>
          <!-- 分页 -->
        ...
        <CommonDialog :modal-obj="addGroupObj">
          <el-form :model="addGroupModel">
            ...
            <el-form-item label="选择上级分组" label-width="100px">
              <el-cascader change-on-select :options="groupList" style="width:100%" v-model="addGroupModel.groupId" :props="addProjectFormOpts"></el-cascader>
            </el-form-item>
        ...
        <CommonDialog :modal-obj="moveGroupObj">
          <el-form :model="moveGroupModel">
            <el-form-item label="选择分组" label-width="100px">
              <el-row :gutter="12">
                <el-col :span="18">
                  <el-cascader change-on-select :options="groupList" style="width:100%" v-model="moveGroupModel.groupId" :props="addProjectFormOpts"></el-cascader>
                </el-col>
                <el-col :span="5">
...
import {
  ...
  FunctionMktProductAnalysisList
} from "@/api/myApi";
...
export default {
  ...
  methods: {
    ...
    updateTreeData() {
      getGroupingList({ groupType: this.groupType }).then(data => {
        this.groupList = [data];
      });
    },
    ...
    createNode(data, type, cb) {
      ...
      const { parentGroupId, groupName, groupId } = data;
    ...
    // 添加分组
    addGroup() {
      const parentGroupId = this.addGroupModel.groupId.slice().pop();
      const { groupName } = this.addGroupModel;
      this.createNode({ parentGroupId, groupName });
    },
    ...
    moveOut(groupId = -1) {
      ...
      //   .then(() => {
      this.$axios
        .post("/ProductAnalysis/updateGroup", {
          taskIds,
          groupId: groupId
        })
        .then(res => {
    ...
    // 请求点数
    getChargeApi() {
      this.$axios.post(FunctionMktProductAnalysisList).then(res => {
        // console.log(res, "竞品分析扣点");
        // this.point.surplusAmount = res.data.data.surplusAmount;
        this.point.charge = res.data.data.charge;
        this.$store.commit("competitive/product_charge", this.point.charge);
      });
    },
    ...
    deleteProduct() {
      ...
      // console.log(taskIdArr, "即将要删除的项");
      // console.log(this.listData, "列表所有的项");
      // 一个个检查是否存在监控状态
      let Items = [];
    ...
    searchTabPosition(day, _this, pageNumber, pageSize, sku) {
      ...
      this.$store.commit("competitive/tableLoading", true);
      let params = {
        platformId: 1,
        sku: sku,
        groupId:
    ...
    // tableType为列表数据的分页切换和时间段切换todonow
    changeTabPosition(day, _this, pageNumber, pageSize) {
      ...
      let params = {
        ...
        // sku: "B00AHRDKOO",
        // startTime: "2018-6-12",
        // startTime: startTime || "", //functions
        // endTime: "2018-6-15",
        // endTime: endTime || "", // functions
        pageNumber: pageNumber || this.page.currentPage,
        pageSize: pageSize || this.page.Size
      };
      if (this.DateMsg.DateValue !== null) {
