  ...
  <div class="chart4" v-loading="Loading">
    ...
    <main style="padding:10px 20px;border:1px solid #e4e4e4;box-size:border-box;">
      <div>
        ...
        <echart auto-resize :options="echartData" v-show="this.chartShow" id="cate_chart4" style="width:100%; height:360px;margin-top:39px;"></echart>
      </div>
    </main>
  </div>
</template>
...
export default {
  ...
  methods: {
    downloadExl() {
      ...
      // this.$.downloadExl(data, `总数量(类目:${category})`);
    },
    //获取子组件传来的日期
    getChildDate(msg) {
    ...
    drawChart1(list) {
      try {
        ...
        let myChart = echarts.init(document.getElementById("cate_chart4"));
        // myChart.clear();
        let dataAxis = [],
          data = [];
