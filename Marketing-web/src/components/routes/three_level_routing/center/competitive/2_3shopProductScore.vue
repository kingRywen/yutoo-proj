...
<template>
  <div v-if="this.taskStatusAndType">
    ...
    <!-- <el-button @click="toShop" style="padding:6px 15px;position:relative;top:-5px;color:#00c0de">&nbsp;返回店铺详情</el-button> -->
...
export default {
  ...
  methods: {
    toShop() {
      this.$router.push(sessionStorage.getItem("shopFullName"));
    },
    ...
    getShopProductHistoryList(pageNumber, pageSize, sku) {
      ...
      let params = {
        ...
        statTime: this.$store.state.competitive.ChartDate || getDay().today,
        newArrivalDay: this.newArrivalDay,
        taskType: 1,
        taskId: parseInt(this.clickItem.taskId)
        // monitorTo: this.taskStatusAndType.taskStatus.monitorTo.slice(0, 10) //已改
      };
      params = this.$.dealObjectValue(params);
      this.$axios
        .post(ShopProductHistoryList, params)
        .then(res => {
          if (res.status == 200) {
    ...
    // 点击查看产品详情 （chart789和tab3一样的函数）只是monitorTo在这里是从vuex里面取得
    checkProduct(item) {
      ...
      this.$router.push(
        ...
        "/admin/competitive/productAnalysis/checkProductAnalysis?sku=" +
          item.row.sku +
          "&taskId=" +
          parseInt(this.clickItem.taskId) +
          "&taskType=1"
      );
