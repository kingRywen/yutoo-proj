  ...
  <div class="shopStatuContainer">
    ...
    <div class='tabRight'>
      ...
      <div style="margin-right:-5px;font-size:13px;position:relative;top:-12px;height:40px;line-height:40px;" v-show="isShowMoniContrl">
        <span>新品：上架时间</span>
        <span style="color:#00c0de">{{value}}</span>
        <span>天</span>
        <span class="changeDate" v-show="ShowSwitch" @click="ShowSwitch=false">切换</span>
        <span v-show="!ShowSwitch">
          <el-select :popper-append-to-body="false"v-model="value" style="width:70px;margin-left:10px;" @change="selectDay">
      ...
      <!-- 时间，开关 -->
      <ul style="display:flex;position:relative;right:-10px;" class="font12">
        <li>
          更新时间：
          <span class='timeColor'>{{this.childStatus.updateTime||"未开启监控"}}</span>
        </li>
...
<script>
import { ShopMonitoringUrl, ShopAnalysisRemoveMonitor } from "@/api/myApi";
export default {
  name: "moniCotrl",
  components: {},
  props: {
  ...
  methods: {
    ...
    confirmDay() {
      ...
      // 切换新品时间存store
      this.$store.commit("competitive/saveDay", parseInt(this.value));
    },
    // 开启还是取消监控
