<template>
  <div class="upload_wrapper" ref="content" @drop.stop.prevent="handleDrop" @dragover.stop.prevent="handleDragover" @dragleave.stop.prevent="handleDragleave">
    <!-- {{files}} -->
    ...
    <div class="upload">
      <div class="table-responsive">
        ...
        <ul class="upload-allfiles">
          <el-checkbox-group v-model="checkList">
            <li v-for="(file, index) in files" :key="file.id" :style="{background: !file.imgFolderId ? '#ececec': '#ecebe2', cursor: !file.imgFolderId ? '': 'pointer'}">
              <div v-if="!file.imgFolderId">
                <div class="divContent">
                  <img :src="file.imgUrl ||file.thumb || '/static/no-image.jpg'" width="150" height="auto" />
                </div>

                <div class="bot">
                  <div class="first">
                    <span class="title">{{file.imgName || file.name}}</span>

                    <el-checkbox v-if="file.imgId" :label="file.imgId">{{ }}</el-checkbox>
                    <el-radio v-if="false" v-model="radio" :label="index">主图</el-radio>
                  </div>
                  <div class="second">
                    {{file.size | fixNumber}}KB
                    <i class="el-icon-success" v-if="file.has" style="color:#67c23a"></i>
                  </div>
                  <div>
                    <!-- remark定义备注名字 -->
                    <el-input placeholder="请输入图片备注" v-model.trim="file.remark" @input="val => file.singleRemark = val" autosize></el-input>
                  </div>
                </div>
              ...
              <!-- 文件夹 -->
              <div v-else @click="$emit('open-folder', file)">
                <div class="divContent">
                  <img src="~@/assets/folder.png" width="150" height="auto" />
                </div>

                <div class="bot" style="padding:0 10px">
                  <div class="first">
                    <span class="title">{{file.name}}</span>
                    <el-checkbox @click.native.stop="$emit('none')" v-if="file.imgFolderId" :label="file.imgFolderId">{{ }}</el-checkbox>
                  ...
                  <div>
                    <!-- remark定义备注名字 -->
                    <el-input style="opacity:0;z-index:-1" placeholder="请输入图片备注" v-model.trim="file.remark" @input="val => file.singleRemark = val" autosize></el-input>
                  </div>
                </div>
                <el-button type="danger" icon="el-icon-delete" class="dropdown-item del-file-btn" circle @click.prevent.stop="removeFile(file, index)"></el-button>
              </div>
            </li>
          </el-checkbox-group>
        </ul>

        <table style="display:block" class="table table-hover">
      ...
      <div class="example-foorer">
        <div class="btn-group" v-if="!$attrs.show">
          <el-row :gutter="20">
            <el-col :span="16">
              <file-upload v-if="showBtn" class="el-button el-button--primary" style="padding: 0px 20px;line-height: 27px;height:32px" :post-action="postAction" :extensions="extensions" :accept="accept" :multiple="true" :directory="directory" :size="size || 0" :thread="thread < 1 ? 1 : (thread > 5 ? 5 : thread)" :headers="headers" :drop="true" :drop-directory="true" :add-index="addIndex" :value="files" @input="getFile" @input-filter="inputFilter" @input-file="inputFile" ref="upload">
                <i class="el-icon-plus" style="vertical-align: middle;"></i>
                <span style="vertical-align: middle;">添加图片</span>
              </file-upload>

              <el-button v-if="!showUpload && showBtn" type="primary" style="vertical-align: middle;" icon="upload" :loading="loading" @click.prevent="startUpload">上传文件</el-button>
              <el-button v-if="!showUpload && hasImg.length > 0" type="primary" style="vertical-align: middle;" icon="upload" :loading="loading" @click.prevent="selectAll">{{isSelectAll ? '取消全选': '全选'}}</el-button>
              <el-button v-if="!showUpload && checkList.length" type="primary" style="vertical-align: middle;" icon="upload" :loading="loading" @click.prevent="reverseSelect">反选</el-button>
              <el-button v-if="!showUpload && hasImg.length > 0" type="primary" style="vertical-align: middle;" icon="upload" :loading="loading || delLoading" @click.prevent="delItem">
                {{delLoading ? '删除中...' : '删除'}}
              </el-button>
            ...
            <el-col :span="8" style="margin-top:8px;" v-if="showProgress">
...
export default {
  ...
  watch: {
    checkList(e, olde) {
      this.checkList1.splice(0);
      e.forEach(i => {
        let element = this.files.find(
          el => el.imgFolderId === i || el.imgId === i
        );
        this.checkList1.push(element);
      });
    },
    ...
    "files.length"() {
      this.$emit("update:propgressLineStatus", "");
      this.$emit("update:propgressLine", 0);
    },
  ...
  methods: {
    ...
    //   e.forEach(el => {
    //     let element = this.files.find(
    //       el => el.imgFolderId === el || el.imgId === el
    //     );
    //     this.checkList1.push(element);
    //   });
    // },
    ...
    initStoreList(val) {
      ...
      res.map((el, index) => {
        ...
        el.remark = el.imgRemarks;
        el.thumb = el.url;
        el.progress = 100;
        el.has = true;

        el.size = el.imgSize ? parseInt(el.imgSize) : "-";
      });
    ...
    delItem() {
      ...
      this.$confirm("此操作将删除选择的文件, 是否继续?", "警告", {
        ...
        .then(() => {
          ...
          this.$emit("delItem", this.checkList1, () => {
    ...
    reverseSelect() {
      this.files.forEach(el => {
        let id = el.imgId || el.imgFolderId;
    ...
    selectAll() {
      ...
      let vm = this;
      let list = vm.checkList;
      this.files.forEach(el => {
        let id = el.imgId || el.imgFolderId;
        if (
          el.has &&
          (el.imgId === id || el.imgFolderId === id) &&
          list.indexOf(id) == -1
        ) {
          list.push(id);
        }
  ...
  computed: {
    isSelectAll() {
      let ret = true;
      this.files.filter(el => el.has).forEach(el => {
        let id = el.imgId || el.imgFolderId;
        if (!this.checkList.find(el => el === id)) {
          ret = false;
          return false;
        }
      });
    ...
    showProgress() {
      return this.files.some(el => !el.has);
    },
    ...mapState("imgUpload", {
