<template>
  ...
  <el-switch
    ...
    :active-value="item.activeValue == null ? 1 : item.activeValue"
    :inactive-value="item.inactiveValue == null ? 0 : item.inactiveValue"
    @change="$emit('input', $event)"
    :value="currentVal"
  ></el-switch>
  <!-- 选择 -->
  ...
  <div v-else-if="type === 'upload'">
    ...
    >
      ...
      <div class="el-upload__tip" slot="tip"><span v-if="uploadOpts.text && uploadOpts.text.length">只能上传{{uploadOpts.text.join('/')}}文件，</span>{{item.limitSize ? `文件大小不超过${item.limitSize}M` : ''}}</div>
    </el-upload>
