  ...
  >
    <el-row>
      <template v-for="(item, index) in fields">
        ...
        <el-col v-else-if="!item.hidden || item.hidden(formData)" :span="isNormal ? item.span || 24 :(item.span ||6)">
          <el-form-item
            v-if="!item.title"
            :label="item.label"
            ...
            :required="item.requiredFun ? item.requiredFun(formData) : !!item.required"
            :rules="rules[item.name] ? null : [{required: true, message:`${item.label}不能为空`}]"
