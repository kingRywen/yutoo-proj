<template>
  <div>
    ...
    <yt-dialog :options="dialogOptions" :events="dialogEvents">
      <el-form ref="addForm" class="custom_from" label-width="50px" size="mini" :model="addForm">
        <el-form-item label="类目" prop="categoryId" :rules="validator(true, '类目')">
          <SyncCascader size="mini" v-if="refresh" v-model="addForm.categoryId" :load="loadTree" :props="treeProps"/>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    loadData(data) {
      return this.$api[`main/libMessageContentList`](data)
        .then(data => {
