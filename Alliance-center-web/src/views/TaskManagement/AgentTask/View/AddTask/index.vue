<template>
  <div v-if="showEdit" v-loading="loading">
    <el-page-header
      :class="[{'no-back': view}, 'mb20']"
      :content="view ? '查看任务 （任务ID：'+ $route.params.id + '）' :quick ? '快速发布任务' : edit ? '编辑任务 （任务ID：'+ $route.params.id + '）'  : '发布任务'"
    ></el-page-header>
...
export default {
    ...
    return {
      ...
      base: {
        ...
        terminalType: {
          ...
          clearable: true,
          span: 12,
          options: this.$const['OTHER/termnalType'].slice(0, 2)
        },
        sellerName: {
          label: '店铺名称',
          span: 12,
  ...
  methods: {
    isWhat(plats = []) {
      if (!this.value1 || !this.value1.platformCode) {
        return
      }
      ...
      return plats.indexOf(this.value1.platformCode.toLowerCase()) > -1
    },
    setNums(num) {
    ...
    reviewImpSuc(data) {
      ...
      if (data.code == 0) {
        ...
        this.reviewData.data.forEach((el, index) => {
          if (data.rows[index]) {
            el.content = data.rows[index].content
            el.imgUrlOne = data.rows[index].imgUrlOne
            el.imgUrlTwo = data.rows[index].imgUrlTwo
            el.imgUrlThree = data.rows[index].imgUrlThree
            el.title = data.rows[index].title
          }
        })
    ...
    getMoney(data, platformCode) {
      ...
      let isAli = platformCode.toLowerCase() == 'aliexpress'
    ...
    submit() {
      ...
      Promise.all(promises)
        .then(() => {
          ...
          let params = {
            taskSourceId: this.$route.params.id,
            taskReviewArray:
    ...
    downloadTemp() {
      this.$api[`main/taskSourceExportTemplate`]({
        // platformCode: this.$store.state.currentSiteInfo.selectedSite[0].toLowerCase()
      }).then(data => {
      ...
      this.$api[`main/taskSourceInfo`]({
        ...
        .then(data => {
          ...
          let {
            ...
            reviewGradeFourNum,
            reviewGradeFiveNum,
            taskMoney: brokerages,
            ...info
          } = data.rows
          this.importRvSkm.value.data.platformCode = data.rows.platformCode
          this.getMoney(brokerages, data.rows.platformCode)
