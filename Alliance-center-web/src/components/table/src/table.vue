        ...
        <template v-if="item.merge">
          ...
          >
            <template slot-scope="scope">
              <table-colvalue-1 :item="k" :scope="scope" :style="k.style ||''" />
            </template>
          </el-table-column>
        </template>
        <template slot-scope="scope">
            ...
            <el-tooltip effect="light" placement="right">
              ...
              >
                <div>
                  <div
                    class="__img_wrapper mar-auto"
                    :style="{width:(item.style && item.style.width || '45') + 'px', height: (item.style && item.style.width || '45') + 'px'}"
                  >
                    <img
                      class="loadingImg"
                      :key="scope.row[item.value]"
          ...
          <!-- 普通 -->
          <div
            @click="handleColClick(item.noTooltip, scope.row[item.value])"
            :title="item.noTooltip ? '单击复制': null"
            :style="{cursor: item.noTooltip ? 'pointer': null}"
            v-else
          >
            <table-colvalue-1
              :arr="data"
...
function _copyText(text) {
  let _this = Object.getPrototypeOf(new Vue())
  import('vue-clipboard2').then(c => {
    Vue.use(c.default)
    _this
      .$copyText(text)
      .then(() => {
  ...
  render(h) {
    ...
    if (_enum || money || isPercent || showTooltip || url) {
      if (url) {
        return (
          <div class="over">
            {getLink(h, item, scope, this.treeTableOtions, numJsx)}
          </div>
        )
      }
      if (_enum) {
    ...
    let content = (
      <div
        ...
        class="over"
        style={{
          ...(expand ? { paddingLeft: (_level - 1) * 20 + 'px' } : {}),
          cursor: copy ? 'pointer' : ''
        }}
      >
        {expandJsx}
