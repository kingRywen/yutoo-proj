  ...
  <el-collapse class="custom-collapse custom-collapse-1" v-model="activeName" accordion>
    <el-collapse-item name="1">
      <div class="collapse-item-title" slot="title">
        <span>筛选条件</span>
        <div v-if="showReset">
          <ElButton @click.stop="resetTable(false)" style="margin-right: 8px" type="text">刷新</ElButton>
          <ElButton @click.stop="resetTable(true)" style="margin-right: 8px" type="text">重置筛选</ElButton>
        </div>
      </div>
      <slot></slot>
    </el-collapse-item>
...
export default {
  ...
  methods: {
    resetTable(refresh) {
      this.$parent.$parent.$parent.getList({}, refresh)
    },
    handleBack() {
      try {
