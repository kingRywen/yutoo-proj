<template>
  <el-input-number
    ...
    :precision="item.precision == null ? 2 : item.precision"
  ...
  <div class="currency" v-else-if="item.type === 'currency'">
    ...
    ></el-input-number>
    <span class="__suffix" v-if="!item.noShow">{{item.suffix ||'$' }}</span>
  </div>
  <!-- 多行文本 -->
  <el-input
    :disabled="disabled"
  ...
  <el-input
    clearable
    :placeholder="disabled ? null : (item.placeholder || `请输入${item.label}`)"
    v-else
    :disabled="disabled"
    :style="{width: item.width || '100%'}"
    :value="currentVal"
