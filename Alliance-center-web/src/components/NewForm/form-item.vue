<template>
  ...
  >
    ...
    <div :class="[{'form-flex': schema.widget !== 'radio'}, {'full-screen': schema.width === 'full'}]">
      <FormInputs
        :item="schema"
        :disabled="disabled"
        :value="value"
      ...
      <template v-if="!schema.showInnerTipFunc || (schema.showInnerTipFunc(rootVal))">
        <span v-if="schema.innerTips && schema.widget !== 'radio'" class="innerTips">{{schema.innerTips}}</span>
      </template>
  ...
  >
    ...
    <template v-else>
      <div v-for="(item, index) in schema.arrayMax" class="flex" :key="index">
...
.noxin /deep/ .el-form-item__label:before {
  // content: '';
  display: none;
  // color: #f56c6c;
  // display: none;
