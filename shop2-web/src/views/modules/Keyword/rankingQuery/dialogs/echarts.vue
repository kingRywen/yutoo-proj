...
export default {
  ...
  methods: {
    ...
    initEcharts() {
      ...
      this.chart.setOption({
        ...
        legend: {
          data: ['关键词：' + this.params.keyword + '的排名']
        },
        tooltip: {
          trigger: 'axis',
          axisPointer: {
          ...
          formatter: function(params, ticket, callback) {
            ...
            let showField2 = vm.echartsData.find(e => e.sendTime == showField1)
              .rankPages
            let showField3 = vm.echartsData.find(e => e.sendTime == showField1)
              .pagesRank
            return `第${showField2 === 0 ? '-' : showField2}页第${
              showField3 === 0 ? '-' : showField3
            }名`
        ...
        yAxis: {
          ...
          inverse: true,
          min: 1,
          max: getBigNumMethod(yMax)
        },
        series: [
          {
            data: datas,
            name: '关键词：' + this.params.keyword + '的排名',
            type: 'line',
            smooth: true,
            itemStyle: {
              normal: {
    ...
    getData() {
      ...
      this.$api['keyword/taskKeywordRankHistoryList'](this.formInline)
        .then(data => {
  ...
  created() {
    ...
    this.formInline.keyword = this.params.keyword
    this.formInline.asin = this.params.asin
    this.formInline.platformId = this.storeInfo.platformId
    this.formInline.siteId = this.storeInfo.siteId
    this.getData()
  },
  beforeDestroy() {
    if (!this.chart) {
