  ...
  <div v-loading="loading">
    <div v-if="!noData" class="main-img">
      <div class="left">
        ...
        <template v-if="data[0] && data[0][0]">
          <div class="text-box" v-for="(item,index) in data[0][0].skuAttrList" :key="index">{{item.attrName}}</div>
        </template>
      ...
      <div id="slider" class="slider-wrapper" @mouseenter="showArrow = true" @mouseleave="showArrow = false">
          ...
          <ul class="slider" v-for="(list, index) in data" :style="getStyle(index)">
            ...
            >
              <div class="__img_wrapper img-wrapper">
                <img v-lazy="item.imageUrl" alt="asin" class="img normal loadingImg" />
              </div>
              <span class="text">
                <b>{{item.asin || '-'}}</b>
              </span>
              ...
              <span
                ...
                v-for="(item,index) in item.skuAttrList"
                :key="index"
              >{{item.attrValue != null && item.attrValue !== '' ? item.attrValue : '-'}}</span>
              ...
              <!-- <span class="text">
                <b v-if="item.sellerCnt != null && item.sellerCnt !== ''">{{item.sellerCnt}}</b>
                <span v-else>-</span>
              </span>-->
            </li>
          </ul>
        ...
        <template v-if="data.length > 1">
          <transition name="carousel-arrow-right">
            <button v-show="showArrow" @click="next" :disabled="index >= data.length - 1" type="button" class="el-carousel__arrow el-carousel__arrow--right">
              <i class="el-icon-arrow-right"></i>
            </button>
          </transition>
          <transition name="carousel-arrow-left">
            <button v-show="showArrow" @click="prev" :disabled="index <= 0" type="button" class="el-carousel__arrow el-carousel__arrow--left">
        ...
        <transition name="el-fade-in">
          <ul
            v-show="showArrow"
            class="el-carousel__indicators el-carousel__indicators--horizontal custom-carousel__indicators"
          >
            <li
              v-for="i in data.length"
              @click="go(i-1)"
              :class="['el-carousel__indicator','el-carousel__indicator--horizontal', {active: i === index+1}]"
            >
        ...
        <!-- 变体标识器 -->
        <div class="numIdentity">{{curNum}}/{{asinsNum}}</div>
      </div>
    </div>
    ...
    <ElDialog width="1200px" :title="`查看跟卖-${title}`" :visible.sync="visible" @close="activeName = 'first'">
      <ElTabs v-model="activeName" type="card" style="margin-top:-20px;margin-bottom:-24px;">
        ...
        <ElTabPane lazy name="second" label="跟卖卖家列表">
          <YtTableSelect
            url="bigProductSellingSellerList"
            :params="params"
...
export default {
              ...
              render(h, scope) {
                ...
                return (
                  ...
                  >
                    ...
                    <span style="text-overflow: ellipsis;display:inline-block;width:170px;white-space: nowrap;overflow: hidden">
                      {skuAttrList.map(el => (
                        <span>
                          {el.attrName}:{el.attrValue} ;{' '}
                        </span>
  ...
  computed: {
    asinsNum() {
      return this.data.reduce((total, cur) => (total += cur.length), 0)
    },
    curNum() {
      return this.focusIndex[0] * 6 + (this.focusIndex[1] + 1)
    },
    params() {
      return {
