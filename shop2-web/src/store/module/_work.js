...
import { createEntry, fetchWorksData } from "./_strapi";
...
import { takeScreenshot } from "../../utils/editor/canvas-helper.js";
...
export const actions = {
  ...
  saveWork(
    ...
    { isSaveCover = false, loadingName = "saveWork_loading" } = {}
  ) {
    // TODO:保存work
    localStorage.setItem("__work", JSON.stringify(state.work));
    // const fn = (callback) => {
    //   new AxiosWrapper({
    //     dispatch,
    //     commit,
  ...
  fetchWorks({ commit, dispatch, state }, workId) {
    commit("setWorks", fetchWorksData);
    // new AxiosWrapper({
    //   dispatch,
    //   commit,
