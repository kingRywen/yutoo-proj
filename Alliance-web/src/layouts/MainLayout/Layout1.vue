<template>
  <el-row type="flex" class="main-layout">
    ...
    <el-col
      :span="sideBar && !dataList && showLeft ? 20 : 24"
      class="right-sidebar"
      :style="{marginLeft: sideBar ? '10px' : 0}"
    >
      <slot name="top"></slot>
      <!-- button -->
      <div class="button" v-if="!dataList">
        <template v-for="(item, index) in btns">
          ...
          <el-button
            ...
            :type="item.name.indexOf('删除') > -1 ? 'danger': item.type || 'primary'"
            :plain="item.plain || item.name.indexOf('删除') > -1"
            size="small"
            @click="item.fn"
          >{{item.name}}</el-button>
        </template>
      ...
      <CollapseWrapper :showReset='false' v-if="Object.keys(searchFields).length" :expand="expand" class="mb10 search-card mt10">
        <div class="search">
          ...
          <div v-if="showSearchBtn" class="search-btn">
            <el-button
              ...
              @click="handleSearch"
            >搜索</el-button>
            <el-button type="warning" size="mini" plain icon="el-icon-reset" :disabled="tableLoading" @click="reset">重置</el-button>
          </div>
        </div>
      </CollapseWrapper>
      <!--</el-card> -->
      ...
      <slot name="table">
        ...
        >
          <transition-type slot="empty" name="fadeIn-fadeOut">
            <div v-if="!tableLoading">
              <NoneData></NoneData>
            </div>
          </transition-type>

          <el-table-column type="selection" v-if="selectionFlag"></el-table-column>
          <ElTableColumn type="index" v-if="showIndex !== false"></ElTableColumn>
          <template v-for="(item, index) in tableCols">
            <el-table-column
              ...
              :render-header="handleRenderHeader.bind(null, item)"
              :show-overflow-tooltip="!item.wrap"
              v-bind="item.props"
              :key="item.prop"
            >
              <template slot-scope="scope">
                <table-colvalue-1 :item="item" :scope="scope" />
              </template>
            </el-table-column>
          </template>
        ...
        <el-table-column label="上级类目" prop="parentName"></el-table-column>
          ...
          <el-table-column label="操作" :width="editWidth" v-if="edits.length" fixed="right">
            <template slot-scope="scope">
              ...
              <!-- <template v-if="$slots.errorMsg" >123132123</template> -->
              <template v-else>
...
Vue.component('table-colvalue-1', {
  props: ['item', 'scope'],
  render(h) {
...
export default {
  ...
  data() {
    return {
      ...
      filterText1: '',
      passText: '',
      maxHeight: this.tableMaxHeight || 570,
      selectedTreeNode: null,
      showTree: true,
      show: true,
      showLeft: this.defaultShowTree,
      refresh: true,
      formData: {},
      leftLoading: false,
      selection: [],
  ...
  methods: {
    ...
    // 表格渲染函数
    handleRenderHeader(item, _h, { column, $index }) {
      if (item.question) {
        return (
          <span>
            ...
            <el-tooltip content={item.question} placement="top">
              <i
                ...
                style="margin-left: 6px; color:#108ee9;cursor:pointer"
              ></i>
            </el-tooltip>
          </span>
        )
