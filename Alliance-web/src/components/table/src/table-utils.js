...
export const getLink = (h, item, scope, treeTableOtions, numJsx) => {
  ...
  if (scope.row[item.value] != null && scope.row[item.value] !== '') {
    if (item.showBtn) {
      !item.noVal && _jsx.push(<ElTooltip placement="left" content={scope.row[item.value]}><span>{scope.row[item.value]}</span></ElTooltip>)
    } else {
      _jsx.push(
        item.routerTo ? <router-link tag="a" class="url" to={item.routerTo(scope.row)} target="_blank">{scope.row[item.value]}</router-link> : item.btnClick ? <a
          class="url"
          href="javascript:;"
    ...
    _jsx.push(
      <div style={item.noVal ? { visibility: 'hidden' } : {}}>-</div>
    )
  }
  let btnJsx = <div class={item.showTooltip ? 'fix__row_2' : null}>{_jsx}{item.showBtn ? null : numJsx}</div>
  let inner = (!item.showTooltip || scope.row[item.value].length < 8) ? btnJsx : <ElTooltip content={scope.row[item.value]} placement="top" effect="dark">{btnJsx}</ElTooltip>
  ...
  let jsx = (
    <div
      ...
      style={item.showBtn ? { paddingRight: '30px', position: 'relative' } : null}
    >
      {inner}
      {item.showBtn && item.numField && scope.row[item.numField] ? <a
        class="url"
        href="javascript:;"
        onClick={(e) => item.btnClick(scope, e)}
        class={[item.showBtn ? 'link-btn' : '', item.btnBg, { 'no-val': item.noVal }]}
      >
