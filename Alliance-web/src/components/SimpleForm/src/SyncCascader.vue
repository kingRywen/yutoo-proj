      ...
      vm.load().then(data => {
        if (data && Array.isArray(data)) {
          ...
          vm.options2 = data.map(el => ({
            ...el,
            disabled: !((this.itemDisabled != null) ? el[vm.props.value] !== this.itemDisabled : true),
            enabled: (this.itemDisabled != null) ? el[vm.props.value] !== this.itemDisabled : true
          }))
          quene(vm.value.slice(0, vm.value.length)).then(res => {
            res.forEach((el, index) => {
              vm.findItem(vm.options2, vm.value[index])
              if (vm.selEl) {
                if (vm.selEl[vm.props.children]) {
  ...
  methods: {
    handleVisibleChange() {
      if (!this.options2.length) {
        this.$nextTick(() => {
          Array.from(
            document.querySelectorAll('.el-cascader-menus.el-popper ul')
          ).map(el => {
            el.innerHTML =
              '<li class="flex ___noData"><span>暂无数据</span></li>'
          })
        })
      }
    },
    handlerChange(val) {
      ...
      if (this.getEl) {
        ...
        if (!this.selEl) {
          ...
          _el = this.selEl[this.props.children].find(
    ...
    handleItemChange(e) {
      ...
      if (vm.selEl.childs.length === 0) {
        if (this.city) {
          ...
          // 正常的
          vm.load(e, vm.selEl, this.itemDisabled).then(data => {
            data = data.map(el => ({
              ...el,
              disabled: !el[vm.enabledKey]
            }))
