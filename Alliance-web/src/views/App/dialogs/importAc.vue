...
export default {
  ...
  data() {
    return {
      ...
      uploadConfig: {
        url:
          CONST_PORT_CONFIG.BASE_URL +
          (this.type == 'mobile' ? 'mobile/importMobile' : 'pc/importPc'),
        headers: {
          token
        },
        data: {
  ...
  methods: {
    downloadTemp() {
      this.$api[
        `main/${
          this.type == 'mobile' ? 'mobileExportTemplate' : 'pcExportTemplate'
        }`
      ]({}).then(data => {
        download(data, (this.type == 'mobile' ? '手机' : 'PC') + '导入模板.xls')
      })
    },
    handleBeforeUpload() {
