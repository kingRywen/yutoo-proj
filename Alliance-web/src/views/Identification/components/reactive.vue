        ...
        <el-row type="flex">
          ...
          <ElCol :span="12">
            <ElFormItem label="应付金额" prop="needPay">
              <el-input-number disabled class="w100" controls-position="right" v-model="data.needPay" :min="1" placeholder></el-input-number>
            </ElFormItem>
          </ElCol>
        </el-row>
      </el-form>
          ...
          <template slot-scope="scope">
            <div>
              <el-popover placement="right" trigger="hover" @show="handleShow(scope)">
                <div>
                  <el-table border :show-header="false" v-if="scope.row.cateList" :data="scope.row.cateList || []">
...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      this.getNeedPay(this.outerData.list.length)
    },
    getNeedPay(count) {
      this.$api[`main/systemSettingPriceReactive`]({count})
      .then(price => {
        this.data.needPay = price.needPay * this.num
      }).catch((err) => {})
    },
