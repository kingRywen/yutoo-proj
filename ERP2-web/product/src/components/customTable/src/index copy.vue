<template>
  <div>
    <search v-on="$listeners" :useNumber="selectData.length" v-bind="$attrs"></search>
    <el-table
      class="special-table"
      :data="tableData"
      style="width: 100%"
        ...
        >
          <template slot-scope="scope">
            <ElButton
              ...
              size="small"
              :disabled="!scope.row[rowKey]? false : true"
              @click="item.fn(scope.$index, scope.row,item.value)"
            >{{scope.row[item.value] || '请选择'}}</ElButton>
          </template>
        </el-table-column>
        <el-table-column
          width="450"
          ...
          <template slot-scope="scope">
            <div :key="indexKey">
              <div class="custom-wrap" v-demo="scope.row">
                <div class="costom-item" v-for="(i, index) in scope.row.btnList" :key="i.id">
