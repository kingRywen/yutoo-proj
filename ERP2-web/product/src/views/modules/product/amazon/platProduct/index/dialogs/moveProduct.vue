<template>
  <div style="display: flex;width:100%;height: 100%;justify-content: space-between;">
    ...
    <div class="rightTable">
      ...
      <div style="margin-top: 15px;">
        <yt-table :columns="columns" :data="rightTableData" selection="radio" @selectChange="rightSelectionChange"></yt-table>
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          value: '',
          render(h, scope) {
            return scope.row.variantProperty &&
              scope.row.variantProperty.length ? (
              <div>
                {scope.row.variantProperty.map(item => (
                  <div>
                    {item.propertyName} : {item.propertyValueShort}
                  </div>
                ))}
              </div>
            ) : (
  ...
  methods: {
    ...
    changeMoreMethod() {
      this._openDialog({
        ...
        params: {
          ...
          catePlatId: this.leftTableData[0].catePlatId,
    ...
    delLeftMethod() {
      this.$confirm('此操作将删除,是否继续', '提示', { type: 'warning' })
        .then(() => {
          let list = this.changeLeftData.map(e => {
            return e.productRepoId
          })
          list.forEach(e => {
            let delIndex = this.leftTableData.findIndex(i => {
    ...
    getRightList() {
      ...
      let api = 'prProductPlatAmzListParent'
      if (this.storeProduct) {
        api = 'prProductSellerAmzListParentMovable'
      }
    ...
    _submit() {
      if (this.changeLeftData.length == 0)
        return this.$message.warning('请选择左边列表,至少一项')
      if (this.changeRightData.length != 1)
        return this.$message.warning('请选择右边列表,只能一项')
      let params = {
        platSiteId: this.platSiteId,
        storeId: this.changeLeftData[0].storeId
      }
      if (this.storeProduct) {
        params.productSellerIds = this.changeLeftData.map(e => e.productSellerId)
        params.parentProductSellerId = this.changeRightData[0].parentProductSellerId
      } else {
        params.productPlatIds = this.changeLeftData.map(e => e.productPlatId),
        params.parentProductPlatId = this.changeRightData[0].productPlatId
      }
      return this.$api[
        `product/${
          this.storeProduct ? 'prProductSellerAmzMove' : 'prProductPlatAmzMove'
        }`
      ](params)
    }
  },
