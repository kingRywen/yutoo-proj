    ...
    <div>
        <el-menu class="navbar" mode="horizontal" :default-active="defaultActive" @select="handleSelect">
            ...
            <el-menu-item :index="item.activeIndex" v-for="item in mainMenu" :key="item.index">
                <div class="nav-item" @mouseenter="handleMouseEnter($event, item.activeIndex)" @mouseleave="handleMouseleave">{{item.name}}</div>
            </el-menu-item>
            <!--
            <el-menu-item index="product">
        <div class="nav-item">产品库</div>
      </el-menu-item>
      <el-menu-item index="3">
        <div class="nav-item">采购管理</div>
      </el-menu-item>
      <el-menu-item index="4">
        <div class="nav-item">客服管理</div>
      </el-menu-item>
      <el-menu-item index="5">
        <div class="nav-item">仓储物流管理</div>
      </el-menu-item>
      <el-menu-item index="6">
        <div class="nav-item">财务管理</div>
      </el-menu-item>
            ...
            <div class="user-profile-container" trigger="click">
                <div class="user-profile-content">
                    <div class="menu-icons top-menu-row">
                        ...
                        <span> <router-link tag="a" class="url" to='/ShopManagement'>店铺管理</router-link></span>
                    </div>
                    <el-dropdown>
        ...
        <div class="top-menu" v-show="showTopMenu" @mouseenter="handleMouseEnter" @mouseleave="handleMouseleave">
            <ElRow class="top-menu-row">
                <ElCol :span="4" class="left">
                    <ul>
                        <li v-for="item in leftMenu" @mouseenter="activeMenu = item.index" :key="item.index" :class="{active: item.index === activeMenu}">{{item.name}}</li>
                    </ul>
                </ElCol>
                <ElCol :span="20" class="right">
                    <ElRow :gutter="20">
                        <ElCol :span="12">
                            <ElTree @node-click="toRouter" :data="currentMenu" :props="defaultProps" :indent="22" :expand-on-click-node="false" default-expand-all>
...
export default {
    ...
    computed: {
        ...
        // 当前模块左侧菜单
        leftMenu() {
            return this.currentTopMenu.children.map(el => ({name: el.name, index: el.activeIndex}))
        },
        // 当前模块右侧菜单
        menuData() {
            let ret = {}
