    ...
    <v-row>
      <v-col span="24">
        <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="产品配件">
          <v-card title=" " class="showCard" @mouseleave.native="changeShow" @mouseenter.native="changeShow">
            <div slot="extra">
              <v-button type="primary" v-if="!$attrs.show" @click="addParts">添加</v-button>
              <v-button type="danger" v-if="!$attrs.show" @click="delParts">删除</v-button>
            </div>
            <v-data-table stripe ref="tablePei" :data='loadData' :pagination="false" :columns='columns' check-type="checkbox" @checkall="checkAll" @clickrow="clickRow" :bottomGap="isIE11" :current-data.sync='tableData'>
            </v-data-table>
          </v-card>
        </v-form-item>
      </v-col>
      <v-col span="24">
        <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="产品相关附件">
          <v-card title=" " class="showCard" @mouseleave.native="changeShow" @mouseenter.native="changeShow">
            <div slot="extra">
              <v-upload :action="attachmentConfig.path" :header="attachmentConfig.header" :data="uploadData" :accept="attachmentConfig.accept" @change="onChange" style="display:inline-block" class="noneFileList">
                <v-button type="primary" v-if="!$attrs.show">添加</v-button>
              </v-upload>
              <v-button type="danger" @click="delFile" v-if="!$attrs.show">删除</v-button>
            </div>
            <v-data-table stripe ref="tableFu" :data='loadUploadData' :pagination="false" :columns='uploadColumns' check-type="checkbox" @checkall="checkAll" @clickrow="clickRow" :bottomGap="isIE11" :current-data.sync='tableData1'>
              <template slot="td" slot-scope="props">
                <div v-if="props.column.field=='edit' && !$attrs.show">
                  <v-button size="small" @click="editPropName(props, $event)">编辑</v-button>
                  <v-button size="small" @click="open(props, $event)">打开链接</v-button>
                </div>
                <div v-else-if="props.column.field=='fileSize'">
                  {{props.content | fixNumber}}KB
                </div>
                <div v-else-if="props.column.field=='attachmentName'">
                  <dynamicInput :isInput="props.item.isInput" :item="props" :text="props.content" @edit="updateName(props, $event)" @cancel="cancel(props, $event)"></dynamicInput>
                </div>
...
export default {
  ...
  methods: {
    ...
    delParts() {
      ...
      arrs = arrs.map(el => el.variantRelId);
      fetchData({
    ...
    delFile() {
      ...
      fetchData({
        ...
        data: {
          parentFlag: this.$attrs.parent ? 1 : 0,
