<template>
  <div class="purchase_box">
    <v-card class="product_box">
      <v-button type="primary" @click="addItem">添加</v-button>
      <v-button type="primary" @click="delItem">删除</v-button>
      <v-data-table stripe :tree-option='treeOption' ref="table" :data='list' :columns='columns' check-type="checkbox" class="margin-top-25" :current-data.sync='tableData'>
        <template slot="td" slot-scope="props">
          <div v-if="props.column.field=='edit'">
            <v-button type="primary" @click="editItem(props, $event)">修改</v-button>
          </div>
...
export default {
  ...
  methods: {
    ...
    // 删除操作
    delItem() {
      let data = this.$refs.table.getCheckedData().map(el => el.variantRelId);
      let vm = this;
