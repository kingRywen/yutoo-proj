<template>
  <!-- 正常输入 -->
  <v-form-item :label="$attrs.label" :label-col="labelCol" :wrapper-col="wrapperCol" v-if="$attrs.type == 1">
  ...
  <v-form-item :label="$attrs.label" :label-col="labelCol" :wrapper-col="wrapperCol" v-else-if="$attrs.type == 9">
    <v-button @click="chooseColor" style="width:100%">选择{{item.propertyName}}</v-button>
