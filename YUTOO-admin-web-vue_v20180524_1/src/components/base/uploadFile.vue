      ...
      <div class="table-responsive">
        ...
        <ul class="upload-allfiles">
          <li v-for="(file, index) in files" :key="file.id">
            ...
            <!-- <v-button type="error" class="dropdown-item del-file-btn" size="small" shape="circle" icon="minus" @click.prevent="$refs.upload.remove(file)"></v-button> -->
          </li>
        </ul>
      </div>
        ...
        <div class="btn-group" v-if="!$attrs.show">
          <v-row :gutter="10">
            <v-col span="8">
              ...
              <v-button type="primary" style="vertical-align: middle;" icon="upload" :loading="$refs.upload && $refs.upload.active" @click.prevent="startUpload">上传文件</v-button>
            </v-col>
...
export default {
  ...
  methods: {
    ...
    inputFile(newFile, oldFile) {
      ...
      if (newFile && oldFile) {
        ...
        if (newFile.active !== oldFile.active) {
          // fetchData({ ...apis.PRODUCT_IMG_UPLOAD }).then(res => {
          //   console.log(res);
          // });
          ...
          //   // vm.files[vm.files.length - 1].base64 = reader.result;
          //   Vue.set(
          //     vm.files,
          //     vm.files.length - 1,
          //     Object.assign({}, vm.files[vm.files.length - 1], {
    ...
    startUpload() {
      ...
      formData.append("file", this.singleFile.file);
      formData.append("productId", +getQueryString("productId"));
      formData.append("remark", this.files[0].remark);

      uploadFileToOss(this.singleFile.file, id, productPlatformId, res => {
        // 如果上传成功，设置状态为成功
        if (res.data.code === 0) {
