  ...
  <div class="main-layout">
    <ElRow :gutter="20">
        ...
        <slot name="left">
          ...
          >
            <div slot-scope="{node, data}">
              ...
              <span :style="{fontWeight: node.level < 3 ? 'bolder' : null}" :class='{treeNodeClass: leftTree.showClassName && !data.leaf}'>
      ...
      >
        <template>
          <!-- 最上面的按钮 -->
          <div v-if="!simple || hiddenTopBtn" class="top-btn">
            <!-- 左边批量 -->
            <div class="top-btn-left">
              ...
              <el-input
                clearable
                :style="{width: topBatchData.search.width}"
                v-model="topBatchSearchVal"
                :placeholder="topBatchData.search.placeholder"
                @clear="searchData[topBatchData.search.field] = null"
              ></el-input>
              <slot name="otherSearch"></slot>
              <div>
                <el-button type="primary" @click="$set(searchData, topBatchData.search.field, topBatchSearchVal)">搜索</el-button>
              </div>
            </div>
        ...
        <div v-if="tabs.length" class="tabs mb10">
          ...
          <el-popover placement="bottom" trigger="click">
            ...
            <div class="list-btn-wrapper" slot="reference">
              <el-button class="list-btn" type="text" icon="iconcaidan iconfont"></el-button>
            ...
            <template slot="right">
              <el-table-column label="操作" align="left" :width="editWidth" v-if="edits.length">
                <template slot-scope="scope">
                  ...
                  <el-button
                    v-else-if="edits[scope.row.buttonList ?scope.row.buttonList[0] - 1 :0].show == null ? true : typeof edits[scope.row.buttonList ?scope.row.buttonList[0] - 1 :0].show === 'boolean' ? edits[scope.row.buttonList ?scope.row.buttonList[0] - 1 :0].show : edits[scope.row.buttonList ?scope.row.buttonList[0] - 1 :0].show(scope)"
                    size="small"
                    @click="edits[scope.row.buttonList ?scope.row.buttonList[0] - 1 :0].fn(scope)"
                    type="primary"
                    plain
...
export default {
  ...
  computed: {
    ...
    topBatchBtnText() {
      return `应用于${this.selection.length}件选定的${this.topBatchData.title}`
    },
    searchTrueData() {
      if (!this.outerParams) {
        return {
      ...
      return Object.assign({}, this.searchData, this.outerParams)
    },
    events() {
  ...
  created() {
    ...
    if (
      ...
      const saveData = storage.get('local', this.saveName)
      if (saveData) {
  ...
  methods: {
    // 保存为常用筛选条件
    saveNormal() {
      storage.set('local', this.saveName, this.searchData)
