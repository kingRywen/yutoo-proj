<template>
    <div>
        <v-form :model="ruleForm" v-if="showSimple" class="____multi_form" ref="ruleForm">
            <v-form-item label="SKU">
                <v-input placeholder="请输入SKU" v-model="ruleForm.sku">
                    <v-select search v-model="skuType" ize="lg" slot="before" style="width: 90px;" :allowClear="false" :data="$attrs.select">
                    </v-select>
                </v-input>
            </v-form-item>
            <v-form-item label="产品名称">
                <v-input placeholder="请输入产品名称" v-model="ruleForm.name">
                    <v-select :allowClear="false" v-model="productNamelanguage" style="width: 80px;" slot="before" :data="[{value: 1, label: '中文名称'}, {value: 2, label: '英文名称'}]">
                    </v-select>
                </v-input>
            </v-form-item>
            <!--  -->
            ...
            <el-collapse-transition>
                <div class="isMultiple" v-if="isMultiple">
                    <v-form-item label="选择时间">
                        <v-select search size="lg" v-model="timeType" style="width: 120px;" :data="timeList" :allowClear="false">
                        </v-select>
                        <el-date-picker size="small" value-format="yyyy-MM-dd HH:mm:ss" v-model="ruleForm.date" type="daterange" align="right" unlink-panels range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickerOptions">
                        </el-date-picker>
                    </v-form-item>
                    <v-form-item label="是否需要拍照">
                        <v-select search size="lg" v-model="ruleForm.photoFlag" :data="[{label:'是', value:1},{label:'否', value:0}]" style="width:100px"></v-select>
                    </v-form-item>
                    <v-form-item label="选择仓库">
                        <v-select search size="lg" v-model="ruleForm.warehouseId" style="width:100px" :data="dataList.warehouseList" label="warehouseName" clue="warehouseId"></v-select>
                    </v-form-item>
                    <v-form-item label="预售属性">
                        <v-select search size="lg" v-model="ruleForm.presellFlag" style="width:100px" :data="[{label:'是', value:1},{label:'否', value:0}]"></v-select>
                    </v-form-item>

                    <v-form-item label="供应商">
                        <!-- {{dataList.supplierList}} -->
                        <v-select label="supplierName" clue="supplierId" search size="lg" v-model="ruleForm.supplierId" style="width:100px" :data="dataList.supplierList"></v-select>

                    </v-form-item>
                    <v-form-item label="产品来源">
                        <v-select search size="lg" placeholder="请选择产品来源" style="width: 185px;" :allowClear="false" v-model="ruleForm.sourceType" :data="productSourceList">
                        </v-select>
                    </v-form-item>
                    <v-form-item label="物流属性">
                        <v-select search size="lg" v-model="ruleForm.logisticType" style="width:100px" :data="dataList.logisticList" label="value" clue="value"></v-select>
                    </v-form-item>
                    <v-form-item label="侵权风险">
                        <v-select search size="lg" v-model="ruleForm.riskType" style="width:100px" :data="dataList.productRiskList" label="value" clue="value"></v-select>
                    </v-form-item>
                    ...
                    <v-form-item label="开发人员">
                        <v-select search size="lg" v-model="ruleForm.developId" style="width: 162px;" :data="dataList.developList" label="username" clue="userId"></v-select>
                    </v-form-item>
                    <v-form-item label="采购人员">
                        <v-select search size="lg" v-model="ruleForm.purchaseId" style="width: 185px;" :data="dataList.purchaseList" label="username" clue="userId"></v-select>
                    </v-form-item>

                    <v-form-item label="产品类型">
                        <v-select search size="lg" v-model="ruleForm.productType" style="width:100px" :data="dataList.productTypeList" label="value" clue="key"></v-select>
                    </v-form-item>
                    <v-form-item label="产品品牌">
                        <v-select search size="lg" v-model="ruleForm.brand" style="width:100px" :data="dataList.brandList" label="brandNameZh" clue="productBrandId"></v-select>
                    </v-form-item>
                    <v-form-item label="是否支持海外仓">
                        <v-select search size="lg" v-model="ruleForm.overseaFlag" style="width:100px" :data="[{label:'是', value:1},{label:'否', value:0}]"></v-select>
                    </v-form-item>
                    <!--  -->

                    <v-form-item label="产品尺寸">
                        <v-select search size="lg" v-model="size" style="width:100px" :data="[{label:'长', value:1},{label:'宽', value:2},{label:'高', value:3}]"></v-select>
                    </v-form-item>
                    ...
                    <v-form-item label="产品重量">
                        <v-select search size="lg" v-model="weight" style="width:100px" :data="[{label:'毛重', value:1},{label:'净重', value:2},{label:'包裹重', value:3}]"></v-select>
                    </v-form-item>
                    ...
                    <v-form-item label="价格类型">
                        <v-select search size="lg" v-model="priceType" style="width:100px" :data="[{label:'采购价', value:3},{label:'原价', value:1},{label:'现价', value:2},{label:'共享价', value:4}]"></v-select>
                    </v-form-item>
        ...
        for (let key in this.ruleForm) {
          ...
          if (
            ...
            if (this.weight == 1) {
              ...
              params["packageWeightStart"] = this.ruleForm[
                "packageWeightStart"
              ];
          ...
          if (
            ...
            if (this.priceType == 1) {
              params["originalPriceStart"] = this.ruleForm[
                "originalPriceStart"
              ];
              ...
              params["purchasePriceStart"] = this.ruleForm[
                "purchasePriceStart"
              ];
