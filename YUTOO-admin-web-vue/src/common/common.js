 ...
 */
export const showDialog = (componentName, modelOpts, opts, listeners, fullScreen) => {

  let vm = top.vm.$children[0]
  if (fullScreen) {
    vm.wrapFullName = 'full_screen'
  } else {
    vm.wrapFullName = ''
  }
...
// 上传图片到oss
export const uploadFileToOss = (file, dir, callback) => {
  oss.getUploadParam(file, dir, callback)
}

// 批量上传图片到oss
export const uploadFileToOssMulti = (file, dir, callback) => {
...
export const logout = () => {
  ...
  fetchData({ ...apis.MERCHANT_USER_LOGOUT
  }).then(res => {
 ...
 * 
 */
export const downloadFile = (url, txt) => {
  if (!txt) {
