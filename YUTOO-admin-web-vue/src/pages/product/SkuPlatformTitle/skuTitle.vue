<template>
    <div>
        <el-breadcrumb separator-class="el-icon-arrow-right" class="nav">
            ...
            <el-breadcrumb-item>sku标题管理</el-breadcrumb-item>
        </el-breadcrumb>
        <div style="padding:10px;">
            ...
            <div class="operation_btn">
                <el-button type="primary" @click="delChecked" size="small">批量删除</el-button>
                <el-button type="primary" @click="found" size="small">添 加</el-button>
                <router-link :to="{ name: 'title-management', params: {} }" style="margin-left:15px;">
                    <el-button type="primary" size="small">标题库管理</el-button>
                </router-link>
            </div>
            ...
            <div class="keyword_list">
                <el-table ref="multipleTable"
                    ...
                    :data="skuList" tooltip-effect="dark" style="width: 100%" border @selection-change="checkboxArr">
                    ...
                    <el-table-column  label="适用店铺">
                        <template slot-scope="scope">
                           <span v-for="item in scope.row.storeEntities" style="margin-right:5px;">{{item.storeRealName}}</span>
                        </template>
                    </el-table-column>
                    <el-table-column fixed="right" label="操作" width="220">
                        <template slot-scope="scope">
                                ...
                                <el-button type="text" @click="relevance(scope.row.titleRuleId)">关联店铺</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </div>
            ...
            <el-dialog :title="dialogTitle" :visible.sync="establish" width="820px">
                <el-form :model="form" :rules="rules" ref="ruleForm" label-width="110px" class="rule_content">
                    <el-form-item label="标题名称：" prop="name" class="form_item">
                        <el-input v-model="form.name" placeholder="请输入标题名称" />
                    </el-form-item>

                    <el-form-item class="form_item" label="平台类型:" prop="platformId">
                            ...
                            <div class="lebel">
                                <el-checkbox-group v-model="form.formalList" size="mini">
                                    <el-checkbox :label="item" border v-for='item in titleLanguageList' class="checkbox">
                                        {{item.name}} <i v-if="item.removeFlag" class="el-icon-delete delicon" @click.stop="deltitle(item.titleGroupId)"/>
                                    </el-checkbox>
            ...
            <el-dialog title="选择店铺" :visible.sync="shopListDialog" center width="342px">
                ...
                <el-checkbox-group v-model="checkShopArr" class="check_warp">
                    <el-checkbox v-for="item in shopList" :label="item.storeId" :key="item.storeId">{{item.storeRealName}}</el-checkbox>
                </el-checkbox-group>
                <div slot="footer">
    ...
    watch: {
        'form.formalList' (arr) {
            arr.forEach((item,index)=>{
                if(this.formalList.find(j=>j.groupId==item.titleGroupId))return;
            ...
            this.formalList.forEach((item,index)=>{
                if(arr.find(curitem=>curitem.titleGroupId == item.groupId))return
    ...
    methods: {
        ...
        deltitle(titleGroupId){
             this.$confirm('确定删除吗?', '提示', {
                ...
                getData('title/group/remove',{titleGroupIds:Array.of(titleGroupId)}).then(res=>{
                    if(res.data.code==0){
                        this.$message({
                          type: 'success',
                          message: `成功${res.data.rows.success}条，失败${res.data.rows.fail}`
                        });
                        this.getTitleLanguage()
                    }else{
        ...
        //关联店铺搜索
        selectShopName() {
            this.getShopList({name:this.relevanceNameInput,titleRuleId:this.titleRuleId})
        },
        //关联店铺确定
        relevanceConfirm(){
        ...
        submitTitleGroupForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    getData("title/group/save",{name:this.titleGroupForm.text}).then(res=>{
                        if(res.data.code==0){
                            this.$message({
                                type: "success",
                                message: res.data.msg
        ...
        delselect(index, groupId) {
            this.formalList.splice(index, 1);
            let formIndex = this.form.formalList.findIndex(item => item.titleGroupId == groupId)
        ...
        delsend(titleRuleIds) {
            getData("title/rule/remove", {
                ...
                if (res.data.code == 0) {
                    this.$message({
                        type: "success",
                        message:`成功${res.data.rows.success}条，失败${res.data.rows.fail}条`
                    });
                    this.getPlatformSku();
                } else {
                    this.$message.error(res.data.msg);
        ...
        redact(titleRuleId) {
            ...
            getData("title/rule/info",{titleRuleId}).then(res => {
                if (res.data.code == 0) {
                    ...
                    this.form.activateFlag = ruleInfo.activateFlag?true:false
        ...
        pushTitle(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    ...
                    //添加
                    if (this.dialogTitle == "添加标题规则") {
                        this.send("title/rule/save",params);
                    //编辑
                    } else {
                        this.send("title/rule/update",{...params,titleRuleId:this.titleRuleId})
                    }
                } else {
                    return false;
                }
            ...
            getData("title/group/list",{}).then(res => {
                if (res.data.code == 0) {
                    ...
                    this.titleLanguageList.forEach(item=>{
                        if(this.formalList.find(curitem=>item.titleGroupId == curitem.groupId)){
                            this.form.formalList.push(item)
                        }
                    })
                } else {
        ...
        search() {
             this.loading = true;
            this.getPlatformSku({...this.searchInfo,pageSize:10,pageNumber:1});
        },
        //获取所有店铺
        getAllShopList(){
            getData("store/list",{pageNumber:1,pageSize:100}).then(res=>{
                if(res.data.code==0){
