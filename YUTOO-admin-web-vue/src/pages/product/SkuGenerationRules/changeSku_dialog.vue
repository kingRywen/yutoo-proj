...
<template lang="html">
    <div class="">
        <el-dialog :title="title" :visible.sync="_ruleDialog" width="900px" modal-append-to-body :append-to-body="true">
            <el-form :model="form" :rules="rules" ref="ruleForm" label-width="90px" class="rule_content">
                ...
                <el-form-item class="form_item" label="预览：">
                    <div class="preview_inputBox">
                        <el-input placeholder="选择规则输入预览" v-model="preview" disabled/>
                        <!-- <span>{{preview}}</span> -->
                    </div>
                </el-form-item>
...
export default {
    ...
    computed: {
        ...
        applyTitle(){
            ...
            // debugger
            if(str.indexOf('店铺') != -1){
                this.$set(this.rules,"objectId",[{required: true,message: '请选择店铺',trigger: 'change'}])
                return 1
            }
            if(str.indexOf('平台') != -1){
                this.$set(this.rules,"platformId",[{required: true,message: '请选择平台',trigger: 'change'}])
                return 2
            }
            if(str.indexOf('仓库') != -1){
                return 3
        ...
        ['form.formalList']:{
            ...
            handler(val,oldval) {
                ...
                val.forEach((item,index)=>{
                    if(this.formalList.find(j=>j.key==item.ruleKey))return
                ...
                this.formalList.forEach((item,index)=>{
                    if(val.find(curitem=>curitem.ruleKey == item.key))return

                    if(oldval[index].hasOwnProperty('content'))oldval[index]['content'] = "";
                        ...
                        if(i.key == j.ruleKey){
                            if(i.content){
                                ...
                                setTimeout(()=>{
                                    ...
                                    this.isImport(Number(i.content),index,vm.formalList[index])
                                },200)
                                return
                            }
                            vm.$set(vm.form.formalList,index,j)
    ...
    methods: {
        ...
        isImport(val, index, item) {
            ...
            this.preview = this.previewArr.join("")
            //随机数字
        ...
        onPreserve(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    if(this.formalList.find(item=>item.key==2)  && this.formalList.find(item=>item.key==9)){
                        if(this.formalList.every(item=>item.digit==false)){
                           console.log("全部通过")
                            ...
                            if(this.applyTitle==1){
                               params = {...params,"objectId":this.form.objectId}
                            }else if(this.applyTitle==2){
                               params = {...params,"objectId":this.form.platformId,}
                            }
