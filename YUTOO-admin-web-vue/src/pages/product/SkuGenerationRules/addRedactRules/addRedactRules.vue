        ...
        return {
            rules: {
                ruleName: [
                    ...
                    {min: 2,max: 10,message: '长度在 8 到 10 个字符',trigger: 'blur'}
                ],
                objectId:[{required: true,message: '请选择店铺',trigger: 'change'}],
                formalList:[{required: true,message: '请输入规则',trigger: 'blur'}]
            },
            formalList:[],
            preview:'输入规则预览',
            form: {
    ...
    methods:{
        ...
        add(curitem) {
            if(this.formalList.length){
                if (this.formalList.find(item => item.globalSkuRuleId === curitem.globalSkuRuleId)) return;
    ...
    watch:{
        formalList(val){
            ...
            val.forEach((item,index)=>{
                this.form.formalList.push(Object.assign({},{
                    "key":item.ruleKey,
                    "digit":'',
