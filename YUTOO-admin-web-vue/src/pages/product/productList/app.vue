      ...
      <div>
        ...
        <el-dropdown-menu slot="dropdown">
          ...
          <el-dropdown @command="handleCommand4">
            <v-button type="primary">批量导入
              <v-icon type="down" size="sm"></v-icon>
            </v-button>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="a">批量导入产品
                <input type="file" ref="file" style="display:none" @change="handleImportFile" accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel">
              </el-dropdown-item>
    ...
    <v-row>
      <v-col>
        <CustomTransition v-if='tableData'>
          <v-data-table showVar :bottomGap="10" stripe :tree-option='treeOption' ref="table" :data='lists' :page-size="pageSize" :page-no="pageNo" :columns='columnsList' check-type="checkbox" @checkall="checkAll" @clickrow="clickRow" class="margin-top-10 showInfoData" :current-data.sync='tableData'>
            <template slot="th" slot-scope="props">
              ...
              <div style="text-align:center" v-else-if="props.column.field=='edit'">
            ...
            <template slot="td" slot-scope="props">
              <div @click="linkToView(props)" style="color:#333">
                <div v-if="props.column.field=='photo'" style="padding:0 10px">
                  <div v-if="!props.item.variantNum && props.item._variant" style="text-align:center">
                    <el-tooltip placement="right" effect="light" :open-delay="300">
                      <div slot="content">
                        <img :src="props.item.imgPath && props.item.imgPath.replace(/,h_\d+,w_\d+/ig, ',h_240,w_240') || '/static/no-image.jpg'">
                      </div>
                ...
                <div v-else-if="props.column.field=='productId'" style="text-align:center">
                  <p>{{props.content}}</p>
                  <v-button v-if="props.item.variantNum" class="viewVariant" type="primary" size="small" style="line-height:24px;height:24px;margin-top:4px" :disabled="isLoading" @click.stop="viewVariant(props, $event)">查看变体({{props.item.variantNum}})</v-button>
                </div>
                ...
                <div v-else-if="props.column.field=='nameZh'" style="max-width:260px" class="__ellapase">
                  <el-tooltip class="item" effect="dark" :content="props.item.nameZh || props.item.variantNameZh" placement="top-start" :open-delay="500">
                ...
                <div v-else-if="props.column.field=='nameEn'" style="max-width:260px" class="__ellapase">
                  <el-tooltip class="item" effect="dark" :content="props.item.nameEn || props.item.variantNameEn" placement="top-start" :open-delay="500">
...
export default {
  ...
  methods: {
    ...
    getDiffTableData(notValid) {
      ...
      variant = data
        .filter(el => el.variantSku != null)
        .map(el => el.productId);
      parent = data.filter(el => el.sku != null).map(el => el.productId);
