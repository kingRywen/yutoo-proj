    ...
    <v-data-table stripe :tree-option='treeOption' ref="table" :data='list' :columns='columns' class="margin-top-25" :current-data.sync='tableData'>
      <template slot="td" slot-scope="props">
        ...
        <div v-else-if="props.column.field=='edit'">
          <v-button :disabled="$attrs.show" :type="props.item.variant.activateFlag?'':'primary'" @click="setStatu(props, false)">无效</v-button>
          <v-button :disabled="$attrs.show" :type="props.item.variant.activateFlag?'primary':''" @click="setStatu(props, true)">有效</v-button>
        </div>
        <div v-for="item in propCol" v-if="propCol.length && props.column.field==item.field">
          {{props.item.variantPropertyList[item.field].propertyValue || ''}}
        </div>
