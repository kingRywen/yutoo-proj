      ...
      <v-form :model="ruleForm" direction="horizontal" :rules="renderedField.vali" ref="ruleForm">
        <v-row>
          ...
          <v-row v-for="(item, key, index) in ((showField.indexOf('productStatus') === -1) ? {main: ruleForm.main} : ruleForm)" :key="'productStatus'+key" v-if="showInputs['productStatus'] || key ==='main'" class="openTab">
            <v-col span="12">
              <v-form-item :label-col="{span:8}" :wrapper-col="{span:16}" :label="key === 'main' ? '产品状态' : item.skuCustom" :prop="`${key}.productStatus`" :required="showRequire.indexOf('productStatus') >= 0" :rules="renderedField.vali['productStatus']">
            ...
            <v-col span="12">
              <v-form-item v-if="ruleForm[key+''].productStatus === 3" :label-col="{span:6}" :wrapper-col="{span:14}" label="到货日期" :prop="`${key}.arrivalDate`" required :rules="{required: true, message: '不能为空'}">
                <el-date-picker style="width:100%" v-model="ruleForm[key+''].arrivalDate" size="small" value-format="yyyy-MM-dd HH:mm:ss" type="datetime" placeholder="选择日期时间" @change="$refs.ruleForm.validateField(`${key}.arrivalDate`)">
                </el-date-picker>
              ...
              <v-form-item v-else-if="ruleForm[key+''].productStatus === 4" :label-col="{span:6}" :wrapper-col="{span:14}" label="停产日期" :prop="`${key}.discontinueDate`" required :rules="{required: true, message: '不能为空'}">
                <el-date-picker style="width:100%" v-model="ruleForm[key+''].discontinueDate" size="small" value-format="yyyy-MM-dd HH:mm:ss" type="datetime" placeholder="选择日期时间" @change="$refs.ruleForm.validateField(`${key}.discontinueDate`)">
                </el-date-picker>
                <div class="iconBox" @click="toggleTabs('productStatus')" v-if="showField.indexOf('productStatus') !== -1">
                  <v-icon type="down" v-if="!showInputs['productStatus'] && key ==='main'"></v-icon>
                  <v-icon type="up" v-if="showInputs['productStatus'] && key ==='main'"></v-icon>
          ...
          <v-col span="24" v-for="(item, key, index) in (!showField.indexOf('statusRemark') === -1 ? {main: ruleForm.main} : ruleForm)" :key="'statusRemark' + key" v-if="showInputs['statusRemark'] || key ==='main'" class="openTab">
            <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" :label="key === 'main' ? '产品状态备注' : item.skuCustom" :prop="`${key}.statusRemark`" :required="showRequire.indexOf('statusRemark') >= 0" :rules="renderedField.vali['statusRemark']">
            ...
            <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="默认产品供货价" prop="main.supplierPrice" :required="showRequire.indexOf('supplierPrice') >= 0" :rules="[{required:true, message: '请设置和选择默认供应商'}]">
              <template v-for="(list, index) in productSupplierPriceEntityList">
                <v-row :gutter="12" style="margin-bottom:12px">
                  ...
                  <v-col span="5">
                    <v-input :disabled="true" :value="list.supplierPrice">
                  ...
                  <v-col span="6" style="display:flex">
                    ...
                    <v-input :disabled="true" v-model="list.purchaseFreight" style="width:100%" @input="setFreight(index)">
          ...
          <v-col span="24" v-for="(item, key, index) in (!showField.indexOf('taxInvoice') === -1 ? {main: ruleForm.main} : ruleForm)" :key="'taxInvoice' + key" v-if="showInputs['taxInvoice'] || key ==='main'" class="openTab">
            <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" :label="key === 'main' ? '开票税点' : item.skuCustom" :prop="`${key}.taxInvoice`" :required="showRequire.indexOf('taxInvoice') >= 0" :rules="renderedField.vali['taxInvoice']">
          ...
          <v-col span="24" v-for="(item, key, index) in (!showField.indexOf('taxPrice') === -1 ? {main: ruleForm.main} : ruleForm)" :key="'taxPrice' + key" v-if="showInputs['taxPrice'] || key ==='main'" class="openTab">
            <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" :label="key === 'main' ? '含税价' : item.skuCustom" :prop="`${key}.taxPrice`" :required="showRequire.indexOf('taxPrice') >= 0" :rules="renderedField.vali['taxPrice']">
              <v-input :disabled="$attrs.show" v-model.number="ruleForm[key+''].taxPrice" @input="setSubVal('taxPrice', $event, index, key)" @blur="handlerBlur('taxPrice', $event, index, key)">
          ...
          <v-col span="24" v-for="(item, key, index) in (!showField.indexOf('salePriceNow') === -1 ? {main: ruleForm.main} : ruleForm)" :key="'salePriceNow' + key" v-if="showInputs['salePriceNow'] || key ==='main'" class="openTab">
            <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" :label="key === 'main' ? '产品最低销售价' : item.skuCustom" :prop="`${key}.salePriceNow`" :required="showRequire.indexOf('salePriceNow') >= 0" :rules="renderedField.vali['salePriceNow']">
              <v-input :disabled="$attrs.show" v-model.number="item.salePriceNow" @input="setSubVal('salePriceNow', $event, index, key)" @blur="handlerBlur('salePriceNow', $event, index, key)">
          ...
          <v-col span="24" v-for="(item, key, index) in (!showField.indexOf('pinkageFlag') === -1 ? {main: ruleForm.main} : ruleForm)" :key="'pinkageFlag' + key" v-if="showInputs['pinkageFlag'] || key ==='main'" class="openTab">
            <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" :label="key === 'main' ? '是否包邮' : item.skuCustom" :prop="`${key}.pinkageFlag`" :required="showRequire.indexOf('pinkageFlag') >= 0" :rules="renderedField.vali['pinkageFlag']">
          ...
          <v-col span="24" v-for="(item, key, index) in (!showField.indexOf('freightPrice') === -1 ? {main: ruleForm.main} : ruleForm)" :key="'freightPrice' + key" v-if="showInputs['freightPrice'] || key ==='main'" class="openTab">
            <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" :label="key === 'main' ? '运费' + productSupplierPriceEntityList[0].supplierPriceUnit : item.skuCustom" :prop="`${key}.freightPrice`" :required="showRequire.indexOf('freightPrice') >= 0" :rules="renderedField.vali['freightPrice']">
              {{item.freightPrice}}
            ...
            <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" :label="key === 'main' ? '产品销售价（共享）' : item.skuCustom">
              <div class="__inlineInput" v-for="(productPrice, subIndex) in item.productPriceRelEntityList" :key="'productPriceRelEntityList'+index +subIndex">
                ...
                <v-form-item :label-col="{span:8}" :wrapper-col="{span:16}" label="数量" :prop="`${key}.productPriceRelEntityList[${subIndex}].numStart`" :rules="numStartVali(item.productPriceRelEntityList, subIndex)">
                  <v-input-number size="large" :min="1" :disabled="subIndex === 0 || $attrs.show" v-model.number="productPrice.numStart" @input="setSubValPrice('numStart', $event, key, item, subIndex, index)" @blur="handlerBlurPrice('numStart', $event, key, item, subIndex, index)">
                  </v-input-number>
                </v-form-item>
                ...
                <v-form-item :label-col="{span:10}" :wrapper-col="{span:14}" label="价格" :prop="`${key}.productPriceRelEntityList[${subIndex}].sharePrice`" :rules="numPrice">
                  <v-input :disabled="$attrs.show" v-model.number="productPrice.sharePrice" @input="setSubValPrice('sharePrice', $event, key, item, subIndex, index)" @blur="handlerBlurPrice('sharePrice', $event, key, item, subIndex, index)">
...
export default {
  inheritAttrs: false,
  props: ["ruleform", "priceUnit", "unit", "productSupplierPriceEntityList"],
  created() {
  ...
  watch: {
    "productSupplierPriceEntityList.0.supplierPrice"(val) {
      _storage.set(
        "session",
