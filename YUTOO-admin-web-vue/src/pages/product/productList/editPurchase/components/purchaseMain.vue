<template>
  <div style="height:100%" class="fix_tabs_height">
    <el-tabs :tab-position="'left'" style="height: 100%;" type="border-card" v-model="name" stretch :before-leave="handlerTabLeave" @tab-click="handlerTabClick">
      <el-tab-pane v-for="(item, index) in tabs" :label="item.title" :key="item.title" :name="item.title" lazy :class="{'cantUse': !sku}" style="height: 100%;">
        <v-card noHovering class="product_box" :title="item.title" :loading="loading">
          <keep-alive>
            <component ref="com" :is="item.component" v-bind="{category, allData,formatField:field, allInfo, tabInfo:allInfo, type, show, parentId}" :data.sync="allData[item.dataName || name]" :cate.sync="catelogy" :tabdata.sync="tabInfo" :flag.sync="flag" :is-purchase="true" :is-variant="isVariantShow" :parent-id="parentId" @toggle-tab="handleToggle"></component>
          </keep-alive>
        </v-card>
      </el-tab-pane>
    </el-tabs>
...
import { fixFrameHeight, fetchData, _storage } from "common/common";
              ...
              for (const key in form) {
                if (form.hasOwnProperty(key)) {
                  ...
                  if (
                    ...
                    element.productEntryPriceEntityList[0].entryInfoPrice = price;
                  }
                }
              }
            }
