<template>
  <v-spin tip="正在读取数据." :spinning="spinning" class="purchase_box">
    <!-- {{ruleForm}} -->
    <productPrice :priceUnit="priceUnit" :unit.sync="unit" v-bind="$attrs" :variantTab="variantTab" v-on="$listeners" :senddata.sync="parentDataMain" :ruleform="parentDataMain" type="1" ref="ruleform" :variantForm="ruleForm" :productSupplierPriceEntityList.sync="productSupplierPriceEntityList"></productPrice>

    <div class="saveBtn" v-if="!$attrs.type && !$attrs.show">
      ...
      fetchData({
        ...
        if (res.data.code === 0) {
          ...
          fetchData({
            ...
            ruleForm.main.productPriceRelEntityList = productPriceRelEntityList;
              ...
              for (const key in ruleForm) {
                if (ruleForm.hasOwnProperty(key)) {
                  ...
                  if (_element.productId === id) {
                    _element.productPriceRelEntityList = _list;
                  }
                }
              }
            });
