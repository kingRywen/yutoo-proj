<template>
  <div>
    <ProductDeliveryPackage v-if="!isTab" ref="parentTab" :productid="parentId" :up-index="curIndex" :show="$attrs.show"></ProductDeliveryPackage>
    <el-tabs v-else ref="tab" type="border-card" v-model="name" class="__flex__column___ fixedHeight" style="padding-bottom: 0;">
      <el-tab-pane label="母体" key="母体" name="母体" lazy>
        <ProductDeliveryPackage ref="parentTab" :productid="parentId" :up-index="curIndex" v-if="name==='母体'" :show="$attrs.show"></ProductDeliveryPackage>
      </el-tab-pane>
      <el-tab-pane v-for="(val, key) in variantEntityList" :key="key" :label="val.variant.sku" v-if="val.variant.activateFlag != 0" :name="val.variant.sku" lazy>
        <ProductDeliveryPackage ref="subTab" :productid="val.variant.productId" :up-index="curIndex" v-if="name===val.variant.sku" :show="$attrs.show"></ProductDeliveryPackage>
      </el-tab-pane>
    </el-tabs>
