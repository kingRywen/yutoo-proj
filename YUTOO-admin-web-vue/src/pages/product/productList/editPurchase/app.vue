<template>
  <div style="height:100%">
    ...
    <keep-alive v-if="type">
      <component is="purchaseMain" :is-variant-show="isVariant" :field="formatField" :category="category" :all-info="tabInfo" :edit="true"></component>
    </keep-alive>

    <!-- 编辑标签页 -->
    <el-tabs :tab-position="'top'" style="height: 100%;" type="border-card" v-else v-model="name" class="__flex__column___">
      <el-tab-pane v-for="(item, index) in mainTabs" :label="item.title" :key="item.title" :name="item.title" lazy style="height:100%">
        <keep-alive>
          <component v-if="item.component && done" is="purchaseMain" :is-variant-show="isVariant" :field="formatField" :category="category" :all-info="tabInfo" :type="type" :edit="true" style="padding-bottom: 50px"></component>
