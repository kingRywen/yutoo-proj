<template>
  <div style="height:100%" v-if="!isBase">
    ...
    <!-- 编辑标签页 -->
    <el-tabs ref="tab" style="height: 100%;" type="border-card" v-model="name" class="info____tab product_box">
      <el-tab-pane v-for="item in mainTabs" :label="item.title" :key="item.title" :name="item.title" style="height:100%;overflow: auto" lazy>
        <component v-if="item.component" is="purchaseMain" :field="formatField" :category="category" :all-info="tabInfo" :type="type" :edit="true" :is-variant-show="true"></component>
        <iframe v-else :src="src" ref="frame" frameborder="0" class="review__frame"></iframe>
      </el-tab-pane>
    </el-tabs>
  </div>
  <!-- 变体展示页面 -->
  <div v-else style="height:100%">
    <div class="product_box" :loading="loading">
      <iframe :src="src" ref="frame" frameborder="0" style="width:100%;height:100%"></iframe>
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    this.isBase = !!getQueryString("base");
  },
  mounted() {
    // 如果是变体页面展示，需要用母体id获取类目
    let id = getQueryString("productId");
    let parentId = getQueryString("parent");
    this.parentId = parentId;

    this.src =
      "/product/productList/editVariant.html" +
