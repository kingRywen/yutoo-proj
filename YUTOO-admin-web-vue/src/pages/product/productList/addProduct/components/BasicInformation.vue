  ...
  <v-spin tip="正在读取数据." :spinning="spinning" style="height:100%;width:100%">
    ...
    <div ref="content">
      <v-form direction="horizontal" :model="ruleForm" :rules="rules" ref="ruleForm">
        ...
        <v-row :gutter="20">
          <!-- {{ruleForm}} -->
          <input-variant v-for="item in inputArrs" @set-input="setCategory(item, $event)" :review="review" :edit="$attrs.edit" :item="item" :cate="catelogy" :options.sync="baseEnum" :group="item.propertyGroupName" :label="item.propertyName" :disabled="item.disabled || show" :prop-item="item.displayKey" :data-type="item.dateType" :input-type="item.inputType" :key="item.displayKey" :type="item.inputType" v-model="ruleForm[item.displayKey]" @selectmore="selectmore" :labelcol="labelCol" :ruleForm="ruleForm" :wrappercol="wrapperCol" :cate-detail="ruleForm.categoryDetail" :cate-second-detail="ruleForm.categorySecondDetail" :history="allData.history"></input-variant>
        </v-row>
        ...
        <v-row :gutter="20" v-if="propertys.length || variantMode">
          ...
          <v-col span="24" class="margin-bottom-20">
            <div v-for="(item, varIndex) in propertys" :key="item.displayKey">
              <input-variant :review="review" :item="item" :edit="$attrs.edit" :parent="1" :label="item.propertyName" :options.sync="baseEnum" :data-type="item.dataType" :input-type="item.inputType" :disabled="item.disabled || show" :prop-item="item.displayKey" :type="item.inputType" v-model="item.propertyValue" :labelcol="labelCol" :wrappercol="wrapperCol" :ruleForm="ruleForm" :cate-detail="ruleForm.categoryDetail" :cate-second-detail="ruleForm.categorySecondDetail" :history="allData.history"></input-variant>
            </div>
          </v-col>
        </v-row>
        ...
        <v-row :gutter="20" v-if="variantNum.length || variantMode">
          <!-- <v-col span="24" v-if="variantMode">
            <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol">
              <h3>{{'母体差异值'}}</h3>
            </v-form-item>
          </v-col> -->
          <v-col span="24" class="margin-bottom-20">
            <div v-for="(item, varIndex) in variants" :key="item.displayKey" style="position:relative">
              <v-row :gutter="16">
                <v-col span="24">
                  <input-variant :review="review" :history="allData.history" :item="item" :edit="$attrs.edit" :parent="1" :label="item.propertyName" :options.sync="baseEnum" :data-type="item.dataType" :input-type="item.inputType" :disabled="item.disabled || show" :prop-item="item.displayKey" :type="item.inputType" :value="ruleForm[item.displayKey]" :ruleForm="ruleForm" @input="setParentVal(item, $event)" :labelcol="labelCol" :wrappercol="{span:18}" :cate-detail="ruleForm.categoryDetail" :cate-second-detail="ruleForm.categorySecondDetail"></input-variant>
                ...
                <template>
                  ...
                  <v-col span="24" v-for="(ii, index) in variantNum" v-if="ii.variant.sku" v-show="showInputs[varIndex]">
                    <!-- {{variantEntityList[index]}} -->
                    <input-variant :ruleForm="ruleForm" :review="review" :history="allData.history" :item="item" :edit="$attrs.edit" :parent="1" :label="ii.variant.skuCustom" :options.sync="baseEnum" :data-type="item.dataType" :input-type="item.inputType" :disabled="item.displayKey === 'sku' || item.disabled || show" :rules="renderedField.vali[item.displayKey]" :prop-item="ii.variant.sku + '_' + item.displayKey" :type="item.inputType" :value="variantEntityList[index][item.displayKey]" :data="variantEntityList[index]" @input="setVariantValue(index, $event, item.displayKey, ii, item)" :labelcol="labelCol" :wrappercol="{span:18}" :cate-detail="ruleForm.categoryDetail" :cate-second-detail="ruleForm.categorySecondDetail" :variant="ii.variant"></input-variant>
                  </v-col>
                </template>
      ...
      <div class="saveBtn" v-if="!$attrs.show">
        <div style="margin: 0 auto">
          <v-button type="primary" :disabled="show" :loading="saveLoading" v-if="!$attrs.show && !variantMode" @click.prevent="save">保存</v-button>
          <v-button type="primary" :disabled="show" :loading="saveLoading" v-if="!$attrs.show && variantMode" @click.prevent="saveVariant">保存</v-button>
          <v-button type="primary" :disabled="show" :loading="commitLoading" v-if="!$attrs.show && !variantMode" @click.prevent="submit">提交</v-button>
        </div>
      </div>
    </div>
  </v-spin>
