      ...
      <v-form :model="ruleForm" direction="horizontal" :rules="rules" ref="ruleForm">
        ...
        <v-row v-if="showInputs['presellFlag'] || key ==='main'" v-for="(item, key, index) in (!showField.indexOf('presellFlag') === -1 ? {main: ruleForm.main} : ruleForm)" :key="'presellFlag' + key">
          <v-col span="24" class="openTab">
            <v-row>
              <v-col span="12">
                <v-form-item :label-col="{span:8}" :wrapper-col="{span:9}" label="是否预售" :prop="`${key}.presellFlag`" :required="showRequire.indexOf('presellFlag') >= 0" :rules="renderedField.vali['presellFlag']">
              ...
              <v-col span="12">
                <v-form-item :label-col="{span:8}" :wrapper-col="{span:12}" label="预售到货日期" v-if="ruleForm[key+''].presellFlag" :prop="`${key}.presellTime`" :required="showRequire.indexOf('presellTime') >= 0" :rules="renderedField.vali['presellTime']">
                  <v-date-picker :disabled="$attrs.show" style="width:100%" :disabled-date="disabledDate" show-time @input="setSubVal('presellTime', $event, index, key)" @blur="handlerBlur('presellTime', $event, index, key)" :value="ruleForm[key+''].presellTime"></v-date-picker>
                </v-form-item>
              </v-col>
            </v-row>
        ...
        <v-row v-if="showInputs['saleTimeStart'] || showInputs['saleTimeEnd'] || key ==='main'" v-for="(item, key, index) in (!showField.indexOf('saleTimeStart') === -1 ? {main: ruleForm.main} : ruleForm)" :key="'saleTimeStart' + key">
          <v-col span="24" class="openTab">
            <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" :label="key === 'main' ? '出售时间' : item.skuCustom" :required="showRequire.indexOf('saleTimeStart') >= 0" :rules="renderedField.vali['saleTimeStart']" :prop="`${key}.saleTime`">
        ...
        <v-row v-if="showInputs['developRemark'] || key ==='main'" v-for="(item, key, index) in (!showField.indexOf('developRemark') === -1 ? {main: ruleForm.main} : ruleForm)" :key="'developRemark' + key">
          <v-col span="24" class="openTab">
            <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" :label="key === 'main' ? '开发备注' : item.skuCustom" :prop="`${key}.developRemark`" :required="showRequire.indexOf('developRemark') >= 0" :rules="renderedField.vali['developRemark']">
        ...
        <v-row v-if="showInputs['purchaseRemark'] || key ==='main'" v-for="(item, key, index) in (!showField.indexOf('purchaseRemark') === -1 ? {main: ruleForm.main} : ruleForm)" :key="'purchaseRemark' + key">
          <v-col span="24" class="openTab">
            <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" :label="key === 'main' ? '采购备注' : item.skuCustom" :prop="`${key}.purchaseRemark`" :required="showRequire.indexOf('purchaseRemark') >= 0" :rules="renderedField.vali['purchaseRemark']">
        ...
        <v-row v-if="showInputs['packageType'] || key ==='main'" v-for="(item, key, index) in (!showField.indexOf('packageType') === -1 ? {main: ruleForm.main} : ruleForm)" :key="'packageType' + key">
          <v-col span="24" class="openTab">
            <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" :label="key === 'main' ? '产品打包状态' : item.skuCustom" :prop="`${key}.packageType`" :required="showRequire.indexOf('packageType') >= 0" :rules="renderedField.vali['packageType']">
            ...
            <v-form-item :label-col="labelCol" :wrapper-col="wrapperCol" :label="key === 'main' ? '上传附件' : item.skuCustom" :prop="`${key}.attachmentRemarkPath`" :required="showRequire.indexOf('attachmentRemarkPath') >= 0" :rules="renderedField.vali['attachmentRemarkPath']">
              <v-row :gutter="10">
                <v-col span="24">
                  ...
                  <v-input v-else disabled v-model="ruleForm[key+''].attachmentRemarkPath"></v-input>
                </v-col>
              </v-row>
