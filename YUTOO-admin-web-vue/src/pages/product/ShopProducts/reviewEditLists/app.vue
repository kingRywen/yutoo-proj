      ...
      <el-tab-pane v-for="(item, index) in mainTabs" :label="item.title" :key="item.title" :name="item.title" lazy style="height:100%">
        ...
        <div class="__review" v-else>
          <v-form direction="horizontal" :model="ruleForm" ref="ruleform">
            <v-row>
              <v-col span="24">
                <v-form-item label="审核" :label-col="labelCol" :wrapper-col="wrapperCol" :rules="[{required:true, message: '请选择审核结果'}]" prop="auditType" required>
                  <v-radio-group v-model="ruleForm.auditType" :data="[{value: 1, text: '审核通过'},{value: 0, text: '审核不通过'}]">
                  </v-radio-group>
                </v-form-item>
              </v-col>
              <v-col span="24">
                <v-form-item label="原因" :label-col="labelCol" :wrapper-col="wrapperCol" :required="ruleForm.auditType === 3" :rules="[{required:ruleForm.auditType === 3 ? true : false, message: '请填写不通过的原因'}]" prop="refuseReason">
              ...
              <v-col span="24" style="text-align:center">
                <v-form-item>
                  <v-button type="primary" @click="commit">确认</v-button>
                </v-form-item>
              </v-col>
            </v-row>
          </v-form>
