    ...
    <div class="___imageLibrary">
      <div class="col left">
        <div>
          ...
          <el-tree :highlight-current="true" v-if="updated" class="filter-tree" :data="data2" :props="defaultProps" :default-expand-all="expandAll" :filter-node-method="filterNode" ref="tree" :expand-on-click-node="false" @node-click="handleNodeClick">
          </el-tree>
        </div>
      ...
      <div class="col right">
        <div v-if="!showInfoImg">
          <!-- {{zipInfo}} -->
          <upload-file :no-edit-photo-btn="true" @successfile="setFile" @getzip="getZip" :zip="zipInfo" key="file"></upload-file>
        </div>
        <div v-if="showInfoImg">
          <upload-img :has-img="imgs" :lib="true" @updatelib="updateData" key="img"></upload-img>
        </div>
      </div>
    </div>
  </div>
...
import { fetchData, toParamUrl, showToast } from "common/common";
