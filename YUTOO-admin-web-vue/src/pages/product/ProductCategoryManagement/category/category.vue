<template>
    ...
    <section class="searchBox">
        ...
        <div style="width:320px;">
            <el-input v-model="select.keyword" clearable size="medium" @input='selectInput(select.keyword)' placeholder="请输入关键字" />
        </div>
        <span class="line">添加时间：</span>
        <el-date-picker v-model="select.timeArr" type="datetimerange" value-format="yyyy-MM-dd HH:mm:ss" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" />
        <el-button type="primary" size="medium" @click="select_category" style="height:36px;">搜 索</el-button>
    </section>
    <!--  -->
    <section class="operationBox">
        <el-button type="primary" @click="edit_category(0)" size="small">添 加</el-button>
        <!-- <el-button type="primary" size="small">删 除</el-button> -->
        <el-button type="primary" @click='record()' size="small">查看操作日志</el-button>
        <el-button type="primary" size="small">
            <router-link :to="{ path: '/record', params: {} }" tag='span'>导 入 记 录</router-link>
        </el-button>
        <el-button type="primary" size="small" @click='ontempalate()'>下 载 模 板</el-button>
        <el-upload class="upload-demo" name="file" :with-credentials="true" :action="uploadTemplateUrl" :headers="headers" :data="file" :before-upload="beforeUploadTemp" :on-success="tolead_success" :on-error="tolead_error" :file-list="fileList" accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel"
            multiple>
    ...
    <section class="categoryBox">
        ...
        <category-tree :data="categoryList" :columns="columns" @addCategoryNext="addCategoryNext" :loading="loading">
                ...
                <template slot-scope="scope">
                        <ul class="btnLists">
                            ...
                            <el-tooltip effect="dark" content="模板设置" placement="top">
                                <router-link :to="{ name: 'templateManagement', params: {categoryId:scope.row.categoryId}}" tag='li'>模</router-link>
                            </el-tooltip>
                            ...
                            <el-tooltip effect="dark" content="编辑" placement="top">
                                <li class="border" @click="edit_category(1,scope.row.categoryId)"><img src="../icon/xieru_icon.png"></li>
                            </el-tooltip>
                            <el-tooltip effect="dark" content="删除" placement="top">
                                <li v-if="scope.row.sourceType=='Merchant'" class="border" @click='delCategory(scope.row.categoryId)'><img src="../icon/del_icon.png"></li>
                            </el-tooltip>
                        </ul>
                    </template>
            </el-table-column>
    ...
    <div class="add">
        <el-dialog :title="title" :visible.sync="addDialog" width='1086px'>
            <div style="height:500px;overflow-y: auto;">
                <el-form :model="CategoryForm" ref="CategoryForm" label-position="right" :inline="true" label-width="135px" class="addForm" size="medium">
                    ...
                    <el-form-item label="类目英文名称：" prop="nameEn" :rules="{required: true, message: '类目名称不能为空', trigger: 'blur'}">
                    ...
                    <el-form-item label="主要开发负责人：" class="take">
                        <el-select v-model.number='CategoryForm.developPicId' clearable @focus="getRelevant(27)">
                            <el-option v-for="item in relevantList" :label="item.nickName" :value="item.userId" />
                        </el-select>
                    ...
                    <el-form-item label="主要图片负责人：" class="take">
                        <el-select v-model.number='CategoryForm.imgPicId' clearable @focus="getRelevant(25)">
                            <el-option v-for="item in relevantList" :label="item.nickName" :value="item.userId"></el-option>
                        </el-select>
                        <div class="show" v-if='type'>
                    ...
                    <el-form-item label="主要编辑负责人：" class="take">
                        <el-select v-model.number='CategoryForm.editPicId' clearable @focus="getRelevant(27)">
                            <el-option v-for="item in relevantList" :label="item.nickName" :value="item.userId"></el-option>
                        </el-select>
                        <div class="show" v-if='type'>
                    ...
                    <el-form-item label="主要采购人：" class="take">
                        <el-select v-model.number='CategoryForm.purchasePicId' clearable @focus="getRelevant(21)">
                            <el-option v-for="item in relevantList" :label="item.nickName" :value="item.userId"></el-option>
                        </el-select>
                        <div class="show" v-if='type'>
                    ...
                    <el-form-item label="主要财务负责人：" class="take">
                        <el-select v-model.number='CategoryForm.financePicId' clearable @focus="getRelevant(30)">
                            <el-option v-for="item in relevantList" :label="item.nickName" :value="item.userId"></el-option>
                        </el-select>
                        <div class="show" v-if='type'>
                    ...
                    <el-form-item label="选择映射平台：" class="map" prop="platformCategoryList" :rules="{required: true, message: '平台不能为空', trigger: 'blur'}">
                        <div class="mapbg">
                            <el-checkbox-group v-model="CategoryForm.platformCategoryList">
                                <el-checkbox-button
                                    :disabled="sourceType=='Default'?true:false"
                                    v-for="item in platformList" :label="item" style="margin:0 15px 10px;">
                    ...
                    <template v-for="(item,index) in userCategorylist" v-if="userCategorylist.length">
                        <el-form-item
                            :label="item[index].platformName+'类目映射：'"
                            v-if='item.length' prop="userSelectSite[0]"
                            :rules="{required: true, message: '站点不能为空', trigger: 'blur'}">
                            <el-checkbox-group v-model="CategoryForm.userSelectSite[index]">
                                <el-checkbox-button
                                :disabled="sourceType=='Default'?true:false"
                                v-for="curItem in item" :label="curItem">{{curItem.siteNameZh}}</el-checkbox-button>
                            </el-checkbox-group>
                        </el-form-item>
                    </template>
                </el-form>

                <template v-for='(item,index) in CategoryForm.userSelectSite' v-if='item.length'>
                            ...
                            <el-table-column fixed="right" label="操作" align='center' width="220">
                                <template slot-scope="scope">
                                    <el-button type="text" size="small"
                                        :disabled="sourceType=='Default'?true:false"
                                        @click="selectPlatform(scope.row.siteId,scope.row.platformId)">选择{{scope.row.platformName}}类型</el-button>
                                    <el-button type="text" size="small"
                                        :disabled="sourceType=='Default'?true:false"
                                        @click="empty(scope.row.siteId,scope.$index,index)">清空</el-button>
                                </template>
                </el-table-column>
                </el-table>
                </template>
            ...
            <div class="extend_content">
                ...
                <div style="display:flex;">
                    ...
                    <el-input type="text" v-model.trim='extendedName' clearable>
                        <el-button slot="append" @click='affirmBtn(1)'>添加</el-button>
                    </el-input>
                </div>
            </div>
            <div slot="footer" class="dialog-footer" style="text-align:center;">
                ...
                <div class="setVariantWarp">
                    ...
                    <p>
                        <el-input type="text" v-model='propertyName' clearable>
                            <el-button slot="append" @click='affirmBtn(0)'>添加</el-button>
                        </el-input>
                    </p>
                </div>
            </div>
                ...
                <div class="setVariantWarp">
                    ...
                    <p style="display:flex;flex-wrap:wrap;padding-left: 35px;">
                        ...
                        <template v-for='(item,index) in uservalueList'>
                                ...
                                <el-select v-if="item.checked" v-model="item.imgPathLevel" @change='lockHierarchy(index)' size="mini" style="width: 130px;margin:0 10px;">
                ...
                <ul>
                    <li>
                        <el-form :inline="true" class="demo-form-inline" size="medium">
                            ...
                            <el-form-item label="到：">
                                <el-input-number controls-position="right" v-model="setActivitiesList[0] && setActivitiesList[0].saleEndCount" :min="setActivitiesList[0] && setActivitiesList[0].saleStartCount" :max="setActivitiesList[0] && setActivitiesList[0].saleEndCount" />
                                <span>&nbsp;&nbsp;个&nbsp;且&nbsp;</span>
                            </el-form-item>
                            ...
                            <el-form-item label="到：">
                                <el-input-number controls-position="right" v-model="setActivitiesList[0] && setActivitiesList[0].delayEndDay" :min="setActivitiesList[0] && setActivitiesList[0].delayStartDay" :max="setActivitiesList[0] && setActivitiesList[0].delayEndDay" /> 天&nbsp;定义为
                    ...
                    <li>
                        <el-form :inline="true" class="demo-form-inline" size="medium">
                            ...
                            <el-form-item label="到：">
                                <el-input-number controls-position="right" v-model="setActivitiesList[1] && setActivitiesList[1].saleEndCount" :min="setActivitiesList[1] && setActivitiesList[1].saleStartCount" :max="setActivitiesList[1] && setActivitiesList[1].saleEndCount" />
                                <span>&nbsp;&nbsp;个&nbsp;且&nbsp;</span>
                            </el-form-item>
                            <el-form-item label="呆滞库龄区间 从：">
                            ...
                            <el-form-item label="到：">
                                <el-input-number controls-position="right" v-model="setActivitiesList[1] && setActivitiesList[1].delayEndDay" :min="setActivitiesList[1] && setActivitiesList[1].delayStartDay" :max="setActivitiesList[1] && setActivitiesList[1].delayEndDay" /> 天&nbsp;定义为
                    ...
                    <li>
                        <el-form :inline="true" class="demo-form-inline" size="medium">
                            ...
                            <el-form-item label="到：">
                                <el-input-number controls-position="right" v-model="setActivitiesList[2] && setActivitiesList[2].saleEndCount" :min="setActivitiesList[2] && setActivitiesList[2].saleStartCount" :max="setActivitiesList[2] && setActivitiesList[2].saleEndCount" />
                                <span>&nbsp;&nbsp;个&nbsp;且&nbsp;</span>
                            </el-form-item>
                            <el-form-item label="呆滞库龄区间 从：">
                            ...
                            <el-form-item label="到：">
                                <el-input-number controls-position="right" v-model="setActivitiesList[2] && setActivitiesList[2].delayEndDay" :min="setActivitiesList[2] && setActivitiesList[2].delayStartDay" :max="setActivitiesList[2] && setActivitiesList[2].delayEndDay" /> 天&nbsp;定义为
                    ...
                    <li>
                        <el-form :inline="true" class="demo-form-inline" size="medium">
                            ...
                            <el-form-item label="到：">
                                <el-input-number controls-position="right" v-model="setActivitiesList[3] && setActivitiesList[3].saleEndCount" :min="setActivitiesList[3] && setActivitiesList[3].saleStartCount" :max="setActivitiesList[3] && setActivitiesList[3].saleEndCount" />
                                <span>&nbsp;&nbsp;个&nbsp;且&nbsp;</span>
                            </el-form-item>
                            <el-form-item label="呆滞库龄区间 从：">
                            ...
                            <el-form-item label="到：">
                                <el-input-number controls-position="right" v-model="setActivitiesList[3] && setActivitiesList[3].delayEndDay" :min="setActivitiesList[3] && setActivitiesList[3].delayStartDay" :max="setActivitiesList[3] && setActivitiesList[3].delayEndDay" /> 天&nbsp;
                            </el-form-item>
                    ...
                    <li>
                        <el-form :inline="true" class="demo-form-inline" size="medium">
                            ...
                            <el-form-item label="到：">
                                <el-input-number controls-position="right" v-model="setActivitiesList[4] && setActivitiesList[4].channelEndSkuCount" :min="setActivitiesList[4] && setActivitiesList[4].channelStartSkuCount" :max="setActivitiesList[4] && setActivitiesList[4].channelEndSkuCount"
                                />
                                <span>&nbsp;&nbsp;个&nbsp;且&nbsp;</span>
                            </el-form-item>
                            ...
                            <el-form-item label="到">
                                <el-input-number controls-position="right" v-model="setActivitiesList[4] && setActivitiesList[4].saleEndDay" :min="setActivitiesList[4] && setActivitiesList[4].saleStartDay" :max="setActivitiesList[4] && setActivitiesList[4].saleEndDay" /> 天&nbsp;定义为
                                <span class="highlight">{{setActivitiesList[3] && setActivitiesList[3].activeName}}</span>
                            </el-form-item>
                        </el-form>
                    </li>
    ...
    <div>
        <el-dialog title="查看操作日志" :visible.sync="recordDialog" width='1086px'>
            <div class="record-body">
                <el-form :inline="true" class="demo-form-inline record_select" size="medium">
                    <el-form-item label="操作时间">
                        <el-date-picker type="datetime" @input='onRecordSelect(false)' v-model='recordForm.startDate' align="right" value-format="yyyy-MM-dd HH:mm:ss" />
                    </el-form-item>
                    <el-form-item label="~">
                        <el-date-picker type="datetime" @input='onRecordSelect(false)' v-model='recordForm.endDate' align="right" value-format="yyyy-MM-dd HH:mm:ss" />
                    </el-form-item>
                    <el-form-item label="操作内容：">
                        <el-input v-model='recordForm.logDesc' @input='onRecordSelect(false)' />
                    </el-form-item>
                    <el-form-item>
                        <el-button type="primary" @click='onRecordSelect(true)'>搜 索</el-button>
                    </el-form-item>
                </el-form>
            ...
            <div slot="footer" class="dialog-footer" style="text-align:center;">
                <el-pagination class="pagination1" @size-change="recordSizeChange" @current-change="recordCurrentChange" :current-page="1" :page-sizes="[8, 15, 25, 30]" :page-size="10" layout="total, sizes, prev, pager, next, jumper" :total="recordList.length">
                </el-pagination>
...
export default {
    ...
    watch: {
        "CategoryForm.platformCategoryList" (val) {
            this.CategoryForm.userSelectSite = val.map(()=>{return []})
            this.userCategorylist = val.map(item => {
                let arr = [];
                // this.CategoryForm.userSelectSite.push([]);
                item.siteList.forEach(j => {
                    arr.push({ ...j,
                        type: null,
                        typeId: null
                    });
    ...
    methods: {
        ...
        selectPlatformtree(val){
            ...
            getData("CategoryPlatform/searchPath",params).then(res=>{
                if(res.data.code == 0){
                    ...
                    data.forEach(item=>{
                        item.categoryPath = item.categoryPath.replace(/:/g,' >> ')
                    })
        ...
        //更新活动数据
        updatEactivity() {
            getData("CateProActiveRule/update", this.setActivitiesList).then(res => {
                if (res.data.code == 0) {
                    this.$message({
                        message: res.data.msg,
                        type: "success"
        ...
        moveFieldsList(type, index, item) {
            ...
            if (type) {
                if (!index) {
                    ...
                    this.userFieldsList.splice(index - 1, 0, this.userFieldsList[index]);
                ...
                if (this.userFieldsList.length - 1 == index) {
                    ...
                    this.userFieldsList.splice(index + 2, 0, this.userFieldsList[index]);
        ...
        delFieldsList(index, id) {
            ...
            this.fieldsList.forEach((item, jindex) => {
                if (item.id == id) {
                    ...
                    this.$refs.table.toggleRowSelection(this.fieldsList[delindex], false);
                }
            });
        },
        //获取差异字段列表
        operateDifference(categoryId) {
            ...
            getData("BasePropertySetM/getListForSetVariants", {
                ...
                if (res.data.code == 0) {
                    ...
                    vm.$nextTick(() => {
                        right.map(item => {
                            let curIndex = vm.fieldsList.findIndex(curItem => item.id == curItem.id)
                            vm.$refs.table.toggleRowSelection(vm.fieldsList[curIndex],true);
                        });
                    });
                } else {
                    this.$message.error(res.data.msg);
            ...
            this.$refs[formName].validate(valid => {
                if (valid) {
                    ...
                    this.CategoryForm.userSelectSite.forEach(item=>{
                        platformCategoryList = item.map(j=>j.typeId)
                    })
                    let params = {
                        categoryM:{
                            parentId:this.CategoryForm.parentId.length?this.CategoryForm.parentId[this.CategoryForm.parentId.length-1]:1,
                            nameEn: this.CategoryForm.nameEn,
                            nameCn: this.CategoryForm.nameCn,
                            relatedCateId:this.CategoryForm.relatedCateId.length?this.CategoryForm.relatedCateId[this.CategoryForm.relatedCateId.length-1]:null,
                    ...
                    //判断选择的站点是否选择了平台类目
                    if(platformCategoryList.every(item=>item!=null)){
                        //编辑保存
                        if (this.type) {
                            params.sourceType = this.sourceType
                            params.categoryM["categoryMId"] = this.categoryId
                            this.addAndUpdate("CategoryM/update",params)
                        //添加保存
                        } else {
                           this.addAndUpdate("CategoryM/save",params)
                        }
                    }else{
                        this.$message.error("请选择平台类目对应的类型")
                    }
                }else{
        ...
        empty(siteId, index, pindex) {
            if (this.userCategorylist[pindex][index].hasOwnProperty("type")) {
                this.userCategorylist[pindex][index]["type"] = null;
                this.userCategorylist[pindex][index]["typeId"] = null;
            }
        },
        //获取平台列表
        getPlatformLists() {
        ...
        //获取相关负责人列表
        getRelevant(num) {
            getData("merchant/user/list/by/role", {
                roleId: num
            },PERMS_URL).then(res => {
                if (res.data.code == 0) {
                    this.relevantList = res.data.rows;
                } else {
                    this.$message.error(res.data.msg);
                }
            });
            ...
            getData("CategoryM/findByCategoryId", {
                ...
                if (res.data.code == 0) {
                    ...
                    this.parentId = categoryInfo.parentId;
                        //编辑添加类目
                        categoryInfo.parentId==1?[]:this.getParentIds(categoryInfo.parentId).then(val=>{
                            this.CategoryForm.parentId = val
                        })
                        categoryInfo.relatedCateId==null?[]:this.getParentIds(categoryInfo.relatedCateId).then(val=>{
                        ...
                        this.CategoryForm.purchasePicId = categoryInfo.purchasePicId
                        ...
                        if(categoryInfo.platformCategoryList instanceof Array && categoryInfo.platformCategoryList.length){
                            categoryInfo.platformCategoryList.forEach((i,index)=>{
                                this.platformList.forEach(j=>{
                                    if(i.platformId == j.platformId){
                                        this.$set(this.CategoryForm.platformCategoryList,index,j)
                                    }
                                })
                                //映射平台站點/類型
                                setTimeout(() =>{
                                   this.userCategorylist.forEach(item=>{
                                       item.forEach(citem=>{
                                           if(citem.siteId == i.siteId){
                                               citem.type = i.plateformCateDesc.replace(/:/g,' >> ')
                                               citem.typeId = i.plateformCateId
                                               this.$set(this.CategoryForm.userSelectSite,index,[citem])
                                           }
                                       })
                                   })
                                })
        ...
        save(type) {
            ...
            if (type) {
                if (this.extend_checkList.length) {
                    ...
                    params = this.attributeList.map(item => {
                        return {
                            ...
                            flag: this.extend_checkList.find(
                                    curItem => curItem.categoryPropertyId == item.categoryPropertyId
                                ) ?
                                1 :
                                0
                        };
                    ...
                    getData("CatePropertyM/updateExtended", params).then(res => {
                        if (res.data.code == 0) {
                            this.$message({
                                message: res.data.msg,
                                type: "success"
                ...
                if (this.uservalueList.length) {
                    let arr = this.attributeList.map(item => {
                        return {
                            ...
                            variantsFlag: this.checkList.find(
                                    i => i.categoryPropertyId == item.categoryPropertyId
                                ) ?
                                1 :
                    ...
                    arr.forEach((item, index) => {
                        params.forEach(curitem => {
                            if (item.categoryPropertyId == curitem.categoryPropertyId) {
        ...
        getCategoryList(
            ...
            getData("CategoryM/list", params).then(res => {
                this.loading = false;
                if (res.data.code == 0) {
                    if (res.data.category == null || res.data.category == undefined) {
                        this.categoryList = [];
                        return;
                    }
