<template lang="html">
  ...
  <ul>
    ...
    <li>
      时间：
      <el-date-picker v-model="selectInfo.startTime" value-format="yyyy-MM-dd" type="datetime" size="small" placeholder="开始时间">
      </el-date-picker> ~
      <el-date-picker v-model="selectInfo.endTime" value-format="yyyy-MM-dd" size="small" type="datetime" placeholder="结束时间">
      </el-date-picker>
    </li>
    <li>
  ...
  <el-table :data="batchList" tooltip-effect="dark" style="width: 100%" v-loading="loading" element-loading-text="拼命加载中" @selection-change="onUserSelectList">
    ...
    <el-table-column label="备货时间" width="120" align="center">
      <template slot-scope="scope">
        {{ scope.row.stockingDays | _formatData }}天
      </template>
    </el-table-column>
    <el-table-column prop="userName" label="操作人员" align="center" width="80" :formatter="_formatter" />
    <el-table-column label="状态" width="120" align="center">
      <template slot-scope="scope">
        <span v-if="scope.row.detailedTypeString === '未处理'" style="color: #f56c6c">{{ scope.row.detailedTypeString | _formatData }}</span>
    ...
    <el-table-column label="操作" width="220" align="center">
      <template slot-scope="scope">
        ...
        <el-button v-if="scope.row.detailedStatus==2" type="text" @click="cancel(scope.row.replenishDetailedId)">取消补货</el-button>
      </template>
    </el-table-column>
  </el-table>
</template>
  ...
  <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[10, 20, 35,50]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total">
  </el-pagination>
</template>
        </IRLayout>
        <el-dialog
      ...
      <el-form ref="form" :model="form" label-width="120px">
        <el-form-item label="选择运输方式">
          <el-select v-model="form.batchNumber" placeholder="请选择运输方式" clearable style="width:100%">
            <el-option v-for="item in lists" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    saveTransport() {
      ...
      fetchData({
        url: "replenishDetailed/setTransport",
        method: "post",
        data: {
