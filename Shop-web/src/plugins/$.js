 ...
 * 转换成太平洋时间
 * 
 * @param  {time}        时间,不传代表转换当前时间为太平洋时间
 * @param  {cFormat}     格式
 * @return {String}      字符串
 *
 * @example              getPecTime('2018-1-29', '{y}/{m}/{d} {h}:{i}:{s}') // -> 2018/01/29 00:00:00
 */
funs.getPecTime = (type = "{y}/{m}/{d}", time) => {
  // return dayjs(Date.now() - 15 * 3600 * 1000).format('YYYY-MM-DD'))
  time = new Date(time).getTime() || Date.now()
  return funs.formatDate(time - 15 * 3600 * 1000, type)
}
 ...
 */
funs.getChartTooltip = (params, cb) => {
  ...
  let html = [
    `${params[0].axisValue}`,
    ...params.map(el => `${el.marker} ${el.seriesName}: ${cb ? cb(el) : el.data}`)
  ].join('<br>')
  return html
}
