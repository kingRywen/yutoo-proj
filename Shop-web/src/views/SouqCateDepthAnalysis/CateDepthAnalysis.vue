...
export default {
  ...
  methods: {
    ...
    getList(arg) {
      ...
      return this.$api["TaskCateAnalysisSouqList2"](params)
        .then(data => {
    ...
    catchCate(arg) {
      this.$api["TaskCateDepthAnalysisSouqReCrawl"](arg).then(data => {
        this.getList();
      });
    },
    // 删除
    deleteTask(taskIds) {
      ...
      return this.$api["TaskCateAnalysisSouqRemove2"](params)
        .then(data => {
          if (data.code === 0) {
            return true;
          } else {
    ...
    // 类目清空表单
    resetFields_add() {
      this.options.URL = "TaskCateDepthAnalysisSouqSave2";
    ...
    delete__(arg) {
      this.$confirm("此操作将删除该项, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          this.$api["TaskCateAnalysisSouqRemove2"](params)
            .then(data => {
              if (data.code === 0) {
                let param = {
                  pageSize: 10,
