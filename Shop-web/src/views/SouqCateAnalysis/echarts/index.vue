...
export default {
  ...
  methods: {
    // 获取chartRank表格数据
    showChartChild(category, startTime, endTime) {
      ...
      this.$api["TaskCateAnalysisSouqListToNewReviewCnByCate"](opts)
        .then(data => {
    ...
    changeTime(val) {
      if (!val) {
        this.showChartChild(this.chartData.category);
        return;
      }
      // 有日期则往下走
      let startTime = this.DateMsg.DateValue[0],
        endTime = this.DateMsg.DateValue[1];
      this.showChartChild(this.chartData.category, startTime, endTime);
    },
    change() {
      this.showChartChild(this.chartData.category);
    },
