<template>
    <div style='min-width:450px;margin-top:20px;padding-bottom:10px;' :style="{width:goodsWidth}">
                        ...
                        <div  slot-scope="{ node, data }" style='width:100%'>
                            <div style='position:relative;'>
                                ...
                                <div style='position:absolute;top:-10px;right:20px;'>
                                    <el-button type="primary" @click='orientationBtn(data)' :disabled="addId.includes(data.pcid)">定位</el-button>
                                    <el-button type="text" @click='refiningBtn(data)' :disabled="addId.includes(data.pcid)">细化</el-button>
                                </div>
                            </div>
                        </div>
                    </el-tree>
        ...
        <yt-dialog :options='options' :events='events'>
            <Refining v-if='options.visible' :addName='addName' :brandsOpts='brandsOpts' ref='RefiningDialog' />
        </yt-dialog>
    </div>
</template>
            ...
            columns:[
                {
                    ...
                    align:'left',
                    render(h,scope){
                        let {asin , name ,brands,priceMin,priceMax,reviewMin,reviewMax,primeEligible } = scope.row
                        //名字
                        let nameC = name ? (
						...
						if(brands && brands.length){
							let str = ''
							brands.forEach((e,index,arr)=>{
								str += e.name
								if(index !== arr.length - 1) str += ','
							})
							brand = ( <span> &nbsp; <b>品牌 :</b>{str}</span> )
						}
                        
                        //价格
                        let minPrice = priceMin ?  ( <span> &nbsp; <b>商品价格 :</b>{priceMin}</span> ) : (<span></span>)
                        let maxPrice = priceMax ?   ( <span>~ {priceMin}</span> ) : (<span></span>)
                        //评分
                        let revie = reviewMin && reviewMax ? (<span> &nbsp; <b>评分 :</b>{reviewMin + '-' + reviewMax}</span>) : (<span></span>)
                        //配送
                        let delivery = primeEligible != null ? (<span> &nbsp; <b>配送 :</b>{primeEligible ? '具备Prime资格' : '不具备Prime资格'}</span> ) : (<span></span>)

                        return(
                           <div> {nameC}  {brand} &nbsp; {minPrice} {maxPrice} &nbsp; {revie} &nbsp; {delivery}</div>
                        )
                    }
                },
                {
            ...
            events:{
                handleOkClick:()=>{
                    this.$refs.RefiningDialog.saveBtn().then(res => {
                        ...
                        this.ruleForm.categorys.push(res)
                        this.addId.push(res.pcid)
                    }).catch(() => {
