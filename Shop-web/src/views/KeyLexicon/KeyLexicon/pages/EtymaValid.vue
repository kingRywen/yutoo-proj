    ...
    <div class="box">
      ...
      <el-collapse-item title="筛选条件" name="1">
        ...
        <div class="box-update-btn">
            <el-button  size="small" type="primary" plain @click="showDialog(null)">添加词根</el-button>
            <el-button  size="small" type="primary" plain @click="deleteRows(null)">删除</el-button>
            <el-button  size="small" type="primary" plain @click="showKeywordRootDialog">加入无效词根</el-button>
        ...
        <div class="box-data">
            ...
            <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="listQuery.pageNumber" :page-sizes="[10,20,30, 50,1000]" :page-size="listQuery.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total">
            </el-pagination>
        </div>
        ...
        <yt-dialog  :options="options" :events="events" class="form-dialog">
            <el-form size="small" label-position="right" label-width="60px" :rules="rules" :model="ruleForm" ref="ruleForm" >
                <el-form-item label="关键词"
                prop="rootText">
                    <el-input v-model.trim="ruleForm.rootText" size="small"
                    type="textarea" placeholder="请输入关键词,多个关键词换行输入,注意:只能输入字母、数字、单引号"
                    :rows="5"></el-input>
                </el-form-item>
            </el-form>
        </yt-dialog>
        ...
        <yt-dialog  :options="keywordRootOptions" :events="keywordRootEvents" class="form-dialog">
            <el-form size="small" label-position="right" label-width="80px"  :model="keywordRootRuleForm" ref="keywordRootRuleForm" >
                <el-form-item label="无效类型"
                prop="rootType"
                :rules="{
                  required: true, message: '无效类型不能为空', trigger: 'blur'
                }">
                   
                    <el-select class="w100" v-model="keywordRootRuleForm.rootType" placeholder="请选择" size="small">
                        <el-option label="精准无效" value="1"></el-option>
                        <el-option label="词组无效" value="2"></el-option>
                        <el-option label="绝对无效" value="3"></el-option><!-- 组合无效/绝对无效 -->
                        <!-- <el-option label="组合有效" value="4"></el-option> -->
                    </el-select>
...
export default {
    ...
    methods: {
        ...
        getList() {
            ...
            let {include,exclude,barring,rootTexts,...info} = JSON.parse(JSON.stringify(this.listQuery));
            let listQuery = {...info};
      ...
      submitKeywordRootForm(){
        ...
        //    this.keywordRootRuleForm.rootType =this.listQuery.rootType
           ...
           let keywordRootRuleForm =_.cloneDeep(this.keywordRootRuleForm)

           keywordRootRuleForm.rootType = parseInt(keywordRootRuleForm.rootType)

           let params ={
            self:this,
            formName:'keywordRootRuleForm',
            formData:keywordRootRuleForm,
            dialogProperty:'keywordRootOptions',
      ...
      deleteRows(row){
        ...
        let ids =[]
            ...
            //     list
            // }
            this.$confirm('此操作将删除类目词库和产品词库中对应的所有记录请慎重操作, 是否继续?', '提示', {
