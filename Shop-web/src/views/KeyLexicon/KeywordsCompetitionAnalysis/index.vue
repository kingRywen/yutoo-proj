<template>
	<div>
		<div v-if='$route.name==="KeywordsCompetitionAnalysis"'>
			<MainLayout
					...
					},
					{
						...
						fn: (scope) => {
							...
							vm.$confirm('此操作将重新抓取,是否继续','提示',{type:'warning'}).then(()=>{
								vm.$api['taskKeywordCompeteReCrawl'](taskIds).then(()=>{
									vm.getLayoutList()
								})
							}).catch(()=>{})							
						}
					...
					},
					{
						...
						fn: ()=>{
							...
							vm.$confirm('此操作将删除,是否继续','提示',{type:'warning'}).then(()=>{
								vm.$api['taskKeywordCompeteRemove']({taskIds}).then(()=>{
									vm.getLayoutList()
								})
							}).catch(()=>{})
						}
