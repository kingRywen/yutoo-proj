    ...
    <div>
        <el-form :label-position="'center'" :model="formInline" ref='formInline'  :rules='rules' label-width='90px'>
            <el-form-item label="选择店铺" prop='sellerId'>
                <el-select v-model="formInline.sellerId" placeholder="请选择" size='small' style='width:400px' clearable @change='addSuccess'>
                    <el-option
                    v-for="item in params.SourceShopsOptions"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value">
                    </el-option>
            ...
            <div>
                <el-form-item label="销售价" prop='source' style='display:inline-block;width:140px;'>
                    <el-select v-model="formInline.source" placeholder="请选择" size='small' style='width:120px;' clearable  @change='sourceChange'>
                        <el-option
            ...
            <el-form-item label="默认价格" prop='defaultValue' v-if='(formInline.source!==0 && formInline.source!==4) && defaultValueShow'>
                <el-input v-model.trim="formInline.defaultValue" placeholder="请输入值" size='small' style='width:400px;' clearable></el-input> $
            </el-form-item>
            <el-form-item label="运费模板" prop='tempId'>
                <el-select v-model="formInline.tempId" placeholder="请先选择店铺,再做选择" size='small' style='width:310px;' clearable>
                    <el-option
    ...
    methods: {
        _submit(){
            return this.$refs.formInline.validate().then( async()=>{
                ...
                let res = await this.$api['imSaleProdExeSave'](this.formInline);
