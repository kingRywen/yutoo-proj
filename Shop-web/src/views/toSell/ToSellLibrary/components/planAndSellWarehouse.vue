            ...
            <div slot="search">
                <el-form :label-position="'center'" :model="formInline" ref='formInline' :inline="true" :rules='rules'>
                    ...
                    <el-form-item label="跟卖最高价格/源总价格" prop="minHighest">
                        <el-input type='text' v-model.trim="formInline.minHighest" size='small' placeholder="请输入小值" style='width:100px;'></el-input> %
                    </el-form-item>
                    <el-form-item label="~" prop="maxHighest">
                        <el-input type='text' v-model.trim="formInline.maxHighest" size='small' placeholder="请输入大值" style='width:100px;'></el-input> %
                    </el-form-item>
                    <el-form-item label="跟卖最低价格/源总价格" prop="minLowest">
                        <el-input type='text' v-model.trim="formInline.minLowest" size='small' placeholder="请输入小值" style='width:100px;'></el-input> %
                    </el-form-item>
                    <el-form-item label="~" prop="maxLowest">
                        <el-input type='text' v-model.trim="formInline.maxLowest" size='small' placeholder="请输入大值" style='width:100px;'></el-input> %
                    </el-form-item>
                    <el-form-item label="是否有亚马逊自营" prop="fbaPriceFlag">
                        <el-select v-model="formInline.fbaPriceFlag" placeholder="请选择" size='small' style='width:180px;'>
                            <el-option
                    ...
                    <el-form-item label="源站点" prop="sourceSiteId">
                        <el-select v-model="formInline.sourceSiteId" placeholder="请选择" size='small' style='width:180px;'>
                            <el-option
                    ...
                    <el-form-item label="源店铺" prop="sourceSellerId">
                        <el-select v-model="formInline.sourceSellerId" placeholder="请选择" size='small' style='width:180px;'>
                            <el-option
                    ...
                    <el-form-item label="是否可送送至目标国家" prop="deliveryFlag">
                        <el-select v-model="formInline.deliveryFlag" placeholder="请选择" size='small' style='width:180px;'>
                            <el-option
                            v-for="item in fbaFlagOptions"
                            :key="item.value"
                            :label="item.label"
...
export default {
    props:['radioSiteId','siteList','pcid','radioSiteName','symbol'],
    data() {
        let vm=this;
        ...
        return {
            ...
            editBtns:[
                ...
                },
                {
                    ...
                    // },
                    fn: () => {
                       return this.globallySetFreight()
                    }
                },
                {
                    name: '设置保底利润率运费',
                ...
                },
                {
                    name: '修改源库存量',
                    disabled: () => {
                        //let firstSellerId = vm.selectIds.length && vm.selectIds[0].sellerId  ;
                        let flag = vm.selectIds.length===0 ? true :false;
            ...
            columns:[
                ...
                },
                {
					...
					render(h,scope){
					    ...
					    //     let priceArr = []
					    //     scope.row.childList.forEach(i=>{
					    //         if( i.targetCommission < 0 ){
					    //             priceArr.push(null)
					    //         }else{
					    //             priceArr.push(i.targetCommission)
					    //         }                             
					    //     });
					    //     let newArr = [];
					    //     priceArr.forEach(i=>{
					    ...
					    //     });
					    //     if( newArr.length ){
					    //          span = (  Math.ceil(Math.min.apply(null,newArr)*10000)/100+'%'+ '~' + Math.ceil(Math.max.apply(null,newArr)*10000)/100+'%'  ) ;
					    //          if(  Math.min.apply(null,newArr) === Math.max.apply(null,newArr) ){
					    //             span = Math.ceil(Math.min.apply(null,newArr)*10000)/100+'%'
					    //         }
					    //     }else{
					    //         span =  ('-')
					    //     }
					    // }else{
					        span = (  (scope.row.targetCommission || scope.row.targetCommission===0) ? Math.ceil(scope.row.targetCommission*10000)/100+'%' : '-' );
					    //};
					    return(
					        <span>{ span }</span>
					    )
                ...
                },
                {
                    ...
                    render(h,scope){
                        ...
                        //     }
                        // }else{
                            span = (  (scope.row.profitRate || scope.row.profitRate===0) ? Math.ceil(scope.row.profitRate*10000)/100+'%' : '-' );
                        //};
                        return(
                            <span>{ span }</span>
                        )
                ...
                },
                {
                   ...
                   // sortable:"custom"
                    render(h,scope){
                        ...
                        //     }
                        // }else{
                            span = ( (scope.row.sourceFare || scope.row.sourceFare===0) ? vm.symbol + scope.row.sourceFare : '-' );
                        //};
                        return(
                            <span>{ span }</span>
                        )
    ...
    methods: {
        ...
        exportBtn(){
            ...
            .then(data => {
                downloadFile(data, '计划跟卖库' + this.$.formatDate(new Date(),'{y}{m}{d}{h}{i}{s}'))
            })
            .catch(() => {})
        },
        //表单搜索的真实数据
        ...
        addToSellLibrary(){
            this.$refs.layout.$dialog({
                ...
                params:{
                    asins:this.selectIds,
                    SourceShopsOptions:this.AuthorizedShops,
                    siteId:this.radioSiteId
                },
                component: () => import('./dialog/AddToSellDialog.vue'),
                okBtnText: '确认',
                cancelBtnText: '取消'
            })
        },
        ...
        globallySetFreight(){
            this.$refs.layout.$dialog({
                ...
                component: () => import('./dialog/SetFreightDialog.vue'),
                okBtnText: '确认',
                cancelBtnText: '取消'
            })
        },
        ...
        getAuthorizationShop(){
            ...
            this.AuthorizedShops = opt
            //console.log(this.AuthorizedShops);
        },
    ...
    watch:{
        radioSiteId:{
            handler(oldName,newMame){
                if( this.radioSiteId) {
                    this.$set(this.outerParams,'productFlag',1);
