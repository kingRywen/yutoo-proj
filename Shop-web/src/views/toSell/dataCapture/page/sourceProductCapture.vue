			...
			return{
				...
				columns:[
					...
					},
					{
						...
						value:'asins',
						render(h, scope) {
						  let num = scope.row.asins.length
						  return num > 0 ? (
						    <el-button
						      type="text"
							  style='color: #108ee9;'
		...
		methods:{
			...
			confirmMethod(type,data){
				this.$confirm(`此操作将${type === 'del' ? '删除' : '重新抓取'},是否继续`,'提示',{type:'warning'}).then(()=>{
					...
					this.$api[`${type === 'del' ? 'taskSaleProductBatchRemove' : 'taskSaleProductReExecute'}`](params).then((e)=>{
