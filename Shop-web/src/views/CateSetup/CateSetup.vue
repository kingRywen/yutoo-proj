    ...
    <el-dialog-yt :options="options" :events="events">
      <div class="box __box_wrapper">
        ...
        <el-form size="small" :model="form" :rules="rules" ref="ruleForm">
          <el-form-item label="类目名称" prop="cusCateName" :label-width="formLabelWidth" style="width:100%;">
            <el-input v-model.trim="form.cusCateName" placeholder="请输入类目名称" style="width:90%" size="mini"></el-input>
          </el-form-item>
          <!-- {{form.ids}} -->
          <el-form-item label="上级类目" prop="ids" :label-width="formLabelWidth" style="width:100%;">
            <el-cascader
              placeholder="搜索/选择类目（为空时默认根类目）"
              ...
              change-on-select
              style="width:90%;"
              :clearable="!handlerType"
              size="mini"
              :filterable="table.filterable"
              :disabled="handlerType"
            ></el-cascader>
            <div class="font_color_tips" v-if="handlerType">温馨提示:新增下级，不能选择上级类目。</div>
          </el-form-item>
          <!-- <el-form-item label="退货率预警值" prop="returnRate" :label-width="formLabelWidth" style="width:100%;">
            <el-input v-model.number="form.returnRate" placeholder="请输入退货率预警值" style="width:90%" size="mini"></el-input>%
          </el-form-item>-->
          <el-form-item
          ...
          >
            <el-checkbox v-model="form.haveChildren" :disabled="showAmazonCate?true:false">是否底层类目</el-checkbox>
            <!-- <el-checkbox v-model="form.haveChildren" :disabled="showAmazonCate?true:false" v-else>已经属于底层类目不能编辑</el-checkbox> -->
          </el-form-item>
          <el-form-item
            v-if="form.haveChildren"
            prop="customCategoryMapList"
            label="亚马逊类目映射"
            :label-width="formLabelWidth"
            style="width:100%;margin-bottom:0"
          >
            <el-checkbox-group v-model="form.siteIds" size="mini" @change="change_">
              <!-- siteList来自vuex -->
              <el-checkbox-button v-for="item in siteList" :label="item.siteId" :key="item.siteId">{{item.siteName}}</el-checkbox-button>
            </el-checkbox-group>
          </el-form-item>
          <el-form-item
            v-if="form.siteIds.length&&form.haveChildren"
            label-width="8px"
            style="width:100%;margin-bottom:15px;"
          >
            <el-table ref="Table2" :data="table2" style="width: 100%" border class="Table2 mt10">
              <el-table-column prop="siteName" label="国家" width="100" align="center"></el-table-column>
              <!-- <el-table-column prop="category" label="亚马逊类目" align="center">
              </el-table-column>-->
              <el-table-column prop="category" label="亚马逊类目">
                <template slot-scope="scope">
                  <div
                    class="fix__row_2"
                    v-if="scope.row.category === '' || scope.row.category == null"
                  >{{scope.row.category ||'-'}}</div>
                  <el-tooltip v-else placement="top" :content="scope.row.category">
                    <div class="fix__row_2">{{scope.row.category | _formatData_cate}}</div>
                  </el-tooltip>
                </template>
              </el-table-column>
              <el-table-column prop=" handle " label="操作 " width="160" align="center">
                <template slot-scope="scope ">
                  <el-button type="text " size="mini" @click="showCate(scope)">选择亚马逊类目</el-button>
                  <el-button
                    type="text "
                    size="mini"
                    @click="clearCate(scope)"
                    :disabled="scope.row.category?false:true"
                  >清空</el-button>
                </template>
              </el-table-column>
            </el-table>
...
export default {
  ...
  methods: {
    ...
    getList(arg) {
      ...
      if (params.cusCateName) {
        this.searchName = params.cusCateName
      } else {
        this.searchName = ''
      }
      // 如果与日期，要分开赋值
        ...
        .then(data => {
          ...
          if (!params.haveChildren) {
            ...
            if (rows) {
              ...
              // if (this.copy_rows) {
              //   this.allRowsArr = [];
              //   this.allRowsArr_copy = [];
              //   this.allRows(this.table.rows, this.allRowsArr);
              //   this.allRows(this.oldData, this.oldDataArr);
              //   console.log(this.allRowsArr, "allRowsArr");
              //   console.log(this.oldDataArr, "copy_rows");
              //   if (this.allRowsArr < this.oldDataArr) {
              //     // 是删除
              //     this.copy_rows.forEach((el, index) => {
              //       //找到本地保存的是否存在新加载的
              //       let item = this.allRowsArr.find(value => {
              //         return value.cusCateId === el.cusCateId;
              //       });
              //       if (!item) {
              //         this.copy_rows.splice(index, 1);
              //         // console.log(index, "index");
              //       }
              //     });
              //     console.log(this.copy_rows, "this.copy_rows1");
              //     // 将父id不存在的也去掉
              //     // this.copy_rows.forEach((el, index) => {
              //     //   //找到本地保存的是否存在新加载的
              //     //   let item = this.allRowsArr.find(value => {
              //     //     return value.cusCateId === el.parentCusCateId;
              //     //   });
              //     //   if (!item) {
              //     //     this.copy_rows.splice(index, 1);
              //     //     // console.log(index, "index");
              //     //   }
              //     // });
              //     // console.log(this.copy_rows, "this.copy_rows2");
              //     this.table.rows = this.copy_rows;
              //     // return;
              //   } else {
              //     this.table.rows = JSON.parse(JSON.stringify(data.data));
              //   }
              //   // console.log(this.copy_rows, "this.copy_rows");
              //   return;
              // }
      ...
      this.$refs['ruleForm'].validate(valid => {
        if (valid) {
          let params = {
            ...
            cusCateName: this.form.cusCateName,
            returnRate: this.form.returnRate
              ? this.form.returnRate / 100
              : null,
            parentCusCateId: this.parentCusCateId,
            haveChildren: this.form.haveChildren === true ? 0 : 1
          }
          console.log(params)
          params = this.$.dealObjectValue(params)

          if (!params.parentCusCateId) {
            // 因为是计算出来的父类目id，如果没有选择的话，不会计算,就给0
            params.parentCusCateId = 0
          }
          if (this.form.haveChildren === false) {
            // alert("params1");
            this.addFun(params)
          } else {
            // alert("params2");
            ...
            if (
              ...
              this.form.siteIds.length === 0
            ) {
              this.$message.warning('勾选了底层类目后，请选择国家')
            } else if (isOk === false) {
              this.$message.warning('请确认是否每一个映射都选择了亚马逊类目')
            } else {
              let params2 = {
                customCategoryMapList: this.table2
              }
              this.addFun({ ...params, ...params2 })
            }
          }
        }
      })
      ...
      this.$api[this.options.URL](params)
        .then(data => {
          ...
          this.$nextTick(() => {
            this.reset()
            this.getList()
            if (this.options.URL === 'CustomCategorySave') {
    ...
    resetFields_add() {
      ...
      this.reset()
      this.options.title = '添加平台类目'
      this.options.URL = 'CustomCategorySave'
      this.form.siteIds = []
    ...
    deleteTask(arg) {
      ...
      return this.$api['CustomCategoryRemove'](params)
        .then(data => {
          if (data.code === 0) {
            if (data.fail) {
              this.getList()
            ...
            if (data.usedList && data.usedList.length) {
              let name = []
              let cateIds = []
              data.usedList.forEach(e => {
                name.push(e.cateFullName)
                cateIds.push(e.cusCateId)
              })
              let reg = new RegExp(':', 'g')
              let newName = name.join(',').replace(reg, '/')
              setTimeout(() => {
                this.$confirm(
    ...
    edit__(arg, handlerType) {
      ...
      if (handlerType === 'edit') {
        this.options.title = '编辑平台类目'
        // alert(2);
        this.options.URL = 'CustomCategoryUpdate'
        this.handlerType = false
        this.options.editId = arg.cusCateId
      } else if (handlerType === 'add') {
        // alert(1);
    ...
    delete__(arg) {
      this.$confirm('此操作将删除该项, 是否继续?', '提示', {
        ...
        .then(() => {
          ...
          this.$api['CustomCategoryRemove'](params)
            .then(data => {
              if (data.code === 0) {
                if (data.fail) {
                  this.getList()
    ...
    showCate(scope) {
      ...
      // 开启弹窗
      this.options_cate.visible = true
    ...
    handler_Amazon_cate() {
      this.$refs['ruleForm_cate'].validate(valid => {
        if (valid) {
          this.options_cate.visible = false
          let key = this.form_cate.key
          let data = JSON.parse(JSON.stringify(this.form.customCategoryMapList))
          let item = data.find(el => {
            // 根据key找到某项
            return key === el.siteId
          })
          let item_index = data.findIndex(el => {
            // 根据key找到某项的下标
            return key === el.siteId
          })
          // 把pcid和category赋值给item 用作展示和提交
          item.pcid = this.form_cate.pcid
          item.category = this.form_cate.category
          // 把整个item改变到table2中
          this.table2.splice(item_index, 1, item)
          // pcid发生变化表示选择好了类目，把数据存到this.form.customCategoryMapList_copy中去，用于用户去掉某个国家站点的再次勾选回来时展示类目
          this.form.customCategoryMapList_copy.splice(item_index, 1, item)
          this.$store.commit('common/clearCate', new Date().getTime())
        }
      })
    },
  ...
  watch: {
    ...
    // 监听添加平台类目的弹窗，true则发起getList的请求，因为
