  ...
  methods: {
    ...
    _submit() {
	    return this.$refs.form.validate().then(() => {
			return this.$api[`ebProductCategoryGetCategory`]({parentPcid:this.formData.categoryId.slice(0).pop(),siteId: this.siteId,platformId: this.ebayPlatformId}).then(res=>{
				if(!res.data.length){
					return this.$api[`ebTaskCateDepthAysSave`]({
						categoryIds: [this.formData.categoryId.slice(0).pop()],
						platformId: this.ebayPlatformId,
						siteId: this.siteId
					})
				}else{
					this.$message.warning('请选择底层类目')
				}
			})
		})
    }
