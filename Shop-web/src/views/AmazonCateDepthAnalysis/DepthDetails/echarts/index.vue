...
export default {
  ...
  methods: {
    ...
    // 获取chartRank表格数据
    showChartChild(sellerId, startTime, endTime) {
      this.dialogLoading = true;
      let opts = {
        taskId: +this.$route.query.taskId,
        sellerId: this.chartData.sellerId,
      ...
      this.$api["TaskCateDepthAnalysisSouqListToNewReviewCnBySellerAmazon"](
        opts
      )
        .then(data => {
    ...
    changeTime(val) {
      if (!val) {
        this.showChartChild(this.chartData.sellerId);
        return;
      }
      // 有日期则往下走
      let startTime = this.DateMsg.DateValue[0],
        endTime = this.DateMsg.DateValue[1];
      this.showChartChild(this.chartData.sellerId, startTime, endTime);
    },
    change() {
      this.showChartChild(this.chartData.sellerId);
    },
