<!--  -->
<template>
  <div class="___common" v-if="$route.name === 'AmazonCateDepthAnalysis'">
    <template>
...
export default {
  ...
  provide() {
    return {
	  ...
	  refresh__item: this.refresh__item,
    };
  },
  data() {
    //   类目校验
  ...
  methods: {
    ...
    getList(arg) {
      ...
      return this.$api["TaskCateAnalysisSouqList2Amazon"](params)
        .then(data => {
    ...
    catchCate(arg) {
		this.$confirm('此操作将重新抓取,是否继续','提示',{type:'warning'}).then(()=>{
		  this.$api["TaskCateDepthAnalysisSouqReCrawlAmazon"](arg).then(data => {
			this.getList();
		  });
	  }).catch(()=>{})
    },
	refresh__item(arg){
		this.$confirm('此操作将重新抓取,是否继续','提示',{type:'warning'}).then(()=>{
			this.$api["TaskCateDepthAnalysisSouqReCrawlAmazon"]({taskIds:[arg.taskId]}).then(data => {
						this.getList();
			});
		}).catch(()=>{})
	},
    ...
    deleteTask(taskIds) {
      ...
      return this.$api["TaskCateAnalysisSouqRemove2Amazon"](params)
        .then(data => {
          if (data.code === 0) {
            return true;
          } else {
    ...
    // 类目清空表单
    resetFields_add() {
      this.options.URL = "TaskCateDepthAnalysisSouqSave2Amazon";
    ...
    //   // 通过编辑
    //   this.options.URL = "TaskCateAnalysisSouqUpdate2";
    ...
    delete__(arg) {
      this.$confirm("此操作将删除该项, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          this.$api["TaskCateAnalysisSouqRemove2Amazon"](params)
            .then(data => {
              if (data.code === 0) {
                let param = {
                  pageSize: 10,
