                    ...
                    <el-table @selection-change="handleSelectionChange" :data="showTableList" style="width: 100%" ref="tableRef" >
						...
						<el-table-column prop="keywordPrice" label="关键词竞价" width="300">
							<template slot-scope="scope">
								<el-input-number  v-model.trim="scope.row.keywordPrice"  size='mini'  style='width:200px;position: absolute;bottom: 10px;' clearable :controls='false' :precision="2" :min='0.1' :max="49"></el-input-number>
							</template>
						</el-table-column>
...
export default {
    ...
    methods: {
		_submit(){
			...
			if(keywords.filter(e=>e.bid == null).length) return this.$message.warning('请补全列表的关键词竞价')
			let params = {
				...this.storeIds,
				...this.params,
				keywords:keywords
			}
			return this.$api['adKeywordBrandSave'](params).then((res)=>{
