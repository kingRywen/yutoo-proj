    ...
    <div v-for="(item, index) in strategyDetail" class="flex">
      <ElFormItem label="时间段">
        <div class="flex">
          ...
          >
            <el-time-select
			  @focus="defaultPriceTip"
              style="width:130px"
              placeholder="起始时间"
              v-model="item.hourStart"
			  @change="changeTotalPriceMethod"
              :picker-options="{start: '00:00',step: '01:00',end: '23:00', minTime: index === 0 ? undefined : getMinTime(data.strategyDetail[index - 1].hourEnd)}"
            ></el-time-select>
          </ElFormItem>
		...
		<!-- 占比 -->
		<ElFormItem
		  v-if="ysTypeShow === 2 ? true : false"
		  label="占比"
		  label-width="60px"
		  class="custom-label-right"
		  style="margin-left: 40px;"
		  :prop="`strategyDetail.${index}.bidPct`"
		  :rules="perRule(item, index)"			  
		>
		  <ElInputNumber  @change="percentMethod($event,index)" v-model.trim.number="item.bidPct" :min="0"  :max="100" :controls="false"></ElInputNumber>%
		</ElFormItem>
      ...
      <ElFormItem
        ...
        label-width="100px"
        class="custom-label-right"
        :prop="`strategyDetail.${index}.bid`"
        :rules="[{ required: true, message: `请输入${title}` }]"
      >
        <ElInputNumber class="inputClass" @focus="defaultPriceTip('input',index)" @change="changeAllMethod(index)" :disabled="ysTypeShow === 2 ? true : false"  v-model="item.bid" :min="title==='关键词出价' ? 0.1 : 1" :precision="2" :max="title==='关键词出价' ? 49 : 999999999" :controls="false"></ElInputNumber>
      </ElFormItem>
  ...
  methods: {
    ...
    validate() {
      ...
      if (this.weekDay != null) {
        if (this.strategyDetail.length === 1) {
          let { bid, hourEnd, hourStart,bidPct } = this.strategyDetail[0]
    ...
    addItem(index) {
      ...
      // this.data.strategyDetail.splice(index + 1, 0, el)		
			this.data.strategyDetail.push(el)
    },
    delItem(index) {
	...
	changeAllMethod(index){						
		this.$nextTick(function(){	
			...
			if(this.ysTypeShow === 1){
				...
				let list = []
				type === 0 ? list = this.data.strategyDetail : list = this.data.strategyDetail.filter((e)=>{return e.weekDay === this.weekDay})
				let timeRange = 0
				let allPrice = 0
				list.forEach(e=>{
