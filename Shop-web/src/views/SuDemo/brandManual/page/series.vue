...
<script>
	...
	export default {
				...
				editBtns: [
						...
						dropList: [
							{
								name: '暂停',
								fn: () => {
									vm.showConfirm('暂停广告','paused','adCampaignBrandUpdate',1)
								}
							},
							{
								name: '归档',
								fn: () => {
									vm.showConfirm('归档广告','archived','adCampaignBrandUpdate',1)
								}
							},
							{
								name:'开启',
								fn: ()=>{
									vm.showConfirm('开启广告','enabled','adCampaignBrandUpdate',1)
								}
							}
						],
					},
					{
						...
						dropList: [{
								...
								fn: () => {
									let data = this.$refs.layout.selection
									this.showDialog('addBudget', '添加定时调整预算策略', data, false, '1010px')
								}
							},
							{
								name: '编辑',
								fn: () => {
									let data = this.$refs.layout.selection
									this.showDialog('addBudget',`编辑定时调整预算策略`,data,true,  '1010px')
								}
							},
							{
								name: '开启',
								fn: () => {
									vm.showConfirm('开启定时调整预算策略',1,'adManualUpdateStrategyStatus',2)
								}
							},
							{
								name: '关闭',
								fn: () => {
									vm.showConfirm('关闭定时调整预算策略',0,'adManualUpdateStrategyStatus',2)
								}
							},
						],
					},
		...
		computed: {
			...
			keyName() {
				return this.$route.name === 'newBrandAdKeyword' ? 'CPC' : '预算'
			},
		},
		methods: {
			...
			showDialog(componentName, title, row, edit, width = '600px') {
				...
				this.$refs.layout.$dialog({
					...
					params,
					width:width,
					component: () => import(`../dialogs/series/${componentName}.vue`),
					okBtnText:'确认',
					cancelBtnText:'取消'
				})
			},
			...
			lookAdEcharts(scope) {
				this.$refs.layout.$dialog({
					...
					width: '1200px',
					showMaxBtn: true,
					params: { strategyType: this.strategyType, objectId: scope.row.objectId },
					component: () => import('../dialogs/series/echartsDialog.vue')
				})
			},
			getSummaries(param) {
