<template>
    <div>
		...
		<div class="content">
			...
			<div class='cont mainForm'>
				<el-form label-position="top" label-width="80px" :model="formData" ref='formData' :rules="rules">
					...
					<div v-if="isShowError" style="color: #f56c6c;font-size: 12px;">关键词只能输入字母、数字、单引号,请重新编辑留下的关键词!</div>
		...
		<div class="content" style="margin-top: 20px;">
			<div class='title'>已添加{{tableData.length}}个关键词</div>
			<div class='cont'>
			    <div class='label'>
					<el-button type="primary" size='mini' plain @click="tableMethod('load')" style="margin-left: -10px;">加载推荐否定词</el-button>
				...
				<div v-if="tableData.length==0 && isShowTip" style="color: #f56c6c;font-size: 12px;">请添加至少一个关键词！</div>
			</div>
		</div>    
    </div>
</template>
...
export default {
    ...
    methods: {
		...
		addKeywordMethod(){
			this.$refs.formData.validate().then(()=>{
				...
				filterNewList && filterNewList.length ? this.$message.success('添加成功') : failList.length ? this.idid = 1 : this.$message.warning('存在相同的关键词')
		...
		tableMethod(e,scopeIndex){
			...
			switch(e){
				case 'load':
					this.$emit('changeChild7')
					this.$nextTick(function(){
						if(this.productIds && this.productIds.length === 0) return this.$message.warning('请添加至少一项商品') 
						if(this.tableData && this.tableData.length === 0) return this.$message.warning('请添加至少一个关键词')
						let params = {
							...this.storeIds,
							productIds:this.productIds,
							keywords:this.tableData.map((e)=>({
								keyword:e.keyword,
								matchType:e.matchType,
							}))
						}
						this.$api['keywordNegativeListByProduct'](params).then((res)=>{
							this.$message.success('操作成功') 
							// res.data = [{
							// 	keyword:'123',
							// 	matchType:'negativeExact'
							// }]
							let data1 = res.data.map(e=>({
								isOuter:true,
								...e
							}))						
							this.$emit('changeChild71',data1)
						})
					})					
				break;
				case 'more':
	...
	mounted(){
		if(this.$route.query && this.$route.query.keywords) this.formData.keywordInp = this.$route.query.keywords
	}
};
</script>
