	...
	arr.forEach((e, index) => {
		if (e.includes('>=')) {
			newArr.push({
				...
				second: '>=',			
				third: e.split('>=')[0] === 'clickRates' || e.split('>=')[0] === 'adConversionRates' ||  e.split('>=')[0] === 'acos' ? e.split('>=')[1] * 100 : e.split('>=')[1],
				fourth: e.includes('clickRates') || e.includes('adConversionRates') || e.includes('acos') ? '%' : e.includes('noDealClicks') ? '次' : null,
				value: e.split('>=')[0],
