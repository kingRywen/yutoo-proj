					...
					<div class="lebel">
						<draggable v-model="formData.expression" @start="drag=true" @end="drag=false" class="draggable">
							<div v-for="(item,index) in formData.expression" :key="index" class="formula-btn">
								<div class="btn-txt" v-if="item.key > 3"  style="display: flex;">
...
<script>
	...
	import {conditionResult} from 'Views/SuDemo/manual/js/adStop.js'
	export default {
		props: ['params'],
		data() {
			return {
				...
				ruleList:[
					...
					{label:'(',value:'(',key:2},
					{label:')',value:')',key:3},
					{label:'曝光量',value:'impressions',key:4},
					{label:'花费',value:'cost',key:5},
					{label:'点击量',value:'clicks',key:6},
					{label:'点击率',value:'clickRates',key:7},
					{label:'订单量',value:'adOrderCnt',key:8},
					{label:'转化率',value:'adConversionRates',key:9},
					{label:'ACOS',value:'acos',key:10},
					{label:'CPC',value:'cpc',key:11},					
				],
				symbolList:['>','<','=','>=','<='],
				outerRuleName:null,
		...
		methods: {
			...
			addBtnMethod(e,index){
				this.formData.expression.push({
					...
					second:null,
					third:null,
					fourth:e.key === 7 || e.key === 9 || e.key === 10 ? '%' : null,
					value:e.value,
					key:e.key
				})
			},
