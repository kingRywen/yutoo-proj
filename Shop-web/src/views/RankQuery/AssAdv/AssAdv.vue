...
export default {
  ...
  methods: {
    ...
    getList(arg) {
      ...
      return this.$api["TaskKeywordRankList4"](params)
        .then(data => {
    ...
    addFun(params) {
      ...
      this.$api["TaskKeywordRankSave4"](params)
        .then(data => {
    ...
    deleteTask(taskIds) {
      ...
      return this.$api["TaskKeywordRankRemove4"](params)
        .then(data => {
          if (data.code === 0) {
            return true;
          } else {
      ...
      this.$refs["ruleForm_add"].validate(valid => {
        if (valid) {
          ...
          if (this.options_add.type === "some") {
            ...
            if (API === "add") {
              Url = "TaskKeywordRankAddMonitor4";
            } else {
              Url = "TaskKeywordRankUpdateMonitor4";
            }
    ...
    cancelMoni(taskIds) {
      let params = { taskIds: taskIds };
      return this.$api["TaskKeywordRankRemoveMonitor4"](params)
        .then(data => {
          if (data.code === 0) {
            return true;
          } else {
    ...
    refresh__(arg) {
      ...
      this.$confirm("此操作将重新抓取该项数据, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$api["TaskKeywordRankReCrawl4"](params).then(data => {
            if (data.code === 0) {
              this.clearSearch = new Date().getTime();
            }
          });
    ...
    check__(row) {
      ...
      this.chartData.itemType = 4;
      this.chartData.type = 1;
      this.chartData.url = "TaskKeywordRankRtItemList4";
