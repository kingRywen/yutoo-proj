...
<template>
  <div class="___common">
    ...
    <!-- 图表 -->
    <el-dialog :close-on-click-modal="false"  :visible.sync="pcMonitoringData" height='505px' width="70%" @close="closeDialog" :fullscreen='dialogFullscreen'>
...
export default {
  ...
  methods: {
    ...
    getList(arg) {
      ...
      return this.$api["TaskKeywordRankList"](params)
        .then(data => {
    ...
    addFun(params) {
      ...
      this.$api["TaskKeywordRankSave"](params)
        .then(data => {
    ...
    deleteTask(taskIds) {
      ...
      return this.$api["TaskKeywordRankRemove"](params)
        .then(data => {
          if (data.code === 0) {
            return true;
          } else {
      ...
      this.$refs["ruleForm_add"].validate(valid => {
        if (valid) {
          ...
          if (this.options_add.type === "some") {
            ...
            if (API === "add") {
              Url = "TaskKeywordRankAddMonitor";
            } else {
              Url = "TaskKeywordRankUpdateMonitor";
            }
            ...
            Url = "taskKeywordRankUpdate";
          }
          let params = {
            frequency:this.form_add.frequency !== "" ? +this.form_add.frequency : "",
          ...
          this.$api[Url](params)
            ...
            .catch(err => {
              this.options_add.loading = false; //按钮loading
            });
        }
      });
    },
    ...
    cancelMoni(taskIds) {
      let params = { taskIds: taskIds };
      return this.$api["TaskKeywordRankRemoveMonitor"](params)
        .then(data => {
          if (data.code === 0) {
            return true;
          } else {
    ...
    refresh__(arg) {
      ...
      this.$confirm("此操作将重新抓取该项数据, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$api["TaskKeywordRankReCrawl"](params).then(data => {
            if (data.code === 0) {
              this.clearSearch = new Date().getTime();
            }
          });
    ...
    check__(row) {
      ...
      this.chartData.url = "TaskKeywordRankRtItemList";
