      ...
      <div class="box __box_wrapper">
        <el-form size="small" :model="form" :rules="rules" ref="ruleForm" style="width:94%;">
          <el-form-item label="选择类目" prop="category" :label-width="formLabelWidth" style="width:100%;">
            <el-cate :Cate.sync="form.category" :width="form.cateWidth" :HaveChildform.sync="form.haveChildren" :CategoryId.sync="form.categoryId"></el-cate>
          </el-form-item>

          <el-form-item label="产品ASIN" prop="skus" :label-width="formLabelWidth" style="width:100%;">
            <el-input type="textarea" v-model.trim="form.skus" :autosize="{ minRows: 2, maxRows: 6}" placeholder="多个产品ASIN换行输入（1-500个）" style="width:100%;" size="mini" />
            <div class="_fontSize" v-model="fontSum">已输入 {{fontSum}} 个</div>
          </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      clearfield:["taskName",'category','sku','monitorFlag','date'],
  ...
  methods: {
    ...
    getList(arg) {
      ...
      return this.$api["TaskKeywordRankList3"](params)
        .then(data => {
    ...
    addFun(params) {
      ...
      this.$api["TaskKeywordRankSave3"](params)
        .then(data => {
    ...
    deleteTask(taskIds) {
      ...
      return this.$api["TaskKeywordRankRemove3"](params)
        .then(data => {
          if (data.code === 0) {
            return true;
          } else {
      ...
      this.$refs["ruleForm_add"].validate(valid => {
        if (valid) {
          ...
          if (this.options_add.type === "some") {
            ...
            if (API === "add") {
              Url = "TaskKeywordRankAddMonitor3";
            } else {
              Url = "TaskKeywordRankUpdateMonitor3";
            }
    ...
    cancelMoni(taskIds) {
      let params = { taskIds: taskIds };
      return this.$api["TaskKeywordRankRemoveMonitor3"](params)
        .then(data => {
          if (data.code === 0) {
            return true;
          } else {
    ...
    refresh__(arg) {
      ...
      this.$confirm("此操作将重新抓取该项数据, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$api["TaskKeywordRankReCrawl3"](params).then(data => {
            if (data.code === 0) {
              this.clearSearch = new Date().getTime();
            }
          });
    ...
    check__(row) {
      ...
      this.chartData.itemType = 2; // 类目排名是2
      this.chartData.type = 1; // PC排名是1，APP是2
      this.chartData.url = "TaskKeywordRankRtItemList3";
      this.chartData.keywords = row.keywords;
      this.chartData.asin = row.sku;
