<template>
  <div>
	...
	<el-button size="small" plain  style="position:absolute;right:100px;top:80px;z-index: 999;" @click="selectBtn">{{btnText}}</el-button>
    <div v-if="isData">
  ...
  methods: {
    ...
    initChart(data,obj) {
	  ...
	  setTimeout(() => {
		  ...
		  let data = this.chart.getOption()
		  let isShowField = Object.values(this.chart.getOption().legend[0].selected).length
		  if(isShowField === data.legend[0].data.length){
			  this.btnText = '全选'
		  }else{
			  this.btnText = '取消全选'
		  }
    ...
    getData(obj) {
      ...
      this.$api[this.params.url](this.formInline).then(data => {
        if (data.code === 0) {
          ...
          this.initChart(this.echartsData,obj)
