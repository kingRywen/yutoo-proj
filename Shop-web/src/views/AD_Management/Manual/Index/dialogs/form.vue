<template>
  <ElForm :disabled="disabled" ref="form" style="flex: 1" size="small" :model="data" label-width="90px">
    <div v-for="(item, index) in strategyDetail" class="flex">
      ...
      >
        <ElInputNumber class="inputClass" @focus="defaultPriceTip('input',index)" @change="changeAllMethod(index)" :disabled="ysTypeShow === 2 ? true : false"  v-model="item.bid" :min="ysTypeShow===2 ? 0 : 1" :precision="2" :max="9999999999" :controls="false"></ElInputNumber>
      </ElFormItem>
