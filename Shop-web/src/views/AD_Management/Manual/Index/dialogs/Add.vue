  ...
  <ElForm :disabled="disabled" ref="form1" :model="data" size="small" label-width="90px" label-position="right">   
	<div style="display: flex;">
		...
		<div style="margin-left: 189px;" v-if="showSeries">
			  <ElFormItem :rules="[{ required: true, message: `请选择预算方式` }]" prop="oldBid" label="预算方式"  label-width="69px">
	...
	<div style="display: flex;">
	  ...
	  <div style="margin-left: 187px;" v-if="showSeries && data.bidType !== 0">
		  <ElFormItem  :rules="[{ required: true, message: `请输入默认预算` }]" prop="defaultAddBid" label="默认预算"  label-width="70px">
			  <ElInputNumber @change="defaultBidMethod" v-model.trim.number="data.defaultAddBid" :min="1"  :precision="2" :max="9999999999" :controls="false"></ElInputNumber>
		  </ElFormItem>
	  </div>
	</div>
    ...
    <template v-if="!data.seoType">
	  <div v-if="showSeries && data.bidType !== 0">
		  <ElFormItem prop="oldBid" label="总预算"  label-width="90px" :rules="totalPriceValidate(0)">
			  <ElInputNumber @change="totalBidMethod" :disabled="data.bidType === 1 ? true : false"  v-model.trim.number="data.totalBid"  :precision="2" :max="9999999999" :controls="false"></ElInputNumber>
		  </ElFormItem>
	  </div>
      <Form :seoTypeItem= 'data.seoType' :defaultPrice = "data.defaultAddBid" @getAllPriceMethod = 'getArrPrice' :ysTypeShow='data.bidType' :allPrice='data.totalBid' :disabled="disabled" :title="labelName1" :data="data" :strategyDetail="data.strategyDetail" ref="form2"/>
    </template>
    <!-- 周一到周日 -->
    <template v-else>
        ...
        <ElTabPane v-for="(item, index) in weeks" :label="item.label" :name="item.name" :key="index">
		  <div v-if="showSeries && data.bidType !== 0">
			  <ElFormItem prop="oldBid" label="总预算"  label-width="90px"  :rules="totalPriceValidate(1)">
				  <ElInputNumber  @change="totalBidMethod" :key="index" :disabled="data.bidType === 1 ? true : false"  v-model.trim.number="data.totalBidList[parseInt(item.name)]"  :precision="2" :max="9999999999" :controls="false"></ElInputNumber>
			  </ElFormItem>
		  </div>
          <Form
		    :seoTypeItem= 'data.seoType'
			:allPrice='data.totalBidList'
			:ysTypeShow='data.bidType'
			:defaultPrice = "data.defaultAddBid"
            :disabled="disabled"
            :title="labelName1"
            :data="item.data"
            :weekDay="+item.name"
            :strategyDetail="item.data.strategyDetail"
			@getAllPriceMethod = 'getArrPrice'
            ref="form3"
          />
        </ElTabPane>
      </ElTabs>
...
export default {
  ...
  methods: {
    ...
    _initData() {
      // console.log(123132);
	  if(this.params && this.params.strategyType === 0){
    ...
    setWeekDay(strategyDetail, save) {
      ...
      strategyDetail.forEach(el => {
		...
		el.totalBid = save ? this.data.seoType == 0 ? this.data.totalBid : this.data.totalBidList.find((e,index)=>{return index === el.weekDay}) : el.totalBid
      })
      ...
      return Promise.all(forms.map(form => form.validate()))
        .then(() => {
          let strategyDetail = [],
            ...
            {objectId, ...infos} = this.baseParams,
            params = {
              ...infos,
              ...info
            }
