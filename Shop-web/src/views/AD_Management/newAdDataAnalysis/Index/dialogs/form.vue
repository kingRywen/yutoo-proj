<template>
  <ElForm :disabled="disabled" ref="form" style="flex: 1" size="small" :model="data" label-width="90px">
    <div v-for="(item, index) in strategyDetail" class="flex">
      ...
      >
        <ElInputNumber v-model="item.bid" :min="title === '预算' ? 1 : 0.02" :precision="2" :max="9999999999" :controls="false"></ElInputNumber>
      </ElFormItem>
    ...
    validate() {
      ...
      if (this.weekDay != null) {
        if (this.strategyDetail.length === 1) {
          let { bid, hourEnd, hourStart } = this.strategyDetail[0]
          if (bid == null && hourEnd == null && hourStart == null) {
