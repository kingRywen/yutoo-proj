...
let sellerData = localStorage.getItem('shop_seller_data')

if (sellerData && !sessionStorage.getItem('shop_seller_data')) {
    sessionStorage.setItem('shop_seller_data', sellerData)
}
...
router.beforeEach((to, from, next) => {
    if (to.meta.title) {
        document.title = `${to.meta.title}__店铺优化`
    }
...
Vue.mixin({
    ...
    methods: {
        _formatData(str, row) {
            return (row[str] != null && row[str] !== '') ? row[str] : '-'
        },
		moreRowText(str){
    ...
    computed: {
        ...
        sellerData() {
            ...
            return JSON.parse(sessionStorage.getItem("shop_seller_data")) || this.$store.state.common.sellerData
        },
        sellerId() {
            // return this.$store.state.common.sellerData.sellerId;
            return this.sellerData && this.sellerData.sellerId
        },
        siteId() {
            // return +this.$store.state.common.sellerData.siteId;
            return this.sellerData && this.sellerData.siteId
        },
        platformId() {
            // let type = this.$store.state.common.routerType
            //          return type==='amazon'?1:2
            return (this.sellerData && this.sellerData.platformId) || 1
        },
        ebayPlatformId() {
            return 2
        },
