<template>
  <div class="main-layout" :key="$route.name">
    <ElRow :gutter="20">
      ...
      >
        <template>
          <CollapseWrapper
            ref="collapse"
            v-if="Object.keys(searchFields).length"
            :backBtn="backBtn"
            :expand="expand"
            class="mb10"
          >
        ...
        <div class="tabs mb10">
          ...
          <!-- 最上面的按钮 -->
          <div ref="btn" v-else-if="!simple || hiddenTopBtn" class="top-btn">
            <!-- 左边批量 -->
        ...
        <div class="table-wrapper">
          <slot name="table">
            ...
            >
              <template slot="right">
                <el-table-column label="操作" fixed="right" align="left" :width="editWidth" v-if="edits.length">
    ...
    return {
      ...
      innerEvents: {
        ...
        handleOkClick() {
          ...
          let res = component._submit(vm.$refs.Dialog)
          if (!res || !res.then) {
  ...
  methods: {
    ...
    $dialog({
      ...
      component().then(com => {
        ...
        vm.innerDialogOpts.height = height
    ...
    //请求列表数据
    getList(_params = {}, resetSearch = false, fromSearch = false) {
      console.log('获取数据')
      ...
      if (!this.url) return
      // console.log(this.lastSearchVal, params)
      if (
        fromSearch &&
        this.lastSearchVal &&
