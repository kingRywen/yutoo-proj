<template>
  <div>
    ...
    <WidgetCardWrapper :isCollapse="false" :defaultHide="false" title="买家偏好">
    ...
    <template v-if="(value1.taskTypeId == 2 || value1.taskTypeId == 3) && taskNum">
...
export default {
    ...
    return {
      ...
      base: {
        ...
        storeId: {
          ...
          options: !this.view
            ? '$store.common/activeStoreList'
            : '$store.common.storeList',
          handler: {
      ...
      task: {
        ...
        relHandle: {
          ...
          label: '关联操作',
          hidden(data) {
            return !data.taskTypeId || data.taskTypeId > 5
          },
          widget: 'select',
          vertical: true,
          // innerTips: '（佣金：$10/单）',
        ...
        reviewSupplyType: {
          ...
          hidden(data) {
            return (
              !data.taskTypeId || (data.taskTypeId == 1 || data.taskTypeId > 3)
            )
          },
          change: data => {
  ...
  methods: {
    ...
    getMax() {
      this.$api[`agent/taskSourceMaxTaskCount`]({}).then(data => {
    ...
    getMoney() {
      let vm = this
      vm.$api[`agent/taskSourceTaskMoney`]({}).then(data => {
        ...
        vm.task.taskTypeId.options = this.$const['OTHER/taskType'].map(
          (el, index) => {
    ...
    getEditData() {
      this.isEdit = true
      this.$api[`agent/taskSourceInfo`]({ id: this.$route.query.id }).then(
        data => {
          let {
            categoryMap,
            platformCode,
          ...
          this.value1 = {
            ...
            ...info,
            platformSiteId: [
              platformCode.charAt().toUpperCase() + platformCode.slice(1),
              platformSiteId
            ],
